...
<li><span>沉淀的梦想</span> 👍（3） 💬（2）<div>为什么nums1和nums2的内存位置有重叠就不能进行矢量化了呢？</div>2019-11-04</li><br/><li><span>pebble</span> 👍（2） 💬（1）<div>支持 AVX-512 指令（也就是它的一个寄存器有 512 位）。每次能处理 8 个 64 位整数，或 16 个 32 位整数，或者 32 个双精度数、64 个单精度数。你想想，一条指令顶 64 条指令，几十倍的性能提升，是不是很厉害！

这句里，最后的32个双精度跟64个单精度数量不对啊，双精度是64位的，512字节应该是8个，单精度也就是16个了</div>2019-11-08</li><br/><li><span>dll</span> 👍（0） 💬（1）<div>gcc -mavx2 simd2.c -o simd2 这部分代码是不是在m1上没法执行
</div>2022-07-27</li><br/><li><span>吃饭</span> 👍（0） 💬（0）<div>python里的pandas处理数据那么快是不是用的就是这个</div>2021-06-28</li><br/>
</ul>