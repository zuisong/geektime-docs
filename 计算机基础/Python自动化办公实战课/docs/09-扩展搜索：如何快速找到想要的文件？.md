...
**首先我来带你看一下glob()函数和它的参数**, 由于glob()进行匹配的是文件的路径和文件名称方式，如: "c:\\somepath\\to\\filename\_include\_BBC\_voice.exe" , 而我们进行文件搜索的时候一般会使用关键字，如“BBC”，因此在搜索时我们需要为关键字加上通配符的形式，如“*BBC*” 。
...
<li><span>回到原点</span> 👍（2） 💬（1）<div>def search_ext(file_ext):
    with open(dbfile, encoding=&#39;utf-8&#39;) as f:
        for line in f.readlines():
...
if __name__ == &quot;__main__&quot;:
    search_ext(&#39;.jpg&#39;)</div>2021-03-28</li><br/><li><span>Geek_cdcb96</span> 👍（0） 💬（1）<div>def write_to_db():    
&quot;&quot;&quot;    写入索引文件    &quot;&quot;&quot;    
current_path = pathlib.PurePath(__file__).parent    
dbfile = current_path.joinpath(&quot;search.db&quot;)    
with open(dbfile, &#39;w&#39;, encoding=&#39;utf-8&#39;) as f: 
...
还有一个问题，我使用locate_file()函数对文件较多的目录进行搜索时，显示直接跳出了，没有返回目录中的文件，但是在命令行条件下可以返回。是怎么回事。</div>2022-05-28</li><br/><li><span>Soul of the Dragon</span> 👍（0） 💬（3）<div>老师，我按照您之前说的，用.py格式的脚本文件运行“current_path = pathlib.PurePath(__file__).parent”这一行代码，为什么还是会出现“NameError: name &#39;__file__&#39; is not defined”这样的报错？</div>2021-03-02</li><br/><li><span>ifelse</span> 👍（0） 💬（0）<div>学习打卡</div>2023-07-08</li><br/><li><span>琥珀·</span> 👍（0） 💬（0）<div>expanded_name = &quot;jpg&quot;
