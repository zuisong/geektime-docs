...
<li><span>朱晓峰</span> 👍（9） 💬（5）<div>你好，我是朱晓峰，下面我就来公布一下上节课思考题的答案：
...
(
      ...
      memberphone TEXT,
      memberpoints INT
);</div>2021-03-29</li><br/><li><span>yaomon</span> 👍（19） 💬（1）<div>我们可以定义字段“barcode”满足唯一性约束。这样一来，条码就不能重复，但是可以为空，而且只能有一条记录条码为空。
-------------
null和任何值不相等，包括null，所以不是可以有多条记录都是null值吗，是8.0规则变了？</div>2021-03-14</li><br/><li><span>lesserror</span> 👍（9） 💬（1）<div>朱老师，周末好。
...
另外，上节课的思考题答案老师按照惯例，麻烦评论区给出一下。方便学习的同学做一个参考呗。</div>2021-03-13</li><br/><li><span>Jun</span> 👍（6） 💬（1）<div>想请教一下老师，为什么 importquantity 用decimal 而不是int,难道进货数量会有小数吗？</div>2021-03-24</li><br/><li><span>右耳朵猫咪</span> 👍（4） 💬（3）<div>老师好，创建表字段的时候是不是应该把多个单词用下划线分开比较好？比如，listnumber改成list_number。</div>2021-03-15</li><br/><li><span>张成虎</span> 👍（2） 💬（1）<div>老师好，思考题并没有修改字段名称，为什么用的是change，不用modify？</div>2021-06-23</li><br/><li><span>星空下</span> 👍（2） 💬（1）<div>这节好实用，都是开发中在用的操作</div>2021-03-13</li><br/><li><span>SharpBB</span> 👍（1） 💬（1）<div>笔记来了
...
3.修改表
	...
	修改表字段
		...
		只改变字段类型
			如 alter table test,user modify id int;</div>2022-02-08</li><br/><li><span>momoko</span> 👍（1） 💬（1）<div>ALTER demo.goodsmaster modify salesprice DECIMAL(10,3) UNIQUE NOT NULL;
</div>2021-08-09</li><br/><li><span>小鱼干</span> 👍（1） 💬（2）<div>int类型，不写字段大小和写字段大小实际存储是一样吗</div>2021-03-15</li><br/><li><span>不管</span> 👍（0） 💬（1）<div>怎么没有介绍提到的各种约束怎么写，能不能加个示例</div>2022-07-11</li><br/><li><span>奕</span> 👍（0） 💬（1）<div>表中的字段顺序 有什么作用的？ 比如创建表的时候指定字段顺序 或者  add column 时候 after </div>2022-06-11</li><br/><li><span>shk1230</span> 👍（0） 💬（1）<div>思考题： 请你写一个 SQL 语句，将表 demo.goodsmaster 中的字段“salesprice”改成不能重复，并且不能为空。
答案：alter table demo.goodsmaster
modify salesprice DECIMAL(5,2) not null unique;</div>2022-03-10</li><br/><li><span>SharpBB</span> 👍（0） 💬（1）<div>思考题：
alter table demo.goodsmaster modify price decimal(5,2) not null unique;
似乎一定要加个类型才能更改
</div>2022-02-08</li><br/><li><span>彭彬</span> 👍（0） 💬（1）<div>alter table emo.goodsmaster modify salesprice unique
alter table emo.goodsmaster modify salesprice not null</div>2021-09-07</li><br/>
</ul>