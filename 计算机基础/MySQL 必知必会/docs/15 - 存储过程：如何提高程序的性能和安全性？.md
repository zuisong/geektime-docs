...
</div>2021-05-17</li><br/><li><span>undefined</span> 👍（7） 💬（1）<div>#不负责任的评论#
...
不过对于商用数据库和部分传统软件业务，还是值得使用的。</div>2021-04-13</li><br/><li><span>lesserror</span> 👍（3） 💬（1）<div>一个番茄钟，补上这一讲的内容。最近实在太忙了。

另外，需要指出这里老师的存储过程代码中的参数类型是TEXT，而不是文中所说的DATETIME类型。

像视图、存储过程、游标......这些概念从伴随MySQL很久了，虽然实际开发中很少用到。但是我们也必须了解，建立起自己完整的知识体系。</div>2021-04-15</li><br/><li><span>floating</span> 👍（2） 💬（2）<div>课后思考题：
...
|    6 |
+------+
1 row in set (0.00 sec)</div>2021-04-19</li><br/><li><span>Jun</span> 👍（1） 💬（2）<div>老师能否解释一下，为什么输入参数trandate的类型用text而不是date?</div>2021-07-13</li><br/><li><span>追忆似水年华</span> 👍（1） 💬（1）<div>在对时间进行判断那里，“b.transdate&gt;startdate AND b.transdate&lt;enddate” 这条语句漏掉了当天 00:00:00 这个时刻的记录，是不是改写成 “b.transdate&gt;=startdate AND b.transdate&lt;enddate” 更合适一些？

另外“这里我使用 CASE 函数来解决销售金额为 0 时计算毛利的问题”，这里应该是解决销售金额为 0 时，计算毛利率的问题吧？</div>2021-06-06</li><br/><li><span>giteebravo</span> 👍（1） 💬（1）<div>
本节课需要掌握关于存储过程的
创建&#47;查看&#47;调用&#47;修改&#47;删除&#47;调试
其中调试需要重点关注
</div>2021-04-14</li><br/><li><span>Geek_3254e0</span> 👍（0） 💬（1）<div>CREATE TABLE IF NOT EXISTS aplus1(
a VARCHAR(5),
b VARCHAR(5)
...
CALL plus8();
SELECT * from bplus;</div>2022-04-28</li><br/><li><span>菜鸟🐤要先飞</span> 👍（0） 💬（1）<div>delimiter &#47;&#47;
...
set @b = 0;
call aAddB(1,@b);
begin</div>2021-05-12</li><br/><li><span>Sinvi</span> 👍（0） 💬（1）<div>DELIMITER &#47;&#47;
