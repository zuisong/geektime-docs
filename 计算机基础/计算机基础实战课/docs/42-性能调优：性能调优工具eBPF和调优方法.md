...
<li><span>LockedX</span> 👍（2） 💬（2）<div>eBPF有访问内核的权限，如果被误用，后果不堪设想，所以要谨慎合理的使用eBPF。</div>2022-11-07</li><br/><li><span>熊悟空的凶</span> 👍（1） 💬（1）<div>eBPF 对系统有侵入性吗，是类似于Agent技术么</div>2022-11-07</li><br/><li><span>童谣</span> 👍（0） 💬（1）<div>这个怎么用？比如我要做一些性能分析，类似io、cpu突刺较高，ebpf有帮助吗？</div>2023-11-09</li><br/><li><span>苏流郁宓</span> 👍（0） 💬（1）<div>eBPF通过安全认证等尽可能排除无用的干扰，但问题是也增加了复杂度。
也就是eBPF应该在关键时候用，而不是频繁的滥用。滥用容易增加系统的复杂度！技术是中性的，有时解决一个问题也容易增加新的问题！越复杂就越容易出错，普通电脑大不了关机重启，但是对数据库电脑，应该是能减则减。性能和稳定兼顾（就比如双11数据库电脑哪怕停几分钟就是大损失）</div>2022-11-07</li><br/><li><span>peter</span> 👍（0） 💬（1）<div>请教老师两个问题：
Q1：BPF比协议栈还优先获取数据报吗？
