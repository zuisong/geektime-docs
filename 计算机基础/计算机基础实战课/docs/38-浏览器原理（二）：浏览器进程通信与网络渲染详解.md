...
<li><span>LockedX</span> 👍（1） 💬（1）<div>浏览器无法知道JS代码会对DOM和CSSDOM操作什么，所以要等JS执行完再渲染页面。尽量把&lt;script&#47;&gt;放在HTML最后执行，对用户来说可能体验好一点。</div>2022-10-28</li><br/><li><span>peter</span> 👍（0） 💬（1）<div>请教老师几个问题：
Q1：一个网络还是多个网络？
...
Q3：mach port是什么通信方式？
Q4：chrome中进程间通信没有采用消息队列，是吗？</div>2022-10-28</li><br/><li><span>苏流郁宓</span> 👍（0） 💬（1）<div>由于cpu（单核），始终实时只能指向一个栈的入口（可以切换不同的栈）
