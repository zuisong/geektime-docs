...
<li><span>苏流郁宓</span> 👍（1） 💬（2）<div>好像上一节问过，储存字节指令，双字节指令，字指令。数据符号可以通过指令翻译来实现，没必要在储存环节特意把有符号和无符号数据分开储存的啊</div>2022-09-14</li><br/><li><span>LockedX</span> 👍（0） 💬（1）<div>既然加载指令已经解决了符号数的问题，存储就不用管这个问题，就存储二进制数据就可以啦</div>2022-09-16</li><br/><li><span>Bean</span> 👍（0） 💬（0）<div>加载指令，CPU无法预测该怎么补高位，于是需要编译器&#47;程序员通过不同指令来区分。而存储指令，CPU只需要截取不需要的高位，简单粗暴，截多少是编译器&#47;程序员的事情，截错了也是编译器&#47;程序员的锅，不是CPU的执行问题。</div>2023-01-16</li><br/>
</ul>