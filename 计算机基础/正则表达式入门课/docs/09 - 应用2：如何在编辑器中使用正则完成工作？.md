...
<li><span>虹炎</span> 👍（7） 💬（1）<div>课后练习后总结：
uniq -c：   统计每行出现次数
sort :
	-n  按数字排序
	-r	逆序排序
...
head -n10： 取前10行数据</div>2020-07-01</li><br/><li><span>William</span> 👍（6） 💬（1）<div>ggrep -Po &#39;[\w\.][\w\.\,\-]*\b&#39; 09.words.txt | sort | uniq -c | sort -nr &gt; 09.cnts.txt

找了一篇infoQ的文章实验，开头可能会以 . 开头，如 .NET；</div>2020-07-05</li><br/><li><span>白了少年头</span> 👍（5） 💬（1）<div>我的理解是：正则表达式就像是内功，正则在高级语言和编辑器中的用法就像是外功招式，内功和外功要结合起来使用才能成为武林巅峰！</div>2020-07-01</li><br/><li><span>wenxuan</span> 👍（3） 💬（1）<div>直接统计文章单词频次TOP10：
grep -Po &#39;\w+&#39; article.txt | sort | uniq -c | sort -nr | head -10</div>2020-07-01</li><br/><li><span>134</span> 👍（1） 💬（1）<div>匹配正则：([^\s]+\s)
替换为:$1\n</div>2020-07-02</li><br/><li><span>Bug? Feature!</span> 👍（0） 💬（1）<div>学会正则表达式，可以提高我们的工作效率，这个是毋庸置疑的，跟着老师一起打怪升级！</div>2020-11-08</li><br/><li><span>Robot</span> 👍（0） 💬（1）<div>1、先处理文本为一行一个单词，编辑器为VS code
...
cat result.txt | sort -fnrk1 | uniq -c | sort -nrk1 | head -n10</div>2020-07-01</li><br/><li><span>Eric@D.F.</span> 👍（1） 💬（0）<div>在编辑多行时，如果我们需要编辑的内容都是纵向上同一个位置，就可以使用 Alt (macOS 上是 Option）加上鼠标拖拽的方式来选择（或者尝试按下鼠标中键拖拽）
