...
<li><span>我来也</span> 👍（8） 💬（1）<div>浓缩的都是精华!
...
</div>2020-09-04</li><br/><li><span>我来也</span> 👍（4） 💬（2）<div>想不到`git reset --hard`还能不带参数.
...
就是因为这个用的太顺手了,导致有些场景我都不用`Fugitive`了.</div>2020-09-04</li><br/><li><span>哒哒爱夏</span> 👍（0） 💬（1）<div>老师您好，我本来的配置让我在 vim 分屏时有竖直分割线和水平分割线的，我加上了 AirLine 之后，只有竖直分割线了，水平分割线变成了 AirLine 提供的当前文件的信息，怎么配置能让我的水平分割线回来呢？</div>2023-05-24</li><br/><li><span>谁家内存泄露了</span> 👍（0） 💬（1）<div>请问老师，有没有能实现cmake的自动补全功能的推荐呀，项目中时不时的会修改cmake，平台移植也会需要。。</div>2022-10-09</li><br/><li><span>谁家内存泄露了</span> 👍（0） 💬（3）<div>吴老师好，
我的操作如下：
1 在源码文件的结尾新创建两行，如：
}
...
我猜想vim应该是在存储的时候做了一些动作，但我期望让他怎么继续保存两行，请问老师是否有什么建议？</div>2022-09-21</li><br/><li><span>剑米</span> 👍（0） 💬（1）<div>老师，请教一下，在编译服务器安装了vim-airline插件，但是putty远程登录上去的状态栏是显示乱码的，按照网上的说法是要更新字体，但是折腾了一圈一直搞不定。我的编译服务器是ubuntu12.04的。最近忽然迷惑了，这个字体我是要安装到服务器上，还是安装在远程登录的windows这边</div>2020-12-09</li><br/><li><span>pyhhou</span> 👍（0） 💬（1）<div>赞，都是非常有用的插件，������

有一个疑问，在使用像 :Git，:PackUpdate 这样的命令。额外的信息会通过新窗口的形式呈现。在当前的配置下，默认都是水平分割窗口。如何改为默认竖直分割窗口呢？（记得之前使用 :PackUpdate 安装插件后，信息都是通过竖直分割的窗口呈现，不知道是不是因为我们配置了一些东西而导致的）尝试过在 .vimrc 文件中加 set spr，但是貌似不起作用</div>2020-09-08</li><br/><li><span>若离 </span> 👍（0） 💬（1）<div>\ 作为快捷键感觉不是很好按到，而且键盘还是 HHKB 哈哈哈</div>2020-09-07</li><br/><li><span>ChamPly</span> 👍（0） 💬（1）<div>都是正在使用的插件:)</div>2020-09-04</li><br/><li><span>YouCompleteMe</span> 👍（0） 💬（1）<div>老师，有什么愉快休闲插件推荐吗😬</div>2020-09-04</li><br/><li><span>瀚海星尘</span> 👍（0） 💬（0）<div>Visual-Multi 完全没用过，这个就是我一直想找的等价于 vscode 里面的批量修改的工具哈！其他几个插件除了 我用的是 lightline 外，之前都装了。但是之前只会最基础的用法，这次学了不少，感觉敲代码要飞的更高了������</div>2020-10-24</li><br/>
</ul>