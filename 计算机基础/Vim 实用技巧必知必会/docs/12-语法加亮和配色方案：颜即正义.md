...
<li><span>我来也</span> 👍（5） 💬（2）<div>今天收获最大的就是这个`termguicolors`参数了.
...
</div>2020-08-21</li><br/><li><span>Geek_fd61d6</span> 👍（0） 💬（1）<div>老师您好，请问为何在 wsl2 ubuntu22 环境下，$COLORTERM 终端下为空，此时设定 termguicolor 是否失败。</div>2024-03-18</li><br/><li><span>YouCompleteMe</span> 👍（0） 💬（1）<div>老师，vim-copy-as-rtf在不支持的平台上会 echomsg &#39;Cannot load copy-as-rtf plugin: unsupported platform&#39;,比如我使用的WSL。我在多个平台共用的一份vimrc，如果在vimrc里判断平台，会增加很多代码。可不可以增加一个option，在不支持的平台，直接finish。</div>2020-09-09</li><br/><li><span>pyhhou</span> 👍（0） 💬（2）<div>老师给的这 3 个配色方案是直接用插件下载安装后就可以生效吗？我试了一下，下载了第 3 个插件（mbbill&#47;desertEx）下来，Mac 下的图形界面的配色还是没有改变。试着在配置文件里加了 set bg=dark 还有 syntax on，但是不起作用。

用 vim-scripts&#47;SyntaxAttr.vim 插件查看 .vimrc 文件中的关键字，输出是

group: vimCommand-&gt;Statement guifg=#ffff60(#ffff60) gui=bold

主要是 Mac 图形界面的关键字是深紫色，高亮和不高亮区别不是特别明显。还是想弄个清晰一点的配色，望老师指点������</div>2020-08-28</li><br/><li><span>AirY</span> 👍（0） 💬（2）<div>大佬，求帮助，我配置Vundle插件后，sudo vim会报错</div>2020-08-24</li><br/><li><span>我来也</span> 👍（2） 💬（0）<div>课后思考题:
...
endfun
```
</div>2020-08-21</li><br/><li><span>瀚海星尘</span> 👍（0） 💬（0）<div>如果之前的课程是“酷”话，这节课后就开始“炫”了😄。终端的色彩实现方案，感觉很有意思啊~</div>2020-09-26</li><br/>
</ul>