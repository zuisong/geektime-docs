...
<li><span>青玉白露</span> 👍（7） 💬（1）<div>这门课我追定了！宫老师加紧更新哈，内容很赞</div>2021-08-09</li><br/><li><span>千无</span> 👍（1） 💬（1）<div>很赞，课后习题一定要做，这才是这门课的精髓，实践它掌握它，课就超值</div>2021-08-10</li><br/><li><span>全国第一菜</span> 👍（1） 💬（1）<div>偶然间习得宫老师的前两门课，如获至宝，对个人提升显著。今天看到开新课了，第一时间来支持，因为知道自己又要变强了！！加油</div>2021-08-10</li><br/><li><span>有学识的兔子</span> 👍（0） 💬（1）<div>给宫老师点赞，老师提供出来的课程都是夯实编程能力的知识。工作中功能业务围绕业务展开，基于成熟的框架设计模式做功能开发或局部优化，过程中也隐约感受缺失对编程世界整体了解带来的麻烦。之前也学习了一点点宫老师《编译原理之美》，很庆幸感受这门实战课。Fighting !</div>2021-08-11</li><br/><li><span>_______R</span> 👍（0） 💬（2）<div>宫老师，我之前看过一点点 ts，现在主学 go，今天刚把 01 的代码用 go 重写，可以提交到码云上吗</div>2021-08-11</li><br/><li><span>qinsi</span> 👍（0） 💬（1）<div>if嵌套看着脑阔疼，建议扩充token类型，增加一些helper方法：
...
```
</div>2021-08-10</li><br/><li><span>被讨厌的勇气</span> 👍（5） 💬（0）<div>递归思想，在程序中常见的实现是：自己调用自己，即函数调用自身。
递归思想的本义是：将一个复杂问题分解成规模较小的几个子问题。从递归思想的角度来看，递归下降算法将语法分析分解为函数声明分析、函数调用分析，且子问题可以进一步分解，所以称之为“递归”。</div>2021-10-15</li><br/><li><span>张贺</span> 👍（4） 💬（2）<div>代码中RefResolver中的visitFunctionBody方法不应该return吧</div>2021-08-11</li><br/><li><span>奋斗的蜗牛</span> 👍（2） 💬（0）<div>期待后面后端的内容，一直卡在这块，找不到学习的思路，老师水平真高，深入浅出！！</div>2021-08-16</li><br/><li><span>小先生</span> 👍（1） 💬（0）<div>“问题 2：在今天的课程里，我们的语法分析的算法是“递归下降”算法。不知道你有没有发现，我们的程序里并没有出现函数的递归调用呀，为什么还要说它是递归的呢？”

回答：递归下降算法里的递归，主要指的是语法规则需要重复递归使用，因此称其为递归。</div>2023-10-25</li><br/><li><span>Geek_9a1bc6</span> 👍（1） 💬（0）<div>代码node命令执行没问题，用tsc命令编译报错： Type &#39;void&#39; is not assignable to type &#39;FunctionCall&#39;. 有没有遇到的童鞋？</div>2021-08-26</li><br/><li><span>Geek_c13jnv</span> 👍（1） 💬（0）<div>请问代码修改后怎么运行呢</div>2021-08-20</li><br/><li><span>🇵 🇦 🇳 🇩 🇦</span> 👍（1） 💬（0）<div>太牛了！</div>2021-08-10</li><br/><li><span>天涯</span> 👍（0） 💬（0）<div>大部分脚本语言，比如python，javascript，ruby，lua，他们的虚拟机都是用C写的，C语言处理这些脚本语言字节码的时候，是否需要经过C语言编译器再编译运行一次，然后在机器CPU上执行呢？</div>2023-05-07</li><br/><li><span>wilson</span> 👍（0） 💬（0）<div>是不是少内容了啊，解释器也没说具体是怎么做的啊，一看示例代码，多出来很多代码，课程里没讲</div>2023-01-31</li><br/>
</ul>