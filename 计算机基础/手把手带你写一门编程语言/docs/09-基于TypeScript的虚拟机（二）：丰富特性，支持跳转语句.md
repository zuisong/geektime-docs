...
<li><span>惜心（伟祺）</span> 👍（0） 💬（0）<div>从计算机执行的指令来看语言设计</div>2022-11-17</li><br/><li><span>ifelse</span> 👍（0） 💬（0）<div>满意，学习打卡</div>2022-09-13</li><br/><li><span>chris</span> 👍（0） 💬（1）<div>文中关于分支和跳转指令的麻烦, 其实都是源于跳转目标是一个绝对地址, 如果设计成一个相对偏移的话, 其实就不存在了. 并且java字节码ifxxx和goto后面跟的也都是相对偏移. 这里不知道老师为什么要设计成绝对地址, 出于什么考虑?</div>2021-09-01</li><br/><li><span>闫志刚</span> 👍（0） 💬（0）<div>念念不忘，必有回响</div>2021-09-01</li><br/><li><span>pedro</span> 👍（0） 💬（0）<div>虚拟机的本质就是：在软件层面上再造物理机。

Java 字节码可以看作是对汇编的一种模仿、简化和实现。</div>2021-08-29</li><br/><li><span>有学识的兔子</span> 👍（0） 💬（0）<div>是不是和老师提示的那样，使用C&#47;C++来编写虚拟机,以获得转译和运行字节码上性能的提升。</div>2021-08-29</li><br/><li><span>qinsi</span> 👍（0） 💬（0）<div>因为最终是编译成js运行的，可以用node —prof看下有没有明显的性能瓶颈</div>2021-08-28</li><br/>
</ul>