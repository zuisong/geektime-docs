...
<li><span>ifelse</span> 👍（0） 💬（0）<div>学习打卡</div>2022-09-23</li><br/><li><span>blackonion</span> 👍（0） 💬（0）<div>rust也可以，例如let mut maybe_some = None；maybe_some = Some(666)  编译器可以算出maybe_some的类型是Option&lt;i32&gt;。</div>2021-10-12</li><br/><li><span>奋斗的蜗牛</span> 👍（0） 💬（0）<div>之前看typescript编译器的类型检验代码，看得很晕，里面的类型推导是真复杂，谢谢老师讲到这块</div>2021-10-08</li><br/>
</ul>