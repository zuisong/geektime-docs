...
<li><span>艾恩凝</span> 👍（1） 💬（1）<div>以前作为使用者去写驱动，现在以提供者的角度去分析驱动，又来到了熟悉的file_operations结构了，写过简单的内核2.6 和 3.4版本的驱动，每个版本都稍有区别，盲猜这个是5版本的，毕竟有些陌生的函数，刚才看到了一个评论驱动是如何操作硬件的，来个最简单的按键驱动就知道了，这还没谈谈设备树，设备树也应该谈谈，到底是哪个大聪明想到的设备树</div>2022-05-09</li><br/><li><span>neohope</span> 👍（10） 💬（1）<div>
关于驱动程序Demo
一、miscdrv是一个内核模块
...
4、T1：sudo .&#47;app
5、T2：ctrl+c
6、T1：sudo rmmod miscdrv.ko</div>2021-08-06</li><br/><li><span>neohope</span> 👍（10） 💬（1）<div>关于数据结构
一、目录组织相关结构
...
通过file_operations获取了open函数指针，并进行了调用</div>2021-08-06</li><br/><li><span>青玉白露</span> 👍（6） 💬（2）<div>在Linux系统中，sudo可以获取超级用户的权利，它之后的命令可以在内核态下进行工作。
而加载miscdrv.ko模块和app测试都需要在内核态下进行。</div>2021-07-19</li><br/><li><span>pedro</span> 👍（2） 💬（1）<div>加载内核模块，使用内核驱动，得 sudo 权限</div>2021-07-19</li><br/><li><span>Geek_Lawrence</span> 👍（1） 💬（1）<div>驱动程序仍然是“软件”部分，“软件”如何驱动“硬件”，这部分能否更细致点，想了解下驱动程序是如何连接硬件并且驱动硬件工作的，其更加底层的工作原理是什么呢？</div>2022-01-29</li><br/><li><span>Zhang</span> 👍（0） 💬（1）<div>终于看到这了，看到曙光了感觉，打个卡，感觉今天课后作业简单了</div>2022-06-25</li><br/><li><span>Mingjie</span> 👍（0） 💬（2）<div>老师，HDMI驱动是属于哪类驱动设备？我觉得是块类型设备，不是有显存这个说法嘛，我想的对吗</div>2021-11-28</li><br/><li><span>驰往</span> 👍（0） 💬（1）<div>既陌生又熟悉的代码。曾几何时，大学的时候就是从platform device入手，深入学习驱动设备模型，然而由于工作环境，越工作越上层，最终没有踏入内核这片圣地。如今再学操作系统，希望能够借此机会，了却当年的初心😄。</div>2021-08-31</li><br/>
</ul>