...
<li><span>neohope</span> 👍（30） 💬（4）<div>一、整理一下结构
...
4、此时，应用程序就可以访问该虚拟地址了</div>2021-06-27</li><br/><li><span>pedro</span> 👍（14） 💬（2）<div>代码已经告诉我们一切:

&#47;&#47;如果异常号等于14则是内存缺页异常
if (faultnumb == 14)
&#47;&#47;打印缺页地址，这地址保存在CPU的CR2寄存器中</div>2021-06-25</li><br/><li><span>springXu</span> 👍（3） 💬（1）<div>对于提的两道问题，其实仔细看本课异常处理的代码（文中能找到）部分，就能知道答案了。变成了一个阅读理解题。东哥是为了让我们认真看文章，而且也增加了评论区的活跃度。哈</div>2021-06-25</li><br/><li><span>ifelse</span> 👍（2） 💬（2）<div>我都直接跳过代码部分</div>2022-02-16</li><br/><li><span>相逢是缘</span> 👍（2） 💬（1）<div>越来越难啊</div>2021-06-27</li><br/><li><span>Geek5167</span> 👍（1） 💬（1）<div>虚拟内存地址分配时，如果申请的空间大于4kb，虚拟内存是不是只能分配8kb,16kb这样的大小？</div>2022-01-10</li><br/><li><span>Dingo</span> 👍（1） 💬（2）<div>十几分钟的课程，一看就是大半天。</div>2021-11-10</li><br/><li><span>不修竹</span> 👍（1） 💬（1）<div>讲得相当清晰了，一学一整天真是痛快~</div>2021-08-16</li><br/><li><span>沐多</span> 👍（1） 💬（1）<div>代码里看到缺页异常是14号，地址保存在cr2寄存器中。</div>2021-06-25</li><br/><li><span>小灰象</span> 👍（0） 💬（1）<div>打卡打卡！
思考题：缺页异常是第14号，缺页地址保存在cr2寄存器中。</div>2024-08-22</li><br/><li><span>jshen28</span> 👍（0） 💬（1）<div>请问变量名是什么意思呢？是如何缩写的呢？</div>2022-08-06</li><br/><li><span>Han</span> 👍（0） 💬（1）<div>LMOS老师您好，看COSMOS代码，20 21课的new_kvmemcbox的返回值(kvmemcbox_t)是19课的kmsob_new返回值强转而来，这和函数返回的是(void*)freobjh_t，new_kvmemcbox函数再将这个void*强转为(kvmemcbox_t*)，这里可以平滑的转吗？两者的结构完全不相同</div>2022-07-26</li><br/><li><span>Ziggy_aa</span> 👍（0） 💬（1）<div>读源码的过程中发现，vma_del_usermsa() 这个函数在释放虚拟内存的时候会从页面盒子中去除挂载的页面数据结构。但是，这个函数没有确认页面盒子的 reference count，直接就从列表里进行了删除。如果有其他进程在共享这个盒子的话，这样直接删除会带来问题吗？</div>2022-07-20</li><br/><li><span>卖薪沽酒</span> 👍（0） 💬（1）<div>都是直接看源码， 看不懂了来看看文章的注释，啊哈哈， MMU那块代码没有展开说，有点可惜</div>2022-07-02</li><br/><li><span>nobel</span> 👍（0） 💬（1）<div>看不下去了</div>2022-04-24</li><br/>
</ul>