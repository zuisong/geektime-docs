...
<li><span>neohope</span> 👍（52） 💬（4）<div>一、再梳理一下内存结构
...
额，好像跑题了。。。</div>2021-06-20</li><br/><li><span>pedro</span> 👍（6） 💬（2）<div>分配的时候，如果是多个内存页面，优先向数组后面寻找，即多个连续的内存页，这就能保证分配的页面是连续的，释放的释放，组合多个页面，保证下次分配时候的连续性。

页大小是 4KB，对于小对象的分配，这样是非常浪费的，每开辟一个小对象都要申请一个物理页，这谁受得了啊，所以啊，linux 提出了 slab 分配算法。</div>2021-06-18</li><br/><li><span>吕默</span> 👍（3） 💬（1）<div>看代码完全看不懂，但是看图马上懂的不行——天生架构师。。。。。。。</div>2021-09-10</li><br/><li><span>Fan</span> 👍（3） 💬（4）<div>在内存页面分配过程中，是怎样尽可能保证内存页面连续的呢？
...
16K --&gt; 9K 4K 2K 1K</div>2021-06-29</li><br/><li><span>沈畅</span> 👍（1） 💬（1）<div>为什么分配pages，只设置链表首尾结构的属性，中间结构不用设置吗？这里不太理解
比如分4个页面，只设置msadsc[0] masadsc[3]的属性吗？</div>2021-09-07</li><br/><li><span>朱熙</span> 👍（1） 💬（1）<div>这节课讲的基本是buddy吧？下面对于小对象就是slab了</div>2021-06-18</li><br/><li><span>blentle</span> 👍（1） 💬（1）<div>回答一下思考题，是不是有个单独的线程或者每次释放内存后开个守护线程进行碎片整理和移动来实现.  还有这节课的代码真的好硬核，需要好长时间消化</div>2021-06-18</li><br/><li><span>Han</span> 👍（0） 💬（2）<div>有个疑问，比如第一次申请9k，第二次申请2k，第三次释放9k
第一次：按照文章中的算法，有7k放到前面的bafhlst_t中：16K --&gt; 9K 4K 2K 1K
第二次：2k的bafhlst_t被占用
第三次：释放至1k位置，2k的bafhlst_t被占用，算法结束，后面的4k就没有被回收了？</div>2022-07-21</li><br/><li><span>艾恩凝</span> 👍（0） 💬（1）<div>打卡，代码全部分析完了，分配的时候，我发现并没有把bafhlst_h 中的af_frelst 这个链表中空闲的 转到 af_alclst中去，释放的时候这个af_alclst 也没用到，那这不是白定义了 ，代码解释太粗糙，还是自己慢慢来看代码。用导图再重新梳理一遍，查漏</div>2022-04-14</li><br/><li><span>沈畅</span> 👍（0） 💬（1）<div>花了三天时间对着代码把物理内存管理内容看完了。收益很大。虽然还有一些字段不明白意思，后续继续研究。这里有个疑问，在释放内存pages时，需要进行page的合并，遍历bafhlst_t数组，这样会不会性能上比较低？</div>2021-09-08</li><br/><li><span>　　　　</span> 👍（0） 💬（0）<div>老师，这个回收算法是否有缺陷？假设我们的机器一共只有4个连续空闲的内存页，然后它们一起被挂载到第三个bafhlst_t结构上。有人调用了4次分配函数，每次分配出去一个。
...
不知道我理解的是否正确？不正确的话，我是哪里出错了呢？</div>2024-06-09</li><br/><li><span>Geek_79a775</span> 👍（0） 💬（1）<div>我想问一个代码细节。比如我要9K的内存空间，那肯定首先会分配给我16k的空间，那idx==dividx==4,就直接进入第一层代码（retrelbhl == retdivbhl)，就不会走后面把7K空间重新挂载这一步了?</div>2023-12-27</li><br/><li><span>lhgdy</span> 👍（0） 💬（0）<div>大佬，一个malloc 很大的一个数组，实际分类物理内存时候会 对应多个 不连续的物理内存页吗？</div>2022-12-13</li><br/><li><span>lhgdy</span> 👍（0） 💬（0）<div>存不存在合并性能很差的问题？ 每个 list 中的数据是无须的，要全部遍历一遍才能知道是否能合并吧？</div>2022-12-13</li><br/>
</ul>