...
<li><span>Amark</span> 👍（16） 💬（1）<div>请教一个问题，CPU调度是以进程为单位的吗，还是以线程?</div>2019-06-29</li><br/><li><span>莫名</span> 👍（13） 💬（1）<div>System V IPC具有很好的移植性，但缺点也比较明显，不能接口自成一套，难以使用现有的fd操作函数。建议对比讲一下比较流行的POSIX IPC。</div>2019-06-28</li><br/><li><span>nightmare</span> 👍（4） 💬（1）<div>信号量大于1的情况，可以让进程不操作共享变量，比如操作不同的变量，比如对一批数据做操作，然后做完之后给消费端读取</div>2019-06-30</li><br/><li><span>许童童</span> 👍（1） 💬（2）<div>信号量大于 1 的情况下，应该如何使用？
可以让多个进程同时访问一个共享内存。</div>2019-06-28</li><br/><li><span>Jason</span> 👍（0） 💬（2）<div>老师好，ftok提示我的机器里没有“&#47;root&#47;sharememory&#47;semaphorekey”这个文件，我随便新建一个文件可以吗？</div>2019-08-28</li><br/><li><span>trllllllll</span> 👍（0） 💬（1）<div>老师，share.h 里面 include 了两次 ipc.h。</div>2019-07-07</li><br/><li><span>Amark</span> 👍（0） 💬（1）<div>如果线程是掉用的到基本单位，那么进程的共享资源呢?</div>2019-06-29</li><br/><li><span>有风</span> 👍（2） 💬（0）<div>信号量大于1，可以用于限流。如线程或进程的个数，访问请求的个数等。</div>2022-02-16</li><br/><li><span>Tianz</span> 👍（2） 💬（0）<div>超哥，现在是不是推荐使用 POSIX 系列的 IPC 呢？</div>2019-06-28</li><br/><li><span>艾瑞克小霸王</span> 👍（1） 💬（0）<div>信号量和锁的区别就是 信号量可以控制资源数量（&gt;1）, 而锁是 互斥排他的？</div>2019-12-06</li><br/><li><span>Jason</span> 👍（1） 💬（0）<div>这篇看的很明白，嘿嘿。</div>2019-08-27</li><br/><li><span>Geek_93a721</span> 👍（0） 💬（0）<div>如果大于1时，应该使用三个信号量，一个表示任务这种资源，一个表示空间这种资源，第三个将其置为1用于互斥访问。</div>2020-09-29</li><br/><li><span>Helios</span> 👍（0） 💬（0）<div>信号量大于1的时候应该就不能控制写操作了。应该是控制读操作的进程数量。</div>2019-10-15</li><br/>
</ul>