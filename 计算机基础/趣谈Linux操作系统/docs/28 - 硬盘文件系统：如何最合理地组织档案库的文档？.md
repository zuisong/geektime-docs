...
<li><span>二三子也</span> 👍（34） 💬（3）<div>&gt;&gt;每个 extent 最大能表示 128MB 的数据。
疑问：ee_len大小为16bit，最大能表示2^16 - 1,如果块大小为4K，则一个extent能表示256MB
解惑：ee_len为一个16bit的无符号整数，但是其最高位在预分配特性中用来标识这个extent是否被初始化过了，所有ee_len中可用的位为15bit，最大可表示2^15个连续的块，如果块大小为4K，则一个extent最大能表示128MB</div>2019-06-10</li><br/><li><span>江山未</span> 👍（17） 💬（2）<div>看完已经不认识&quot;块&quot;这个字了</div>2019-08-05</li><br/><li><span>嘉木</span> 👍（10） 💬（1）<div>每次ls的时候，文件夹大小都显示4096，原来是一个块的大小，4k
vim 文件夹   能看到文件夹下的文件列表</div>2019-07-13</li><br/><li><span>W.jyao</span> 👍（6） 💬（2）<div>每篇文章有点收获就可以</div>2019-06-01</li><br/><li><span>geraltlaush</span> 👍（4） 💬（1）<div>老师，Exents的存储结构的树是红黑树吧</div>2019-06-01</li><br/><li><span>欢乐小熊</span> 👍（3） 💬（1）<div>前几天刚在虹桥关注到这个厕位导航系统, 结果今天就在课上看到了, 有趣有趣!!</div>2019-05-31</li><br/><li><span>Mr.差不多</span> 👍（2） 💬（2）<div>老师，您好。相同的文件夹在不同Linux机器上用ls -U得到的顺序也不一样。这是什么原因呢？文件系统如何确定文件夹中不同文件iNode的存储顺序的？</div>2019-05-31</li><br/><li><span>八台上</span> 👍（1） 💬（1）<div>目录文件和后边讲的目录项（dentry）的关系是什么呢？
我理解目录文件里边的内容就是 目录项列表（即目录文件下边的每一个普通文件和目录文件的目录项集合）</div>2020-06-10</li><br/><li><span>超超</span> 👍（1） 💬（1）<div>df -i 文件</div>2019-07-23</li><br/><li><span>靠人品去赢</span> 👍（1） 💬（1）<div>偷偷地问一句，4K对齐是不是就跟一个block默认大小是4K有关？</div>2019-06-17</li><br/><li><span>李俊君</span> 👍（0） 💬（2）<div>Exents 拼写错里吗？ 是不是该为 Extends </div>2020-02-23</li><br/><li><span>赵国辉</span> 👍（0） 💬（2）<div>老是，麻烦解惑一下，会出现一个文件的inode在一个块组中，而其数据块在另一个块组中吗？</div>2019-08-27</li><br/><li><span>HF</span> 👍（0） 💬（1）<div>老师，系统启动时是先挂载到内存等磁盘驱动程序完成后，再把根目录挂载到磁盘嘛？初始挂载得目录是什么时候生成的？</div>2019-06-21</li><br/><li><span>杨寅冬</span> 👍（0） 💬（1）<div>老师，之前的机械硬盘存储架构和原理，跟现在的ssd硬盘存储的原理，ssd硬盘没有磁头、盘片、磁道这些物理概念了吧，他们之前的技术演进，可以谈谈吗</div>2019-06-20</li><br/><li><span>尚墨</span> 👍（0） 💬（1）<div>老师能扩展一下内容，大概讲下 xfs 文件系统吗？</div>2019-06-06</li><br/>
</ul>