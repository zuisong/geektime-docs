...
<li><span>第九天魔王</span> 👍（40） 💬（1）<div>从网络协议课学到这里，满满的干货啊。特别是配图，太用心了，一看就是非常有心才能做出来的。谢谢刘老师！</div>2019-04-29</li><br/><li><span>青石</span> 👍（16） 💬（3）<div>看这篇内容时，查了几篇资料。

在汇编代码中，函数调用的参数传递是通过把参数依次放在靠近调用者的栈的顶部来实现的。调用者获取参数时，只要相对于当前帧指针的向上偏移即可取到参数。即取调用者函数参数时执行movl 8(%ebp), %edx。</div>2019-04-29</li><br/><li><span>hua168</span> 👍（8） 💬（1）<div>老师，我求您了，能不能把您讲的专栏中涉及的书名加个豆瓣之类的链接😂😂，有些都重名，作者不同，😭😭不带这样玩的😭😭😭</div>2019-04-29</li><br/><li><span>嘉木</span> 👍（5） 💬（1）<div>有个疑问，两个进程切换时，用户栈的上下文保存在哪？</div>2019-05-28</li><br/><li><span>骨汤鸡蛋面</span> 👍（4） 💬（1）<div>为什么用户态的栈不需要 task_struct 维护一个类似 *stack 的指针呢？</div>2019-05-22</li><br/><li><span>Liber</span> 👍（4） 💬（1）<div>0点自动发，应该是自动发布的吧</div>2019-04-29</li><br/><li><span>k先生</span> 👍（2） 💬（6）<div>老师，有个问题能否解答？
task_struct找内核栈过程理解了，但是反过来找，这句“而 thread_info 的位置就是内核栈的最高位置，减去 THREAD_SIZE，就到了 thread_info 的起始地址。”没懂，烦请解答一下

这里说的最高位置应该是栈顶，到了栈顶就是thread_info最低地址，那不就直接找到了吗</div>2019-07-25</li><br/><li><span>...</span> 👍（1） 💬（2）<div>老师有个疑问 A调用B B应该是被调用者吧 是不是应该放在图上面部分</div>2019-08-14</li><br/><li><span>青石</span> 👍（1） 💬（2）<div>在12节里面提到“Linux的任务管理由统一的结构task_struct进行管理”。那么多核CPU的任务切换时，是不是就是将current_task切换到另一个task_struct呢？
...
2. task_struck找内核寄存器是通过 内核栈的首地址（1中的stack指针） + (THREAD_SIZE - TOP_OF_KERNEL_STACK_PADDING)定位到pt_regs的最高位地址，再减一得到pt_regs的最低位地址（首地址）。</div>2019-04-29</li><br/><li><span>石维康</span> 👍（1） 💬（3）<div>文中说“接下来就是 B 的栈帧部分了，先保存的是 A 栈帧的栈底位置,也就是 EBP。因为在 B 函数里面获取 A 传进来的参数，就是通过这个指针获取的，”感觉主流编译器还是直接能通过当前 RBP 或者 RSP 来进行偏移定位到传进来的参数了吧？保存这个 A 栈底位置更多的是为了回复 A 的现场吧？</div>2019-04-29</li><br/><li><span>Bryant.C</span> 👍（0） 💬（1）<div>老师，最后总结时刻那张图内核栈栈底的文字是否应该是pt_regs呢？</div>2020-06-07</li><br/><li><span>追风筝的人</span> 👍（0） 💬（2）<div>被调用函数通过栈指针获取入参内容，那函数调用的参数副本怎么理解？</div>2019-07-12</li><br/><li><span>周平</span> 👍（0） 💬（1）<div>虽然还有些迷糊，但是感觉自己的理解在加深了</div>2019-05-09</li><br/><li><span>安排</span> 👍（0） 💬（1）<div>内核栈的最高地址的那8个字节没看懂，如果没压入ss,sp，那为什么还要访问呢？从源码注释看，加了8个字节保留，如果没有压入ss,sp,这时候访问也是没有意义的，既然没有意义，程序为什么还要访问他啊？一个在程序中不能控制吗？</div>2019-04-30</li><br/><li><span>why</span> 👍（71） 💬（0）<div>- 用户态&#47;内核态切换执行如何串起来
...
- 内核栈结构: 
    ...
    - 通过内核栈找到 task_struct
        ...
        - 64位 每个 CPU 当前运行进程的 task_struct 的指针存放到 Per CPU 变量 current_task 中; 可调用 this_cpu_read_stable 进行读取</div>2019-04-30</li><br/>
</ul>