...
从这段代码可以看出，&#47;dev&#47;xxx设备文件对应的特殊inode还可以表示FIFO、SOCK</div>2019-06-22</li><br/><li><span>小龙的城堡</span> 👍（3） 💬（1）<div>能解释下为什么打开一个块设备需要3个文件系统配合？为什么不是两个，也不是4个？感觉一堆代码分析，越来越晕了。</div>2019-06-14</li><br/><li><span>安排</span> 👍（2） 💬（2）<div>如果block_device是指向某个分区的，我们要先找到hd_struct，然后根据hd_struct找到对应整个设备的gendisk，这里是怎么根据hd_struct找到对应整个设备的gendisk的啊？</div>2019-06-14</li><br/><li><span>安排</span> 👍（1） 💬（1）<div>bdev这个文件系统的内容是不会持久化到磁盘的，既然这样，那么为什么要组织成文件系统的形式呢？
对内存中的文件系统不太了解，下面是我的理解，希望老师给予指正：
bdev里面由各种数据结构组成，例如链表、树之类的。这些结构和通常的文件系统用的那些数据结构相同，而且组织成了和通常的文件系统类似的形式，所以这里就把bdev代表的这一对数据结构组成的东西叫做文件系统。 如果把bdev组织成和通常的文件系统大不相同的形式，是不是也可以完成本文中的功能？那么这时候就不把bdev叫做文件系统了，而它就是一堆普通的数据结构。</div>2019-06-14</li><br/><li><span>一笔一画</span> 👍（0） 💬（1）<div>为什么查找ext4fs文件系统的内容需要devtmps来管理，感觉设计不太合理？</div>2019-06-16</li><br/><li><span>皮皮侠</span> 👍（1） 💬（0）<div>回答老师问题：还可以表示网络socket和管道</div>2020-11-07</li><br/><li><span>小庄.Jerry</span> 👍（1） 💬（0）<div>格式化的时候不会设置superblock吗</div>2019-11-14</li><br/><li><span>Geek_ae11ce</span> 👍（0） 💬（0）<div>最后这张图，尤其里边带了文字步骤，棒</div>2023-07-18</li><br/><li><span>Damoncui</span> 👍（0） 💬（0）<div>没有课代表可让我怎么活啊~</div>2022-12-13</li><br/><li><span>彭东林</span> 👍（0） 💬（0）<div>我记得hd_struct的第0项表示的并不是分区，而是整个磁盘，即指向的是disk-&gt;part0</div>2022-06-26</li><br/><li><span>李富</span> 👍（0） 💬（0）<div>字符设备为什么不用挂载的老师</div>2022-03-19</li><br/><li><span>皮皮侠</span> 👍（0） 💬（0）<div>再请教两个问题：1.block_device结构和gendisk里的operation是不是可以理解为操作系统和硬件两方的结合点？2.adddisk操作为何用自旋锁而不用其他锁呢？</div>2020-11-07</li><br/>
</ul>