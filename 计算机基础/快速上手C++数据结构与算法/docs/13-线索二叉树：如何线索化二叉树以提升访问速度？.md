...
<li><span>阿阳</span> 👍（0） 💬（0）<div>老师好，文章结尾有这样的提示：
...
请问老师，这种应该如何理解？</div>2024-02-03</li><br/><li><span>阿阳</span> 👍（0） 💬（0）<div>中序遍历线索化的二叉树，遍历的实现思路是，找到第一个结点、最后一个结点，在这两个操作的基础上，才能找到当前结点的后继结点和前驱结点，通过不断地向前或者向后，实现正向中序遍历和逆向中序遍历。能想出这种数据结构的科学家，真是让人叹为观止。
