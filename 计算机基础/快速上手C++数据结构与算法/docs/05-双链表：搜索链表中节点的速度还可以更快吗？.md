...
<li><span>雪无痕</span> 👍（0） 💬（1）<div>p-&gt;prior-&gt;next = p-&gt;net-&gt;prior = p，这里p-&gt;net应为p-&gt;next</div>2023-06-15</li><br/><li><span>Yj.yolo</span> 👍（0） 💬（1）<div>vector容器和list容器有以下区别：
...
（5）vector插入删除中间元素时数组会整体移动，而list插入删除直接修改链表指针；因此高效插入删除时还是list好些</div>2023-06-05</li><br/><li><span>Geek_7ba740</span> 👍（0） 💬（1）<div>可以理解为读多写少用vector，写多读少用list，不确定的话用vector吗</div>2023-04-08</li><br/><li><span>Fang</span> 👍（0） 💬（0）<div>p-&gt;prior-&gt;next = p-&gt;next-&gt;prior = p   </div>2024-07-29</li><br/><li><span>铁甲依然在</span> 👍（0） 💬（0）<div>我写了双链表的反转代码
&#47;&#47;双链表的链表反转
template&lt;typename T&gt;
