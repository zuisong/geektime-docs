...
<li><span>kimoti</span> 👍（2） 💬（1）<div>连续三次收到同样的ACK说明还能收到ACK,表明网络还不是那么拥堵,这样就不止于把拥塞窗口调整到0</div>2022-01-25</li><br/><li><span>西门吹牛</span> 👍（1） 💬（0）<div>有空能给搞一波关于 BBR 的拥堵算法加餐吗</div>2022-01-30</li><br/><li><span>那时刻</span> 👍（0） 💬（0）<div>收到三次ack进行重传，是确认网络可用的前提下重传，如果直接超时重传的话，网络的确糟糕，会增加额外的数据包，从而升级网络的压力。
