...
<li><span>那时刻</span> 👍（4） 💬（2）<div>我想最小的segment应该与内存叶相适应，一般是4k。如果是内存大叶，可能是16k吧</div>2022-02-24</li><br/><li><span>泛岁月的涟漪</span> 👍（0） 💬（1）<div>rocksdb主要是cpp吧</div>2022-04-21</li><br/><li><span>Paul Shan</span> 👍（0） 💬（1）<div>Segment是从内存到磁盘的读写单位，最小要设置成虚拟内存的页的大小。个人觉得设置成虚拟内存页的大小的整数倍都可以，太大会影响内存调度。
</div>2022-02-24</li><br/><li><span>小麦</span> 👍（3） 💬（0）<div>MySQL InnoDB 存储引擎也提供了 MRR 优化，将批量随机写入转换成顺序写入。
此外，MySQL InnoDB 在删除行数据时也采用的标记删除，磁盘空间并不会立即回收</div>2022-06-19</li><br/><li><span>蒋某人尚需顿悟</span> 👍（0） 💬（0）<div>es的存储应该也是lsm吧，还有个fsm是什么关系呢</div>2023-05-18</li><br/><li><span>药师</span> 👍（0） 💬（0）<div>每层遍历segment时也应该从最新加入的segment开始遍历吧？一层是有可能出现相同的key出现在不同的segment的情况的，那么越晚加入的到这层的segment的数据越新
