...
<li><span>y</span> 👍（0） 💬（1）<div>redis默认连接数是1w吧？</div>2023-10-22</li><br/><li><span>恰同学少年</span> 👍（0） 💬（1）<div>请教下如果Redis是sentinel集群，是否支持获取集群的信息？当master切换了能否感知到，应该怎么配置</div>2023-04-20</li><br/><li><span>Roy Liang</span> 👍（0） 💬（1）<div>老师在回复云上API监控里写：“周期性自动生成categraf的配置文件”，这是什么意思呢？</div>2023-02-13</li><br/><li><span>hello</span> 👍（0） 💬（1）<div>noeviction 老师这个默认的策略是怎么淘汰的呢</div>2023-02-09</li><br/><li><span>隆哥</span> 👍（0） 💬（1）<div>老师请教一个问题，比如我监控了十台服务器主机，我现在报警规则是内存使用率超过80%触发报警，那么我是在报警规则里面配置十条这样子的规则还是只需要配置一条，如果我只配置一条如何配置呢？</div>2023-02-09</li><br/><li><span>Mori</span> 👍（0） 💬（1）<div>感觉中间件监控方法差不多类似，比如针对云上REDIS，那么通过自己部署categraf采集还是对接云监控指标好一点呢，如果实例数量比较多，其实也能也需要管理多个categraf采集，维护稍微麻烦点
</div>2023-02-09</li><br/><li><span>peter</span> 👍（0） 💬（2）<div>请教老师两个问题：
Q1：categraf是部署在Redis所在的机器上吗？
Q2：最后的图中，”Max Memory Limit”是0，是表示满意限制吗？</div>2023-02-08</li><br/><li><span>隆哥</span> 👍（0） 💬（2）<div>老师，请教你一个问题，就是我夜莺配置了告警规则，但是勾选企业微信和填写回调地址企业微信的webhook，为什么企业微信无法收到报警信息，历史告警里面有记录呢，webhook地址我可以确认是没错的，是我哪里配置不对嘛。</div>2023-02-08</li><br/><li><span>k8s卡拉米</span> 👍（1） 💬（0）<div>老师，您好，我看redis仪表盘中没有关于redis 的状态的图表，需要向监控redis 主从状态，或者redis cluster的状态怎么做呢</div>2023-07-06</li><br/><li><span>志强</span> 👍（0） 💬（0）<div>老师 为什么不用redis-tool</div>2023-02-18</li><br/>
</ul>