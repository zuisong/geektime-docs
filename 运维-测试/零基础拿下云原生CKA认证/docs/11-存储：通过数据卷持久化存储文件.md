  ...
  template:
    ...
    spec:
      ...
      volumes:
        - name: pvc-shared-data
          persistentVolumeClaim:
            claimName: my-pvc-20m  # 部署好的PVC</div>2024-08-07</li><br/><li><span>奔跑的阿飞</span> 👍（0） 💬（1）<div>比葫芦画瓢。
第一题
# my-pv-10m.yaml 
apiVersion: v1
kind: hostPath
  ...
  template:
    ...
    spec:
      ...
      volumes:
        - name: pvc-shared-data
          persistentVolumeClaim:
            claimName: my-pvc-20m  # 部署好的PVC</div>2024-07-22</li><br/><li><span>摩羯</span> 👍（0） 💬（0）<div> ReadWriteOnce（RWO）：读写权限，但是只能被单个节点挂载
PVC  和  PV  都有这个属性。这里的节点指的是 k8s 的节点吗？同一个节点的多个不同pod可以读写同一个pv？这种情况下，还是容易出现 nfs 相同的问题吧？</div>2024-12-25</li><br/><li><span>摩羯</span> 👍（0） 💬（0）<div>ReadWriteOnce（RWO）：读写权限，但是只能被单个节点挂载</div>2024-12-25</li><br/>
</ul>