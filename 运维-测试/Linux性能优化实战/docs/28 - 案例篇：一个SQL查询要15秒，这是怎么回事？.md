...
<li><span>ninuxer</span> 👍（114） 💬（2）<div>打卡day29
echo 1&gt;&#47;proc&#47;sys&#47;vm&#47;drop_caches表示释放pagecache，也就是文件缓存，而mysql读书的数据就是文件缓存，dataservice不停的释放文件缓存，就导致MySQL都无法利用磁盘缓存，也就慢了～</div>2019-01-23</li><br/><li><span>大坏狐狸</span> 👍（59） 💬（1）<div>这个地方因为之前了解过，所以来打个卡，这个算法在高性能mysql里有介绍，叫最优前缀.
...
</div>2019-02-11</li><br/><li><span>王星旗</span> 👍（37） 💬（1）<div>老师，请教一个问题啊，既然只是iowait占比高，cpu占比不高，为什么执行其它命令会感觉卡呢？在等待io的时候，CPU不应该去执行其它进程吗？</div>2019-08-20</li><br/><li><span>rm -rf 😊ི</span> 👍（24） 💬（2）<div>有个疑问，老师之前不是说过数据库有自己一套缓存机制吗？为何删除cached也会影响到mysql的读写呢</div>2019-04-04</li><br/><li><span>hua168</span> 👍（7） 💬（3）<div>老师，问3个很重要的题外问题：
1. 大专学历，想直接自学考本科或研究生，自考学历中大型公司承认的吗？
2. 大公司对年龄有限制的吗？
3. 30多岁，运维（编程自学java一年，没项目经验），只有几年小公司运维经验，技术一般，发展方向是什么？很多IT公司好像都不要年龄大点的~~人生80，那不是40岁就没得工作了？</div>2019-01-23</li><br/><li><span>李逍遥</span> 👍（5） 💬（1）<div>mysql打开慢查询日志，排查SQL性能问题更方便</div>2019-01-23</li><br/><li><span>小老鼠</span> 👍（3） 💬（1）<div>即停止DataService ，又增加索3引是不是性能会更好？</div>2019-01-26</li><br/><li><span>安小依</span> 👍（3） 💬（1）<div>老师好，上次留言说 Java 无法 strace 打日志的问题解决了，原来 strace 必须跟上 -f 选项才可以，最终定位到当疯狂 System.out.println 的时候，原来是写到 pipe 管道里边去了：

java    2794   zk    1w  FIFO               0,12      0t0  3626492 pipe

到这个地方还是理解不了，这个 pipe 最终是存放在哪里了呢？为什么疯狂打日志，会把磁盘打满呢？谢谢</div>2019-01-24</li><br/><li><span>我来也</span> 👍（2） 💬（1）<div>[D28打卡]
居然漏打卡了，回来补上。
平常偶尔手动释放缓存还有可能，但把这个操作放在需要频繁执行的代码中就有点不科学了。</div>2019-01-31</li><br/><li><span>Geek_33409b</span> 👍（1） 💬（1）<div>打卡day29 ，之前在工作中遇到过 释放缓存的情况，但是也会发现echo 1在某些情况下没有办法有效地释放缓存，这会是什么原因引起的呢？</div>2019-02-06</li><br/><li><span>nemo</span> 👍（1） 💬（2）<div>Device:         rrqm&#47;s   wrqm&#47;s     r&#47;s     w&#47;s    rMB&#47;s    wMB&#47;s avgrq-sz avgqu-sz   await r_await w_await  svctm  %util
...
请问下老师,这个万转sas磁盘在写不到10MB的情况下,为什么磁盘使用率100%,等待时间也特别长?</div>2019-01-23</li><br/><li><span>tux</span> 👍（0） 💬（1）<div>回顾阶段，再运行案例，发现总会出两种现象：
1.卡在 进度条(出现最多)
...
是个例吗？</div>2019-10-21</li><br/><li><span>烨</span> 👍（0） 💬（1）<div>之前运行make init的时候，会报错连不上本地数据库，后来发现是因为mysql还没启动好，app就已经启动了，所以一直连不上。将app重启一次就好了</div>2019-08-21</li><br/><li><span>西瓜就咸菜</span> 👍（0） 💬（1）<div>我把insert条数增大到1000000，curl 的响应时间很短，0.0011873245239257812 sec，跟实验的结果不一样，我看系统的状态也没有io问题，通过pidstat也没有mysql的 iodelay</div>2019-07-08</li><br/><li><span>无名老卒</span> 👍（0） 💬（1）<div>实例插入了3W条数据，这样子才使得get data的时间变大了一些，后面增加了索引之后，就变正常了。
```
...
但是后面把dataservice停止之后，还是没有什么提高的效果，所以有可能硬盘使用了SSD，以及内存只有2G的原因导致的。另外说明一点的是，插入3W条数据之后，看到的products.MYD有1.5G了~</div>2019-04-25</li><br/>
</ul>