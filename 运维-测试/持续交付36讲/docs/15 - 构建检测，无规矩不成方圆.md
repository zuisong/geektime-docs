...
<li><span>Triton</span> 👍（4） 💬（1）<div>感谢分享，由于本身没有太多持续集成的经验，听上去比较抽象如果是创业公司应该做到那些？</div>2018-08-07</li><br/><li><span>玉军</span> 👍（2） 💬（1）<div>这种构建检查大部分都是环境的检查？有没有更深层次的检查？比如构建检查提高每日构建的成功率？同时ant有没有这方面的插件？</div>2018-12-22</li><br/><li><span>吃饱了晒太阳</span> 👍（1） 💬（1）<div>遇到个问题想请教下，在服务器上搭建了一个ubuntu docker镜像，里面软件测试都已装好，CI触发脚本测试时候，一直提示redis拒绝连接，调用的脚本也是在容器里的，在容器里执行就可以，但是每次提交触发测试就会提示拒绝连接，是因为什么呢，redis也允许外部连接，是docker网络问题？还请指点</div>2018-08-08</li><br/><li><span>Robert小七</span> 👍（0） 💬（0）<div>请问老师，如何检查项目的继承树？</div>2020-01-01</li><br/><li><span>飞毛兔</span> 👍（0） 💬（0）<div>公司搭建了一个共享的maven库，有多个供应商使用。但个别供应商要求能自行上传依赖并且这些私有依赖不能暴露给其他供应商，请问有什么解决方案吗？</div>2019-12-18</li><br/><li><span>Mingyan</span> 👍（0） 💬（0）<div>都是java的 有php的如何做持续集成吗？</div>2019-08-19</li><br/><li><span>Robert小七</span> 👍（0） 💬（0）<div>这个插件可以在maven的配置文件中配置后，其他任何应用构建时进行检查吗？</div>2019-07-03</li><br/><li><span>YoungerChina</span> 👍（0） 💬（0）<div>更像依赖版本管理检测</div>2019-02-12</li><br/>
</ul>