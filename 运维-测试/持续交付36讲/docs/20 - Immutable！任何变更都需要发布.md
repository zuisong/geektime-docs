...
<li><span>Milo</span> 👍（4） 💬（1）<div>想了解下大家怎么处理数据库版本回滚的</div>2018-08-24</li><br/><li><span>春风百里</span> 👍（1） 💬（1）<div>请问，按照每一次变更都是一次发布，每一次发布都是一次独立镜像启动，那在配置中心做的配置变更，也算是一次发布，但是并没有产生新的独立镜像，这个怎么理解？</div>2020-03-18</li><br/><li><span>禾子先生</span> 👍（1） 💬（0）<div>不可变思想在mybatis的使用中有应用，通过MBG生成的代码不可变，有特殊需求，继承或新增。</div>2018-08-19</li><br/>
</ul>