...
<li><span>道长</span> 👍（4） 💬（1）<div>今天这篇，感觉有点难，可能是没接触过k8s吧
还有负载不均衡我肯定是先去看负载策略，再往下排查；只有当策略都没没问题我才会考虑内部逻辑，个人看法</div>2021-04-28</li><br/><li><span>安静。。。</span> 👍（2） 💬（1）<div>1. 当负载出现不均衡时，主要的分析方向是什么？
看下转发机制是什么，为什么只有部分的服务能接收到请求

2. 什么时候才需要去看组件内部的实现逻辑？
先通过全局监控，确定占用资源大的机器，再查看这个机器上对应的哪个服务抢占资源大
确定到对应的服务之后，可以看这个服务是干什么的，为啥抢占资源大</div>2021-07-12</li><br/><li><span>小乐天天</span> 👍（1） 💬（2）<div>老师，您的整体监控用的是什么工具？</div>2022-02-23</li><br/><li><span>不将就</span> 👍（1） 💬（1）<div>还是只有一个 ES data 的 CPU 使用率高，所以我想查一下 ES 中的数据分布。因为负载均衡的问题解决了，并且知道有三个 ES data 节点。现在我们就要知道是不是每个节点都被访问到了。
----高老师，这里涉及的是什么命令？</div>2021-10-24</li><br/><li><span>Geek_f9e0e5</span> 👍（1） 💬（1）<div>传统架构有什么好方法拆分响应时间吗，非微服务的</div>2021-09-07</li><br/><li><span>yz</span> 👍（1） 💬（1）<div>1，当负载均衡时主要分析方向是 负载均衡服务器？
2，全局分析发现问题再定向分析发现有个</div>2021-06-01</li><br/><li><span>继贤</span> 👍（1） 💬（1）<div>老师，你讲的性能调优要先将资源利用起来，这个资源使用到达多少就算是合理了，比如CPU，有些系统是80%，有些可以到100，该怎么衡量？</div>2021-04-23</li><br/><li><span>亚林</span> 👍（0） 💬（1）<div>这个grafana dashboard的id是多少？</div>2024-04-22</li><br/><li><span>Geek_678a7f</span> 👍（0） 💬（1）<div>这篇确实有点难了，首先不懂K8S的负载均衡怎么搞的，其次es也不太懂。。。</div>2023-05-21</li><br/><li><span>Only look at me</span> 👍（0） 💬（1）<div>skywalking响应时间的分析环节，为什么判断消耗都在ES上呢？</div>2022-12-12</li><br/><li><span>Geek_f9e0e5</span> 👍（0） 💬（1）<div>mq双master部署，会不会有多副本的情况呢</div>2021-09-07</li><br/><li><span>Geek_f93234</span> 👍（0） 💬（2）<div>老师，文中&quot;从上面的信息可以看到，我们的这个集群中有三个 ES data 服务，对应着三条转发规则，其中，第一条规则的匹配比例是：0.33333333349；第二条比例：0.50000000000；第三条是 1。&quot;
其中,第二条比例：0.50000000000，是怎么得出来的？</div>2021-06-08</li><br/><li><span>Technological life</span> 👍（0） 💬（1）<div>这里提到的POD是什么呢？</div>2021-05-20</li><br/>
</ul>