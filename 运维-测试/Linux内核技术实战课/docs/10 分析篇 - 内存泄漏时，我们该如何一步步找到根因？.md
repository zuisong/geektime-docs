...
<li><span>石小</span> 👍（12） 💬（1）<div>“对于有经验的开发者而言，从这个 4K 的保护页就可以猜测出应该跟线程栈有关了。” 请问老师怎么理解这局话？</div>2021-04-08</li><br/><li><span>飘云</span> 👍（5） 💬（2）<div>写应用层代码的时候会用到clone么，不都用pthread么？ 用pthread不会出现例子中的问题吧。 
另外，内存泄露很多时候是malloc了内存但是没有free，有什么好的方法快速定位是哪儿分配的内存忘了free呢？</div>2020-10-30</li><br/><li><span>蓬蒿</span> 👍（5） 💬（1）<div>老师好，这个案例讲的是虚拟内存泄露，我想问一下:虚拟内存分配后在使用前不会真正的分配物理内存，那这个案例是不是仅仅是进程的虚拟内存存在问题？整个Linux系统的内存应该没有异常对不对？</div>2020-10-29</li><br/><li><span>Geek_circle</span> 👍（3） 💬（1）<div>老师好，当系统内存不足,发现kswapd0进程持续运行，使用率超过90%多,load过百,但是系统日志中并没有记录到oom killer 。想知道系统内存不足时，启动kswapd和oom killer条件和优先级是什么样的呢？</div>2020-09-13</li><br/><li><span>dpzain</span> 👍（2） 💬（1）<div>现在生产环境 是jvm 程序 跑在docker容器， jvm 堆内内存 监控一切正常，但是每隔半个月 mem从50%到90% ，按照上面的方法发现确实有对外内存在缓慢的泄漏   非常慢（没5s - 20s 增加几kb甚至  几十个byte ）找不出任何规律 怎么破；</div>2020-11-24</li><br/><li><span>jssfy</span> 👍（2） 💬（0）<div>问题分析过程很赞，开头的总结性图表也很赞！</div>2020-09-13</li><br/><li><span>蔡奇</span> 👍（0） 💬（0）<div>老师好，我现在碰到一个问题，首先，我使用一个接口创建的全局指针，中间也多次使用了该全局变量，最后，退出前也释放了。但是，发现使用asan检查出了该接口的内存溢出问题，这个要怎么分析哈。</div>2024-03-31</li><br/><li><span>mong</span> 👍（0） 💬（1）<div>老师, 看了你的这些文章, 对线上内存的定位也有一些思路, 我正面临的一个Java应用程序内存有问题的, VIRT, RSS的内存很高, 用&#47;proc&#47;pid&#47;smaps, 发现有很多64M的内存申请,占了将近3个G 如下:
...
但到这一步我无法继续的下去, 我用gdp dump了这段的地址的bin下来, 打开文件都Null, 所以我的java内存的堆的内存通过监控看也比较稳定, 所以目前没有什么头绪, 所以相让你指点一点, 接下来应该从哪里继续的排查;</div>2022-09-16</li><br/><li><span>Bachue Zhou</span> 👍（0） 💬（0）<div>不太懂，smaps 的内容这么多，如何找到出问题的部分？是多次 cat smaps 然后做 diff 吗？</div>2022-06-05</li><br/><li><span>Felix</span> 👍（0） 💬（0）<div>真是读书万卷，其意自现</div>2022-05-17</li><br/><li><span>我能走多远</span> 👍（0） 💬（0）<div>对底层的基础理论不牢靠，原来定位问题有这么多的方法。学习了</div>2020-11-07</li><br/><li><span>莫名</span> 👍（0） 💬（0）<div>赞，之前有过比较类似的排查思路。</div>2020-10-22</li><br/><li><span>吃苹果的考拉</span> 👍（0） 💬（0）<div>曲径通幽，豁然开朗。基础要扎实，但工具也要熟啊</div>2020-09-10</li><br/>
</ul>