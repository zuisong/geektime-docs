...
<li><span>莫名</span> 👍（15） 💬（2）<div>赞，非常适合入门 ftrace 的科普文。操控 tracefs 各种文件相对繁琐（但便于理解 ftrace），推荐使用 ftrace 的一个前端工具 trace-cmd，类似 perf 用法，简单易用。</div>2021-02-05</li><br/><li><span>徐少文</span> 👍（5） 💬（2）<div>inline函数在编译的时候实际上不是函数，而是直接在原函数处展开，主要是为了提高效率。所以实际上不能使用。static函数可以使用</div>2022-03-07</li><br/><li><span>lyonger</span> 👍（2） 💬（1）<div>tracefs 这个虚拟文件系统，只能在宿主上操作？ 如果需要在容器里操作的话，有啥方法么？</div>2021-09-18</li><br/><li><span>JianXu</span> 👍（4） 💬（0）<div>今天才看完所有的文章，好多东西要学。周五晚上看到Sherlock&#47;SRE 群里Ralph 分享了一篇文章讲在Clickhouse 基础上支持类Spark 能力的文章，Huai 回复说这才是我们的工程师应该做的事情，顿时觉得自己那些小trick 不值一提。我们还是应该在这些技术上做深耕，Cloud 这条线也是一样的。</div>2022-09-11</li><br/>
</ul>