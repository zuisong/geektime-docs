...
<li><span>刘孟</span> 👍（118） 💬（5）<div>需要创建的 Pod 数目 = 最终需要的 Pod 数目 - 实际在 Running 状态 Pod 数目 - 已经成功退出的 Pod 数目 = 2 - 0 - 0= 2。而parallelism数量为4，2小于4，所以应该会创建2个。</div>2018-10-12</li><br/><li><span>DJH</span> 👍（44） 💬（5）<div>说到pod的重新启动，我想再请教一个问题：假设我把deployment的restart policy设置成always，假设某个pod中的容器运行失败，那么是重新创建了一个新的pod，还是仅仅重启了pod里的容器？pod的名称和ip地址会变化吗？</div>2018-10-12</li><br/><li><span>柯察金</span> 👍（28） 💬（5）<div>问个不相关的问题：configmap 更新，怎么做到不重启 pod 生效</div>2019-01-03</li><br/><li><span>柯察金</span> 👍（19） 💬（1）<div>job 执行结束，处于 completed 状态之后，还会占用系统资源吗，可以让它执行结束后自动退出吗</div>2019-01-03</li><br/><li><span>jssfy</span> 👍（17） 💬（1）<div>请问job成功结束后一直处于completed状态吗？需要手动清吗？</div>2018-11-25</li><br/><li><span>abc</span> 👍（16） 💬（1）<div>请问老师：miss的数目100是默认的吗？哪个参数可以修改呢</div>2018-10-12</li><br/><li><span>silver</span> 👍（10） 💬（3）<div>Job的第三种方法中是不是需要有其他process最后去Kill这个Job，否则Job会在Pod退出后不断创建新的Pod？</div>2018-10-14</li><br/><li><span>参悟</span> 👍（9） 💬（1）<div>最近实践遇到的问题，盼请赐教，多套分支的开发，测试环境，是按一套k8s集群按命名空间分区，还是按多套集群，实践方案哪种更好，有何优缺点？如果按一套多空间，会有nodeport冲突的问题，比如数据库需要暴露稳定的端口，方便运维。</div>2018-10-12</li><br/><li><span>虎虎❤️</span> 👍（6） 💬（1）<div>并发度为4，意味着可以同时启动不超过4个job。

completion2 - running0 - completed0 = 2

所以会启动2个job</div>2018-10-12</li><br/><li><span>DJH</span> 👍（4） 💬（1）<div>另外我想请教一下，CronJob是定期产生新的Job，还是定期重启同一个Job任务？</div>2018-10-12</li><br/><li><span>hochuenw</span> 👍（3） 💬（1）<div>老师请问kubeflow在哪一部分用到了第一种job的使用方法？他们不是自己写了tf-operator吗</div>2018-11-04</li><br/><li><span>Nokiak8</span> 👍（3） 💬（2）<div>老师，请教一个问题，假如Job中定义的pod运行失败，比如有异常。pod就会接着新生成，这样带来的就是会有大量的pod 产生，如何解决这种问题？</div>2018-10-18</li><br/><li><span>DJH</span> 👍（3） 💬（1）<div>放miss数量达到100时，cronjob是永远不再创建新的job（相当于整个cronjob失效），亦或只是不再运行miss（错过）的那些job？</div>2018-10-12</li><br/><li><span>georgesuper GoodTOGreater</span> 👍（2） 💬（1）<div>是不是Spark job,hadoop job,k8s job 底层原理都相似？</div>2018-10-20</li><br/><li><span>Caesar</span> 👍（2） 💬（1）<div>(1) 若所有容器重启成功，pod应该是running，或者succeeded
(2) 若有容器没有成功，看具体原因,pending,failed...</div>2018-10-12</li><br/>
</ul>