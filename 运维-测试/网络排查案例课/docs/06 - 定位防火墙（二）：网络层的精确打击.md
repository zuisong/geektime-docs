...
<li><span>includestdio.h</span> 👍（16） 💬（1）<div>老师我这里有个小问题，既然是被防火墙RST了，为啥客户端抓包看到的SRC IP不是防火墙IP，而仍然是服务端IP，能理解为什么要这样做，但是不太了解这是怎么实现的，有啥说法吗</div>2022-01-24</li><br/><li><span>webmin</span> 👍（7） 💬（3）<div>MAC地址中的组织唯一标识符 （OUI）由，IEEE（电气和电子工程师协会）分配给厂商，那么通过MAC地址可以辨别出厂商，防火墙的主要厂商也不多，从这块信息大约能判断出回包的是不是防火墙，因为是通过二层信息判断，所以这个方法是有局限性的。</div>2022-01-24</li><br/><li><span>ThinkerWalker</span> 👍（6） 💬（3）<div>思考题：traceroute查看数据包经过的路径？</div>2022-01-30</li><br/><li><span>潘政宇</span> 👍（5） 💬（3）<div>老师，有2个问题请教一下：
1. iptables作用点是在tcp IP协议栈之前吗？
2. iptables drop后的包，tcpdump还能抓到吗</div>2022-02-03</li><br/><li><span>WalkRun</span> 👍（4） 💬（2）<div>老师  弱弱的问一句 如果想让vm1访问百度通过vm2那么把vm1的网关设置成vm2的IP不就可以了吗？</div>2022-05-17</li><br/><li><span>那时刻</span> 👍（3） 💬（1）<div>我在Mac里通过docker 模拟两台虚机动手实践rst消息的例子，但是遇到问题
...
具体的信息，请参见 https:&#47;&#47;gist.github.com&#47;richzw&#47;2948a9bc2de8dca1811b69c42796a576。麻烦老师抽空帮忙看看</div>2022-03-09</li><br/><li><span>JianXu</span> 👍（2） 💬（1）<div>我司现在碰到的问题是，当源到目的不通或者间歇性不通的情况下，源端应用的负责人就会找到基础架构问为什么。而我们现在并没有一个很好的办法可以得到答案。然后就变成应用，基础架构和数据中心物理网络把自己的三板斧当成免责，公司的应急反应中心最后就把所有人拉在一起。到了分布式后这个问题越来越严重 查起来也越来越繁琐了。现在有一种趋势就是应用，分布式防火墙，软件负载均衡都说要把日志送到监控部门手上，然后监控部门和网站可用性的副总说有谁对怎么用这些数据负责任吗？我给领导汇报说我觉得流量运维部门是定位这类问题的希望。</div>2022-03-19</li><br/><li><span>江山如画</span> 👍（2） 💬（2）<div>拓展思考，丢弃有害 RST 的办法:
...
</div>2022-01-24</li><br/><li><span>N0mansky</span> 👍（1） 💬（1）<div>这种绕过还是只能针对单向rst吧，如果双向rst就没法绕过了</div>2022-11-15</li><br/><li><span>Geek_nai6tk</span> 👍（1） 💬（2）<div>ip addr add 100.64.0.2 peer 100.64.0.1 dev tun0
想知道这条命令里面的 ip地址是怎么确定的</div>2022-04-12</li><br/><li><span>xuty</span> 👍（0） 💬（1）<div>目前遇到客户端sysbench压测mysql，压测一段时间后会出现mysql回包一直重传导致sysbench压测断开的现象，想知道防火墙上会有什么策略会导致这种现象？</div>2023-09-05</li><br/><li><span>Ironshuai</span> 👍（0） 💬（1）<div>前一段时间，线上遇到了防火墙向客户端和服务端发双向RST的案例。如果早点看到老师的课，兴许能少走一些弯路</div>2023-08-29</li><br/><li><span>斯蒂芬.赵</span> 👍（0） 💬（1）<div>第一个案列想不通啊，如果是防火墙的问题的话三次握手的时候就应该RST，三次握手成功了说明没触发防火墙，怎么会在三次握手成功后响应之前触发防火墙呢</div>2022-11-25</li><br/><li><span>piboye</span> 👍（0） 💬（2）<div>老师, 服务收到的syn 包的ttl 是 112,  客户端收到的 synack包的ttl 是 110,  这是说两个方向的路径不一样, 经过了不同的设备?</div>2022-08-02</li><br/><li><span>藖</span> 👍（0） 💬（1）<div>如果服务器上面是centOS7的系统，使用的是firewall防火墙，能否使用iptables的规则进行设置？</div>2022-04-07</li><br/>
</ul>