...
<li><span>石云升</span> 👍（3） 💬（0）<div>基于梯度的剪枝：
  计算每个参数对损失函数的梯度。
...
基于敏感度的剪枝：
  分析每个参数对模型性能的敏感度。
  敏感度低的参数可以被剪掉。</div>2024-09-08</li><br/><li><span>潇洒哥66</span> 👍（0） 💬（0）<div>除了参数对于模型输出重要性的考虑，有其它标准作为剪枝的依据吗？类似于参数对于模型稳定性，甚至是泛化能力的影响。</div>2024-10-16</li><br/>
</ul>