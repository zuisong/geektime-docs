...
<li><span>Geek_cbcfc8</span> 👍（1） 💬（1）<div>老师 ，实战六 大纲里不是训练自己的模型吗，这是临时新增了一个其它的？  本章提供的三种风格化模型在实战中表现都不是很稳定，而且效果一般，
自己研究发现目前SD风格比较好的有，reference_only、ipAdapter以及Blip2Diffusion</div>2023-09-11</li><br/><li><span>bookchan</span> 👍（0） 💬（1）<div>请教个问题，什么类型的模型才可以merge？比如sd模型和lora或者controlnet可以进行merge吗</div>2023-09-14</li><br/><li><span>cmsgoogle</span> 👍（0） 💬（0）<div>您好，指令集修图代码给的是sd1.5，请问有没有支持sdxl的指令集修图模型或者代码么？</div>2023-10-25</li><br/>
</ul>