...
<li><span>🐱您的好友William🐱</span> 👍（9） 💬（2）<div>bandit是 reinforcement learning 的只有一个state和多个action的情况，我觉得thompson sampling的问题好像是它应该是有一个assumption，它假设了每个action背后会不会有反馈是一个Bernoulli Distribution，但是人的兴趣会不断地变化，所以assumption可能不hold，所以需要不断地online learning估计才行。在我来看，UCB最能直接反应问题的本质，我比较喜欢UCB。 </div>2018-10-03</li><br/><li><span>那年岁月</span> 👍（1） 💬（1）<div>Thompson采样，既然是冷启动用户，都初始化为1，下次这个用户可能就不是冷启动了，这个矩阵就没用了啊。只存储m个物品的矩阵就行吧</div>2018-04-10</li><br/><li><span>王千发</span> 👍（0） 💬（1）<div>请问对于新item的冷启动，应该怎么做呢？大致思路是怎样的？</div>2019-02-26</li><br/><li><span>大猫星球</span> 👍（0） 💬（1）<div>目前我们冷启动采用标签与topic探索推荐</div>2018-04-13</li><br/><li><span>林彦</span> 👍（28） 💬（1）<div>1. Epsilon贪婪算法的不足:
...
Epsilon贪婪算法的不足的(3)和(4)适用于所有的Bandit算法。</div>2018-04-09</li><br/><li><span>曾阿牛</span> 👍（5） 💬（0）<div>讲得很直白，赞。回到正题：
1）Bandit算法是试验型算法，基于大数定理，收敛应该不快
2）汤普森算法保留的参数量有点大，适用场景有点受限</div>2018-04-10</li><br/><li><span>yya</span> 👍（1） 💬（0）<div>这个属于强化学习的方法吗</div>2018-04-09</li><br/><li><span>Geek_015fd0</span> 👍（0） 💬（0）<div>exp3算法是不是比它们更好一些？</div>2021-05-18</li><br/><li><span>指尖旋律</span> 👍（0） 💬（0）<div>Bandit算法是反映了用户的一个长期兴趣吗，一般会考虑长期使用bandit算法建立的标签体系吗？如果使用了如何考虑时间因素呢</div>2019-09-11</li><br/><li><span>mi</span> 👍（0） 💬（0）<div>2mn个参数会不会太稀疏了啊？会不会导致a和b参考意义不大呢？</div>2019-04-01</li><br/><li><span>叶晓锋</span> 👍（0） 💬（0）<div>汤普森采样</div>2018-05-02</li><br/><li><span>EAsY</span> 👍（0） 💬（0）<div>冷启动用类似人群划分推荐（比如同机型 年纪 性别 年龄等相似人群喜好 ）效果好 还是bandit测试推荐好 目前我们用的是前种</div>2018-04-11</li><br/><li><span>jt120</span> 👍（0） 💬（0）<div>是不是可以这样理解，前面的协同过滤和lr都是找相似，而bd是找选择，选大概率的选项</div>2018-04-10</li><br/><li><span>hqzhao</span> 👍（0） 💬（0）<div>bandit算法需要不断的试错，虽然原理和想法很好，但难以实际应用。顺便请教一个问题，bandit算法属于强化学习的范畴么？强化学习也是跟用户产生交互，并根据用户的反馈来更新参数</div>2018-04-10</li><br/>
</ul>