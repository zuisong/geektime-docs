...
<li><span>TKbook</span> 👍（18） 💬（1）<div>源代码中：
...
应该是：
test_x, test_y = x[: 2000], y[: 2000]
train_x, train_y = x[2000:], y[2000:]</div>2019-03-05</li><br/><li><span>third</span> 👍（7） 💬（1）<div>结果仍然为AdaBoost算法最优。
个人发现，前两个分类器出结果很快
分析最优：
...
print(u&#39;AdaBoost 分类器准确率为 %.4lf&#39; % np.mean(cross_val_score(ada, train_features, train_labels, cv=10)))</div>2019-03-04</li><br/><li><span>Geek_hve78z</span> 👍（6） 💬（1）<div>由于乘客测试集缺失真实值，采用 K 折交叉验证准确率
--------------------
运行结果：
决策树弱分类器准确率为 0.7867
决策树分类器准确率为 0.7813
...
print(u&#39;AdaBoost 分类器准确率为 %.4lf&#39; % np.mean(cross_val_score(ada, train_features, train_labels, cv=10)))</div>2019-03-04</li><br/><li><span>梁林松</span> 👍（3） 💬（1）<div>跑第二块代码是需要引入两个模块
from sklearn.tree import DecisionTreeRegressor
from sklearn.neighbors import KNeighborsRegressor
</div>2019-03-04</li><br/><li><span>Liam</span> 👍（2） 💬（1）<div>ax = fig.add_subplot(111)ax.plot([1,n_estimators],[dt_stump_err]*2, &#39;k-&#39;, label=u&#39;决策树弱分类器 错误率&#39;)ax.plot([1,n_estimators],[dt_err]*2,&#39;k--&#39;, label=u&#39;决策树模型 错误率&#39;)ada_err = np.zeros((n_estimators,)).  疑问：这里*2是什么意思，能解析下代码吗？</div>2021-03-26</li><br/><li><span>滢</span> 👍（1） 💬（1）<div>得到结果：
...
</div>2019-04-21</li><br/><li><span>小晨</span> 👍（0） 💬（1）<div>弱分类器准确率为 0.7868
决策树分类器准确率为 0.7823
...
</div>2021-03-10</li><br/><li><span>萌辰</span> 👍（0） 💬（1）<div>在AdaBoost、决策树回归、KNN房价预测对比中发现，随机种子对决策树的预测结果有影响。
分别测试了三种不同的随机种子：
...
KNN算法无随机种子影响。</div>2020-07-05</li><br/><li><span>§mc²ompleXWr</span> 👍（0） 💬（1）<div>使用自带的数据集就不用做数据规范化么？</div>2020-06-09</li><br/><li><span>鲨鱼鲸鱼鳄鱼</span> 👍（0） 💬（1）<div>老师，请问AdaBoost模型在预测前需不需要对数据进行标准化或者归一化，做有什么好处，不做有什么好处呢</div>2020-05-25</li><br/><li><span>张贺</span> 👍（0） 💬（1）<div>老师讲的很清晰</div>2020-03-27</li><br/><li><span>滨滨</span> 👍（0） 💬（1）<div>分类和回归都是做预测，分类是离散值，回归是连续值</div>2019-04-21</li><br/><li><span>JingZ</span> 👍（0） 💬（1）<div># AdaBoost
...
Adaboost cross_val_score 准确率为 0.8104</div>2019-03-05</li><br/><li><span>FORWARD―MOUNT</span> 👍（0） 💬（1）<div>老师，房价预测这个算法，50个弱分类器是怎么来的？</div>2019-03-05</li><br/><li><span>佳佳的爸</span> 👍（0） 💬（1）<div>你好老师，完整的源代码在哪里可以下载到?  我说的是每节课里边的源代码。</div>2019-03-04</li><br/>
</ul>