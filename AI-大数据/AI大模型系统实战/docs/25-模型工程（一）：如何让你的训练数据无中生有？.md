...
<li><span>顾琪瑶</span> 👍（5） 💬（1）<div>上限:
* 就目前使用GPT来生成样本数据集的话, 那么上限应该是GPT模型本身的知识范围, 在多次对特定领域进行样本数据生成时, 次数越多, 重复的样本也会相应增多</div>2023-10-16</li><br/><li><span>l_j_dota_1111</span> 👍（1） 💬（1）<div>如果相对垂直领域进行模型微调，但是现在的垂直领域的数据都是文本（就是一个一个word文档），如何生成可以使用的样本数据呢（任务就是正常的对话）</div>2024-01-09</li><br/>
</ul>