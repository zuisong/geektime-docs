...
<li><span>海阔天空</span> 👍（10） 💬（5）<div>自动倾斜处理后，如何保证同样key的数据在同一个reduce里执行</div>2021-10-23</li><br/><li><span>小新</span> 👍（0） 💬（3）<div>感觉这讲深度不够</div>2021-12-02</li><br/><li><span>Geek_1e4b29</span> 👍（0） 💬（1）<div>同问，相同的key如何解决,加问一个，你们的生产上开了这个参数吗，作业有多大，有没什么风险😬</div>2021-10-26</li><br/><li><span>bian</span> 👍（0） 💬（0）<div>老师，Spark作业输入数据20亿，每条记录都很小，过滤后1.8亿，之后repartition 5000,写入Hdfs，发现总有个别几个任务shuffle read读不完一直卡住，也不报错，没有倾斜问题，也没有特别大的记录，不知道为什么</div>2023-05-31</li><br/><li><span>唐方刚</span> 👍（0） 💬（0）<div>与广播阈值对比的文件系统和dag中间结果，这两个是同一个级别的概念吗，之前我有碰到过mapjoin的时候，几十M的orc表文件，4G的客户端内存都oom了</div>2022-08-13</li><br/>
</ul>