...
<li><span>pythonbug</span> 👍（2） 💬（1）<div>老师好，房价预测那个例子，我用GBDT试了一下，结果发现比咱们一开始入门那个例子计算出来的rmse还要大。。没有明显变小，树的深度和最大棵数以及阈值都是和您一样的。
其中，预测对比我用的是：
...
全部代码比较长，贴不过来</div>2021-11-22</li><br/><li><span>小新</span> 👍（0） 💬（1）<div>老师后面可以出一个详细的ML的课程吗？也是这样的讲解方式。</div>2021-12-05</li><br/><li><span>六月的余晖</span> 👍（0） 💬（1）<div>老师后续有计划出Flink的课程吗</div>2021-11-12</li><br/><li><span>liangyu</span> 👍（0） 💬（0）<div>.setInputCols(numericFields ++ indexFields) 这里一个typo ， numericfields 是没有转换成int的 ，前文用的是numeric features</div>2022-12-15</li><br/>
</ul>