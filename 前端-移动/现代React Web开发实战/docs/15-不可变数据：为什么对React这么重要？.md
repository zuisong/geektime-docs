...
<li><span>相望于江湖</span> 👍（1） 💬（1）<div>我有个疑问，不可变其实就是把对象、数组深度克隆一遍。
深度克隆这个操作应该比深度比较更耗时吧，而且对象克隆来克隆去，难道不会造成内存暴涨，甚至泄露吗？</div>2023-09-08</li><br/><li><span>01</span> 👍（0） 💬（3）<div>可能需要deepFreeze。 本身存在冻结不应该冻结对象的风险
preact给自己乃至react 提供了 signals。 用到了proxy</div>2022-10-11</li><br/><li><span>thomas</span> 👍（0） 💬（0）<div>保存一份数据的多个版本变得可行。
--------------------------------&gt;
