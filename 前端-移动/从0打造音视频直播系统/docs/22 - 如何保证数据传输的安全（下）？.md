...
<li><span>杨柳</span> 👍（4） 💬（1）<div>这里有一个疑问，DTLS本身对数据层有加密，SRTP也是对数据的加密，是否重复了？经查资料是这样的，SRTP数据，经DTLS发送会跳过加密层，不进行二次加密，不知理解是否正确。</div>2020-03-27</li><br/><li><span>重生</span> 👍（4） 💬（1）<div>老师，有一点疑惑，SDP中交互用户名和密码是明文，这样黑客不是可以获取到吗？或者模仿对应得用户名和密码机制，这样无法提供安全保护吧？</div>2019-09-08</li><br/><li><span>Beast-Of-Prey</span> 👍（2） 💬（1）<div>加密应该是对payload部分进行加解密 这样的话 接收端可以解析出数据部分再解密 否则接收端应该找不到数据部分吧</div>2019-09-12</li><br/><li><span>Geek_4b4f4a</span> 👍（2） 💬（1）<div>老师，能不能介绍一些关于WebRTC源码分析方面的资料，谢谢</div>2019-09-03</li><br/><li><span>王乐</span> 👍（0） 💬（1）<div>请问加密后的SRTP数据是用OpenSSL接口发送的，还是使用之前的RTCPeerConnection接口来发送？</div>2019-10-17</li><br/><li><span>Beast-Of-Prey</span> 👍（0） 💬（1）<div>老师 A与B通讯那个地方 加解密不应该是公钥加密 私钥解密吗？</div>2019-09-12</li><br/><li><span>知更</span> 👍（0） 💬（0）<div>rtc有端到端加密机制吗？支持iOS和android的，客户端自定义加密，服务端如何解密呢？</div>2024-11-25</li><br/><li><span>Hulu warrior</span> 👍（0） 💬（0）<div>您好，对于非对称加解密，我有些疑惑，一般不是应该用公钥加密，而只有对应的私钥可以解密嘛？文中提到用私钥加密，公钥解密和我的认知有些出入。请问是我理解错了吗？</div>2023-07-06</li><br/><li><span>颜</span> 👍（0） 💬（0）<div>老师您好，有几个问题。
经过DTLS握手后，双方确定了对称加密算法的密钥后，再使用SRTP进行对称加解密的吗？SRTP进行加解密操作是基于openssl库实现的，因为webrtc使用的是libsrtp，那本质上是不是libsrtp去调用openssl的加解密方法呢？这些加解密操作是openssl对DTLS协议的实现，所以SRTP协议是基于DTLS协议的？</div>2022-07-02</li><br/><li><span>2xshu</span> 👍（0） 💬（0）<div>使用RTCDataChannel，有没有选项设置为不加密呢？假设场景只是用RTCDataChannel传输视频，视频本身是drm流了，就没必要再加密了。</div>2022-03-26</li><br/>
</ul>