...
<li><span>swtgeo</span> 👍（4） 💬（2）<div>请问有没有关于在服务端进行渲染，然后直接将渲染后的图片输出给前端可视化的参考项目实例呢？遥感影像能不能在后端动态切片渲染，然后输出到前端显示呢，省略切片缓存的过程。</div>2020-11-10</li><br/><li><span>木瓜777</span> 👍（1） 💬（1）<div>距离场 能不能 实现 绘制 球 啊？ 如果可以，能不能提供下思路？ 示例中 float d = distance(st, vPos);  其中 vPos 如果是三维坐标 该如何处理？
</div>2020-09-30</li><br/><li><span>我执</span> 👍（0） 💬（1）<div>大佬， 针对 geojson 过大有哪些优化方案呢？
...
最后能想到的还是 针对原始的 geojson 去做处理， 看能不能降低精度。 但是没有找到相应资料， 希望大佬能提供一定解决思路</div>2021-09-23</li><br/><li><span>Zack</span> 👍（0） 💬（1）<div>服务端渲染里没办法使用gpu吧？</div>2020-12-14</li><br/><li><span>Geek_dc1539</span> 👍（0） 💬（0）<div>gl.POINTS 是有大小限制的，绘制大圆有什么高性能的办法吗</div>2023-08-14</li><br/><li><span>东东</span> 👍（0） 💬（0）<div>厉害👍🏻</div>2020-09-11</li><br/>
</ul>