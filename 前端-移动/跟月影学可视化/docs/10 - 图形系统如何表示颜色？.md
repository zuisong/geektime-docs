...
<li><span>kkopite</span> 👍（8） 💬（2）<div>简单画了下
https:&#47;&#47;codepen.io&#47;action-hong&#47;pen&#47;gOPdjze

hsv2rgb 里传的参数的取值范围都是 (0, 1) 我一开始以为 hue的取值的按(0, 360)传的 ... 看了大半天画出来都是红色 ...
</div>2020-07-15</li><br/><li><span>大土豆</span> 👍（1） 💬（1）<div>老师，还有大名鼎鼎的YUV颜色空间啊。。。</div>2021-07-20</li><br/><li><span>李冬杰</span> 👍（1） 💬（1）<div>https:&#47;&#47;codepen.io&#47;makeco&#47;pen&#47;JjXJbbW?editors=1010

用glsl-doodle实现了半径做饱和度的圆盘</div>2020-08-29</li><br/><li><span>.a_c.</span> 👍（3） 💬（0）<div>深入学习：Lab颜色模型由三个要素组成，一个要素是亮度（L），a 和b是两个颜色通道。a包括的颜色是从深绿色（低亮度值）到灰色（中亮度值）再到亮粉红色（高亮度值）；b是从亮蓝色（低亮度值）到灰色（中亮度值）再到黄色（高亮度值）。因此，颜色混合后将产生具有明亮效果的色彩。

Lab颜色空间中的L分量用于表示像素的亮度，取值范围是[0，100]，表示从纯黑到纯白；a表示从红色到绿色的范围，取值范围是[127，-128]；b表示从黄色到蓝色的范围，取值范围是[127，-128]。

</div>2020-09-01</li><br/><li><span>量子蔷薇</span> 👍（1） 💬（0）<div>终于做出来了，踩了一个坑
...
而片元着色器中拿到的顶点数据是经过线性插值的，是片元着色器当前正在处理的像素的坐标，在这里能拿到圆内每个像素的坐标，远远不止64个！</div>2022-09-28</li><br/><li><span>维李设论</span> 👍（0） 💬（0）<div>个人总结了一个色彩相关知识的视频，B站搜索“前端需要了解的色彩知识”，欢迎浏览</div>2023-01-31</li><br/><li><span>维李设论</span> 👍（0） 💬（0）<div>关于色彩的知识，个人写了一篇总结：https:&#47;&#47;vleedesigntheory.site&#47;tech&#47;cs&#47;color.html</div>2023-01-31</li><br/><li><span>量子蔷薇</span> 👍（0） 💬（0）<div>关于课后题
webgl的渐变只能在顶点之间变化，而圆形的所有顶点都在圆周上，似乎无法实现圆锥渐变和径向渐变？</div>2022-09-27</li><br/><li><span>浩荡如空气</span> 👍（0） 💬（0）<div>老师，请问“人眼看到的颜色vsRGB能表示的颜色”这张图如何理解？横纵坐标表示什么？</div>2021-11-30</li><br/><li><span>攻城狮小白</span> 👍（0） 💬（0）<div>厉害了，老师</div>2021-04-19</li><br/><li><span>蓝洛</span> 👍（0） 💬（1）<div>提问：关于 CIELab 案例 中第3行 const c = d3.lab(30, i * 15 - 150, i * 15 - 150).rgb();  为什么可以直接调用d3.lab? 是需要但单独安装或者引入什么库嚒</div>2021-02-25</li><br/><li><span>.a_c.</span> 👍（0） 💬（0）<div>深入学习：CIE Commission International Eclairage 
...
</div>2020-09-01</li><br/><li><span>Mingzhang</span> 👍（0） 💬（0）<div>受教了，谢谢月影！</div>2020-08-01</li><br/><li><span>Cailven</span> 👍（0） 💬（0）<div>提到颜色，我觉得最搞的是计算颜色之间的差值，在不同颜色系统中差值颜色完全就不一样的。</div>2020-07-15</li><br/>
</ul>