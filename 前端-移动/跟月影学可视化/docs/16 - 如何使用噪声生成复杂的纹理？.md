...
<li><span>MG.Fre</span> 👍（5） 💬（2）<div>小试牛刀：
void main() {
		vec2 st = vUv * 3.0;
		vec3 color = vec3(.0);
		...
	...
	}</div>2021-10-14</li><br/><li><span>化石</span> 👍（2） 💬（0）<div>技术升华成艺术</div>2021-01-26</li><br/><li><span>李冬杰</span> 👍（1） 💬（0）<div>在pixel shader生成艺术中，一般会使用噪声生成数据（纹理）的方式，而不是使用外部数据。</div>2020-09-08</li><br/><li><span>阿不晕</span> 👍（0） 💬（1）<div>这篇太硬核了。。话说这些算法里，dot 的使用目的是什么，为了算夹角么？</div>2022-08-19</li><br/><li><span>西门吹雪</span> 👍（0） 💬（0）<div>太牛了！</div>2020-10-10</li><br/>
</ul>