...
<li><span>别拦我让我学</span> 👍（7） 💬（1）<div>老师，react-loadable与react自身提供的懒加载的api，React.lazy(() =&gt; import())的区别是什么？</div>2021-07-28</li><br/><li><span>Brave</span> 👍（5） 💬（2）<div>1. 合并请求接口
2. 使用OSS服务加载图片资源
3. 制定编码规范、提取公共组件、减少重复代码</div>2021-08-25</li><br/><li><span>珍惜眼前人</span> 👍（2） 💬（0）<div>老师好，serviceWorker的代码，可以加到 https:&#47;&#47;codesandbox.io&#47;s&#47;react-hooks-course-20vzg
吗，想跟着实现一下</div>2021-07-02</li><br/><li><span>Geeker</span> 👍（2） 💬（3）<div>之前项目中有一个需求是，合并多个指定 url 的请求（高频请求，请求地址相同，请求参数不同），想合并请求参数并只请求一次。不知道这样的需求用 service worker 是不是可以实现，求思路，谢谢。</div>2021-07-01</li><br/><li><span>大神博士</span> 👍（1） 💬（0）<div>这里的service worker 缓存 和 设置强缓存有什么区别？</div>2022-02-17</li><br/><li><span>大神博士</span> 👍（1） 💬（0）<div>```javascript
...
export default App;
```</div>2021-07-02</li><br/><li><span>ES12</span> 👍（0） 💬（0）<div>http1.0-&gt;http2.0、浏览器缓存、cdn缓存、swr、组件拆分结合react useMemo&#47;memo这些缓存方式</div>2024-04-16</li><br/><li><span>INFRA_UEX</span> 👍（0） 💬（0）<div>听说 react-loadable 已经不在维护了？不知道是不是确切。</div>2022-11-07</li><br/><li><span>七秒</span> 👍（0） 💬（0）<div>想请教下王老师，这个service worker的代码、注册service worker的代码以及应用的代码 在服务器中说什么样的位置关系，希望得到解答，感激不尽！</div>2021-10-26</li><br/><li><span>知故</span> 👍（0） 💬（0）<div>1. 使用http 2.0
2. cdn</div>2021-09-03</li><br/><li><span>君</span> 👍（0） 💬（0）<div>const AsyncLoadable = (dir: string, loading = Loading) =&gt;
  Loadable({
...
这样的写法貌似不生效 组件无法渲染</div>2021-07-07</li><br/><li><span>L</span> 👍（0） 💬（1）<div>直接利用浏览器本身的缓存机制也可以吧</div>2021-07-06</li><br/><li><span>林大熊</span> 👍（0） 💬（2）<div>老师，你好，对于 react-loadable 和 Service Worker 有两点比较疑惑的地方。
