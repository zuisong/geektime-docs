...
<li><span>何以解忧</span> 👍（5） 💬（1）<div>感觉loader像是编译阶段的，plugin像是编译后的整体的个性化处理</div>2021-07-11</li><br/><li><span>giteebravo</span> 👍（2） 💬（3）<div>
支持 css loader 的 Webpack 配置中，style-loader 为什么是最后一个执行呢？从数组的顺序来看，它应该是在 0 的位置呀。
</div>2021-07-09</li><br/><li><span>江谢木</span> 👍（15） 💬（0）<div>loader是针对不同类型文件间做转换、关系处理，最终生成js模块，而plugin是针对某一文件在打包过程的某一时刻做处理。混淆和压缩是针对文件进行的，所以应该用plugin。</div>2021-07-06</li><br/><li><span>简单</span> 👍（9） 💬（0）<div>混淆和压缩应该用plugin，loader主要做转换</div>2021-07-04</li><br/><li><span>灵感_idea</span> 👍（2） 💬（0）<div>本节标题是“打包部署”，但内容只涉及了打包吧？至于部署，现在多是运维要做的。</div>2023-04-05</li><br/><li><span>莫春梦</span> 👍（0） 💬（0）<div>uglify模块可以做代码压缩混淆</div>2023-07-26</li><br/><li><span>奕晨</span> 👍（0） 💬（0）<div>在进行源代码打包时，通常还有一个重要的步骤，就是代码的混淆和压缩。那么在理解了 loader 和 plugin 之后，你觉得混淆和压缩这个功能，应该用 loader 还是 plugin 去实现呢？
