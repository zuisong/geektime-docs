...
<li><span>Geek_e6b8eb</span> 👍（4） 💬（0）<div>真的后悔没有早一点看到这个课程，之前我就维护了很多中间代码，最后弄得一个组件超级复杂。 &#47;(ㄒoㄒ)&#47;~~</div>2022-03-28</li><br/><li><span>Isaac</span> 👍（15） 💬（5）<div>思考题回答：
...
https:&#47;&#47;github.com&#47;streamich&#47;react-use&#47;blob&#47;90e72a5340460816e2159b2c461254661b00e1d3&#47;src&#47;useSearchParam.ts#L8</div>2021-06-10</li><br/><li><span>Jerryz</span> 👍（9） 💬（3）<div>默认 history.pushState 和 history.replaceState 都没有对应的监听事件。react-use patch 了history 对象。</div>2021-06-10</li><br/><li><span>Geek_0330fe</span> 👍（4） 💬（2）<div>react-use 的 useSearchParams 中也使用了一个 state 去维护了一个状态，这是不是说明复杂度不会消失，只会转移呢？我们只是把我们本来要维护的那个状态交给第三方库去维护了。</div>2021-07-18</li><br/><li><span>Geeker</span> 👍（4） 💬（1）<div>状态最小化原则直接影响了代码的复杂度</div>2021-06-10</li><br/><li><span>Isaac</span> 👍（3） 💬（8）<div>老师，文章中的列表筛选的例子，虽然使用 useMemo 可以缓存，但是如果多个组件实例用到，岂不是还是会出现多次计算？</div>2021-06-10</li><br/><li><span>守望</span> 👍（2） 💬（1）<div>这是不是和单向数据流差不多呀！！！
数据流向总是一条线，不要开新分支</div>2021-06-10</li><br/><li><span>傻子来了快跑丶</span> 👍（2） 💬（4）<div>&#47;&#47; 每当 searchKey 或者 data 变化的时候，重新计算最终结果
...
这段代码应该在父组件中，拿到data数据之后再传进去</div>2021-06-10</li><br/><li><span>寇云</span> 👍（2） 💬（0）<div>老师第一个例子中，数据的请求放到当前组件中好还是由外部传入好？</div>2021-06-12</li><br/><li><span>老实人Honey</span> 👍（0） 💬（0）<div>onChange 当props传递， 不担心每次都会变更吗 比较props是否一致用的是Object.is</div>2024-09-15</li><br/><li><span>Geek_130786</span> 👍（0） 💬（0）<div>&#47;&#47; 处理用户的搜索关键字
  const handleSearch = useCallback(evt =&gt; {
    setSearchKey(evt.target.value);
    setFiltered(data.filter(item =&gt; (item.title.includes(evt.target.value))));
  }, [filtered]);
老师，这里useCallback中没有使用到filtered，为什么依赖需要设置filtered？</div>2024-02-20</li><br/><li><span>Geek_bing</span> 👍（0） 💬（0）<div>引入useSearchParam的那个例子，好像有问题呢</div>2024-02-08</li><br/><li><span>吴颜</span> 👍（0） 💬（0）<div>有个问题就是什么时候要用useMemo和useCallback来改善性能而不是直接让组件直接循环执行，这是性能与代码简洁性的平衡</div>2023-09-07</li><br/><li><span>Geek_z46e7g</span> 👍（0） 💬（0）<div>总结：
1.能计算获得的数据，就要重复定义
2.多个状态要注意统一源头
3.组件内部与外部共享状态数据时，统一由外部来控制即可，不用子组件重复定义</div>2023-07-28</li><br/><li><span>潮汐</span> 👍（0） 💬（0）<div>保持状态一致性的两个原则：
1.保持状态最小化
2.保证唯一数据来源
...
老师的几个例子给人豁然开朗，醍醐灌顶的感觉！</div>2023-02-08</li><br/>
</ul>