...
<li><span>润培</span> 👍（6） 💬（1）<div>刚刚提了一个PR</div>2021-12-24</li><br/><li><span>小胖</span> 👍（3） 💬（1）<div>track函数哪里，依赖的添加，最后老师的判断是有问题的，那样写的话对于一个key就只能添加一次依赖。
&#47;&#47; 测试用例改成下面这样，就过不了了
    let val
    let val2 
    effect(()=&gt;{
    ...
    expect(val).toBe(0)
    expect(val2).toBe(0)
</div>2021-12-24</li><br/><li><span>Kim Yin</span> 👍（3） 💬（1）<div>track 函数的 20~27 行好像有点问题，感觉应该把 22~26 行移到 if (!deps) 外面，不然 deps = new Set() 以后，if (!deps.has(activeEffect) &amp;&amp; activeEffect) 这句判断就没意思了 —— deps.has(activeEffect) 永远为 false
</div>2021-12-24</li><br/><li><span>heart</span> 👍（0） 💬（1）<div>github404了</div>2024-07-09</li><br/><li><span>openbilibili</span> 👍（0） 💬（1）<div>你好，问一下，你看的vue是哪个版本的？我对照一些 vue源代码 和 weiyouyi的代码，有些地方不太相同，但是又无法理解。</div>2022-01-05</li><br/><li><span>润培</span> 👍（14） 💬（0）<div>相比于 Vue2 使用的 Object.defineProperty，Vue3 不需要提前递归收集依赖，初始化的速度更快；
Vue2 收集依赖的过程中会产生很多的 Dep 对象，Vue3 可以节省这部分的内存开销；
Vue2 无法监听数组、对象的动态添加、删除，需要通过 $set、$delete，增加学习成本；
Vue2 无法监听 Set、Map，只能处理普通对象。</div>2021-12-24</li><br/><li><span>Geek_13e539</span> 👍（8） 💬（1）<div>computed的讲解还能再详细一点么？看了几次 也还是没太理解</div>2022-01-20</li><br/><li><span>来个offer</span> 👍（4） 💬（0）<div>哪里来的activeEffect？没看到啊</div>2022-06-12</li><br/><li><span>Hector</span> 👍（3） 💬（0）<div>这一篇搭配《vue.js的设计与实现》真是相得益彰。</div>2022-03-22</li><br/><li><span>子阳</span> 👍（2） 💬（0）<div>经过一个星期的学习，终于自己实现了。无比的高兴。继续挑战下一章</div>2022-01-17</li><br/><li><span>大将</span> 👍（0） 💬（0）<div>目前所有课程中最有用的一章</div>2024-03-15</li><br/><li><span>cxwcfea</span> 👍（0） 💬（0）<div>这个实现里的， computed 中的 track 和 trigger 应该都没有实际发挥作用吧。key值都传的不对。然后 computed 的逻辑看起来本来也不需要直接调用 track 和 trigger</div>2023-07-20</li><br/><li><span>Soffio</span> 👍（0） 💬（0）<div>思考题：新增v-memo，核心作用是复用vnode。</div>2022-09-25</li><br/><li><span>Geek_39e094</span> 👍（0） 💬（0）<div>好像监听不到外层对象的变化，只能监听到具体值，</div>2022-08-05</li><br/><li><span>xiao.pao</span> 👍（0） 💬（0）<div>大佬，调度器是不是写的有问题？trigger() 中调用时直接effect.scheduler()，没有将job:()=&gt;void作为参数回传给调度器，那也就控制不了时机吧
</div>2022-07-31</li><br/>
</ul>