...
<li><span>海阔天空</span> 👍（4） 💬（1）<div>表格算是elment中比较难的组件了。感觉 Naive Ui 的使用方式配置得比较多，element 让人更好理解一点
</div>2021-12-17</li><br/><li><span>银太</span> 👍（0） 💬（1）<div>请教下，我是想对table封装成xtable，那么在page中使用的时候原来的table的slot就会写到xtable的slot，那table怎么渲染？
x-table：
&lt;div&gt;
  &lt;a-table&gt;
...
&lt;page&gt;
  ...
  &lt;&#47;x-table&gt;
&lt;&#47;page&gt;</div>2021-12-17</li><br/><li><span>于三妮</span> 👍（0） 💬（1）<div>这里有代码有点看不懂了</div>2021-12-17</li><br/><li><span>Ankhetsin</span> 👍（1） 💬（0）<div>PC端的表格一般带分页</div>2021-12-21</li><br/><li><span>海阔天空</span> 👍（0） 💬（0）<div>这代码在哪里</div>2022-10-11</li><br/><li><span>落风</span> 👍（0） 💬（0）<div>好家伙，是时候优化一下自己的组件库了</div>2022-04-13</li><br/><li><span>CrazyMuyang</span> 👍（0） 💬（0）<div>在移动端，如果在页面最低下有一个列表组件需要用虚拟滚动处理，该怎么实现呢。？
我目前了解的实现方案都要有一个固定高度的滚动区域，但这种场景下并不能给这个列表显示区域一个固定高度</div>2022-03-30</li><br/><li><span>费城的二鹏</span> 👍（0） 💬（0）<div>我们的项目中，会有超级大的列表，目前用v-for做的，数据多了dom会占用很多内存，可以用虚拟滚动优化。</div>2021-12-19</li><br/>
</ul>