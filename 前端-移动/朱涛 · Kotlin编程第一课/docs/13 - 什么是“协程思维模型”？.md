...
<li><span>曾帅</span> 👍（55） 💬（5）<div>之前看过另一位大佬 扔物线 说过的一个点，这里也想请教一下。就是运行在 JVM 上的 Kotlin 协程，本质上就是线程的封装，因为 JVM 上是没有协程这种东西。所以文章中说的一些概念，在 JVM 上运行的 Kotlin 来说，可能不适用。</div>2022-02-22</li><br/><li><span>$Kotlin</span> 👍（42） 💬（1）<div>我已经在网上看了一周的协程资料了，在这里第一次看见这个抓手的思维模型，思路一下子就打开了，太强了。</div>2022-02-11</li><br/><li><span>focus on</span> 👍（33） 💬（1）<div>请问文章中的动图是用什么软件制作的呢？</div>2022-02-11</li><br/><li><span>Paul Shan</span> 👍（27） 💬（1）<div>个人觉得把协程看做是封装的线程的看法帮助不大，线程是操作系统的概念，编程语言只是封装实现，让任务能够方便的被操作系统调度，调度是操作系统的事情，编程语言基本不管。协程是编程语言级别的任务，最终的调度可能是一个线程，也可能是共享的线程库，也可能是多个不同的线程，从线程角度很难理解协程的非阻塞，也无法理解协程之间的协同。我觉得把协程理解为编程语言级别的任务异步任务管理比较合适。虽然最终任务还是要给CPU执行，编译器（包括运行器）承担了大部分的分配调度责任。如果任务是CPU密集的，协程能提高的效率不多，毕竟CPU就这么多，操作系统的线程就是为这种场景设计的。如果任务是IO密集的，当年设计操作系统的时候，可没有想到会有那么多IO并发。协程比线程多出了线程内部异步的能力，让任务在CPU和IO之间不断调度轮转，可以充分利用CPU和IO设备，进而提高效率。Android程序和很多调用其他微服务的api都是IO很多的任务，协程能够提高效率。
</div>2022-03-21</li><br/><li><span>如浴春风</span> 👍（13） 💬（1）<div>「协程的“非阻塞”」那里，“阻塞”的概念可以可能会有同学存在跟我一样困扰。
...
不得不说，作者的协程图示真形象！恳切建议其他同学一起细细揣摩下。</div>2022-05-02</li><br/><li><span>ACE_Killer09</span> 👍（8） 💬（7）<div>repeat(3) { launch { repeat(3) { println(Thread.currentThread().name) delay(100) } } }

repeat 是重复，launch是 创建协程。重复创建3个协程，每个协程执行3次，执行的内容是 打印协程执行时所在的线程。

结果说明同一个协程 可以再不同的线程上执行。</div>2022-02-11</li><br/><li><span>_</span> 👍（5） 💬（1）<div>《深入理解JVM虚拟机》中提到过“协程”是“用户级线程”，不过这是从操作系统效率角度来称呼它，所以反而不太容易让“人”理解。而将“Kotlin协程”理解为“线程级Task”这种从上到下的角度要直观的多！大佬讲的很棒！👍🏻</div>2022-04-29</li><br/><li><span>Geek_0d808c</span> 👍（4） 💬（1）<div>在处理高并发，轻量任务的前提下，协程少了，线程创建的系统调用，上下文切换等时间，以及线程内存开销，是线程的复用，所以特定场景下，代价更小。</div>2022-02-11</li><br/><li><span>$Kotlin</span> 👍（3） 💬（1）<div>协程比乱用线程要高效，和合理使用线程池效率一致。</div>2022-02-11</li><br/><li><span>7Promise</span> 👍（2） 💬（1）<div>协程可以在适当的时机挂起和恢复，一个协程在挂起等待时，另外的协程也可以工作，比阻塞的线程更节省资源。</div>2022-02-11</li><br/><li><span>$Kotlin</span> 👍（2） 💬（1）<div>协程比乱用线程要高效，和合理使用线程池效率一致。</div>2022-02-11</li><br/><li><span>Renext</span> 👍（1） 💬（1）<div>为什么协程可以在线程直接灵活切换呢？</div>2022-02-28</li><br/><li><span>白乾涛</span> 👍（1） 💬（3）<div>所以到底为啥一个线程中能运行成千上万的协程？
还是说这个问题被选择性忽略了一个前提：底层并不是只有一个线程，而是要有一个线程池。

就像我之所以能在辛辛苦苦、省吃俭用奋斗一年后，买名车买豪宅，不是因为我攒了 10 万块，而是我老爸又给我了 1 个亿 ...</div>2022-02-20</li><br/><li><span>陈</span> 👍（1） 💬（1）<div>感觉使用协程可以更充分地使用cpu时间片，减少了线程频繁切换的额外开销</div>2022-02-18</li><br/><li><span>jim</span> 👍（1） 💬（1）<div>非常的精彩，比很多书籍强很多。</div>2022-02-12</li><br/>
</ul>