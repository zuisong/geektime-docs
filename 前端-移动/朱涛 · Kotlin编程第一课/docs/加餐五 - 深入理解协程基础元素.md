...
<li><span>Allen</span> 👍（15） 💬（1）<div>关于思考题的思考：
...
总的来说，suspendCoroutineUninterceptedOrReturn{} 使用的学习成本要高很多。</div>2022-03-24</li><br/><li><span>Paul Shan</span> 👍（4） 💬（1）<div>suspendCoroutine 用了SafeContinuation，里面有原子读和一些状态判断，应该是处理多线程和重复resume的问题。</div>2022-03-25</li><br/><li><span>辉哥</span> 👍（2） 💬（1）<div>suspendCoroutine{}能保证suspendCoroutine的挂起点（也就是传入lambda的continuation参数）只会被resume一次.因为实际上传入的continuation为SafeContinuation,多次调用会抛异常,可以规范用户的使用</div>2022-03-23</li><br/><li><span>ZircoN</span> 👍（1） 💬（1）<div>suspendCoroutineUninterceptedOrReturn 这个方法在IDE里为啥没法引用到？</div>2022-04-18</li><br/><li><span>ACE_Killer09</span> 👍（1） 💬（1）<div>对于 代码段 10 和 11 suspendCoroutineUninterceptedOrReturn 必须要考虑返回值，如果return的不是COROUTINE_SUSPENDED，就不是一个真正的挂起函数。而如果是使用 suspendCoroutine suspendCancellableCoroutine，就不用考虑返回值，他们一定是挂起的，只要最后通过Continuation 返回值即可。

另外  Continuation中 resume resumeWith resumeWithException 的原理都是 resumewith 这三个具体应用场景除了传入值区别 还有 其他区别吗？</div>2022-04-12</li><br/><li><span>杨小妞</span> 👍（1） 💬（1）<div>Continuation 的resumeWith函数只有在，回调函数转挂起函数或者java调用挂起函数的时候才发挥作用吗？
在挂起函数执行挂起函数的状态机里面，好像没看到resumeWith的影子</div>2022-03-29</li><br/><li><span>anmi</span> 👍（0） 💬（4）<div>理解课程的关键：
...
}</div>2024-04-13</li><br/><li><span>anmi</span> 👍（0） 💬（0）<div>前面看了那么多次没有感觉，现在反应过来了：函数也能类型强转！！！！！</div>2023-12-04</li><br/><li><span>Emmm</span> 👍（0） 💬（0）<div>suspendCoroutine可以理解为会阻塞外层协程吧，这样就可以实现同步得到耗时任务回调出来的结果，然后进行后续的操作，这样的话看外层协程作用域的代码就是顺序同步执行</div>2022-07-06</li><br/>
</ul>