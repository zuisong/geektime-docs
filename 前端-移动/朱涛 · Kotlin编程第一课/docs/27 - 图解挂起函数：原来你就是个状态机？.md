...
<li><span>荷兰小猪8813</span> 👍（6） 💬（2）<div>我刚看了字节码，总结如下：
...
​      如果一个挂起函数挂起，它的返回值会是 CoroutineSingletons.COROUTINE_SUSPENDED；</div>2022-04-12</li><br/><li><span>Geek_Adr</span> 👍（4） 💬（1）<div>    &#47;&#47; JAVA 调用 挂起函数
...
</div>2022-04-10</li><br/><li><span>Shanks-王冲</span> 👍（4） 💬（1）<div>思考题：后知后觉
...
*关于如何在Java中调用：之前有课程提到，在Java中的getContext()中可考虑传入EmptyCoroutineContext.INSTANCE</div>2022-03-30</li><br/><li><span>Allen</span> 👍（3） 💬（1）<div>关于思考题的思考：

在 Java 中是可以调用挂起函数的，当挂起函数通过 Kotlin 编译器进行 CPS 转换后，对应的挂起函数就被转换成了带 Callback 参数（Continuation 接口）的普通函数，只要传入 Continuation 接口的实现就可以了。</div>2022-03-21</li><br/><li><span>白</span> 👍（1） 💬（2）<div>我不是很明白是怎么恢复协程，也就是调用resumewith，java反编译过去也没看到调用</div>2022-05-04</li><br/><li><span>神秘嘉Bin</span> 👍（1） 💬（1）<div>HelloSuspendTestKt.hello(new Continuation&lt;Integer&gt;() {
            @NotNull
            @Override
            public CoroutineContext getContext() {
        ...
        });</div>2022-03-24</li><br/><li><span>H.ZWei</span> 👍（0） 💬（1）<div>有点类似rxjava操作符的嵌套</div>2022-04-28</li><br/><li><span>荷兰小猪8813</span> 👍（0） 💬（1）<div>1、invokeSuspend 不是协程的入口，是 resumeWith 的入口，
恢复协程是调用的 continuation.resumeWith，这个会调用 invokeSuspend</div>2022-04-11</li><br/><li><span>荷兰小猪8813</span> 👍（0） 💬（2）<div>这样看，每个挂起函数，都会创建一个继承了 ContinuationImpl 的匿名内部类对象，把传进来的 Continuation 包起来～～～</div>2022-04-11</li><br/><li><span>荷兰小猪8813</span> 👍（0） 💬（1）<div>withContext 为啥没有创建新的协程呢？？？我看源码都有对应的 job 和 coroutome.start</div>2022-04-11</li><br/><li><span>杨小妞</span> 👍（0） 💬（1）<div>以下代码： 挂起函数b中的c、d之间不存在回调关系，而挂起函数a的b，a、b之间存在回调关系？可以这么理解吗？
    suspend fun a() {
        b()
    }
    ...
    }</div>2022-03-29</li><br/><li><span>杨小妞</span> 👍（0） 💬（1）<div>有个疑问：TestContinuation维护一个状态机来执行挂起函数，那么运行结果什么时候回调出去呢？是在最后一个swithc分支吗？例如testCoroutine是被其他挂起函数调用</div>2022-03-29</li><br/><li><span>ACE_Killer09</span> 👍（0） 💬（1）<div>挂起函数中在线程中的执行结果是怎么返回的？比如andorid中子线程的结果怎么给到主线程呢？</div>2022-03-23</li><br/><li><span>魏全运</span> 👍（0） 💬（2）<div>请问kotlin是怎么执行下一个状态机逻辑的呢？</div>2022-03-23</li><br/><li><span>Geek_48edaa</span> 👍（1） 💬（0）<div>请问老师，关于kotlin源码总是找不到实现类，只能通过ctrl shift + F凭运气找。运气好能搜到，运气不好就搜不到。

有时候找到了也看不懂，毫无思路。其实不管是学Android Framework，还是kotlin协程我都有这样的问题。就是只能对照着文章跟博客去学习，一旦没了指引，自己完全没能力自学了。</div>2022-12-05</li><br/>
</ul>