...
<li><span>qinsi</span> 👍（1） 💬（1）<div>尝试用正则分割
...
```
</div>2022-02-02</li><br/><li><span>郑峰</span> 👍（1） 💬（2）<div>```Kotlin
...
```</div>2022-02-02</li><br/><li><span>白乾涛</span> 👍（0） 💬（1）<div>fun solveEquation(equation: String): String { &#47;&#47; x+5-3+2x=6+x-2
...
}</div>2022-03-02</li><br/><li><span>Geek_Adr</span> 👍（0） 💬（1）<div>符号处理复杂了，其它与 @郑峰 略同
    &#47;&#47; 分数的数据结构 symbol为-1 1
    &#47;&#47; 注意分子&#47;分母为正整数
...
</div>2022-02-20</li><br/><li><span>Dash</span> 👍（0） 💬（0）<div>```kotlin
class Solution {
        fun solveEquation(equation: String): String {
...
```</div>2025-01-22</li><br/><li><span>春夏秋冬</span> 👍（0） 💬（0）<div>class Solution {
    fun solveEquation(equation: String): String =
        equation.split(&quot;=&quot;)
            .map(this::statistics)
            .calculate()
