...
<li><span>白乾涛</span> 👍（21） 💬（5）<div>我觉得Java的 &lt;? extends T&gt; &lt;? super Object&gt; 语法不抽象呀，相比 kotlin 中的 in out 反而更清晰</div>2022-01-20</li><br/><li><span>A Lonely Cat</span> 👍（15） 💬（2）<div>Java中的协变：&lt;? extends T&gt;
Java中的逆变：&lt;? super Object&gt;
Java中的“星投影”：&lt;?&gt;</div>2022-01-19</li><br/><li><span>$Kotlin</span> 👍（12） 💬（2）<div>声明处型变无法支持又有in又有out，只能在使用处根据情况型变。</div>2022-01-19</li><br/><li><span>白乾涛</span> 👍（9） 💬（3）<div>协变：&lt;? extends T&gt;  out-作为返回值-可以读取，不可以写入
逆变：&lt;? super T&gt;  in-作为参数-可以写入，不可以读取(只能以Object读取)</div>2022-02-09</li><br/><li><span>白乾涛</span> 👍（4） 💬（1）<div>老师，文稿中说下面的代码会报错，实际上并不会报错呀
...
}</div>2022-02-09</li><br/><li><span>neo</span> 👍（2） 💬（1）<div>从型变的父子关系来分类的话，分为逆变和协变
泛型作为参数，用 in；泛型作为返回值，用 out
这两个条规则有冲突的时候应该怎么办呢</div>2022-04-18</li><br/><li><span>Paul Shan</span> 👍（1） 💬（1）<div>使用处的型变,可以使用星投影，更灵活，但是范围比较小。声明处的型变,解决的是泛型类本身的约束，一次定义，所有使用的地方都受到相应影响，星投影处也不例外，使用处不得修改。个人觉得优先使用声明处的型变，使用处的型变会导致不同的使用处，型变不同的情况，带来额外的复杂性。星投影我在实际中基本没用过，老师能否举一个实际中用星投影能很好解决问题的场景。
</div>2022-03-19</li><br/><li><span>Renext</span> 👍（1） 💬（1）<div>打卡</div>2022-01-24</li><br/><li><span>阶前听雨</span> 👍（1） 💬（1）<div>很赞，基本能分清了，再多看几遍以加深理解。</div>2022-01-23</li><br/><li><span>louc</span> 👍（0） 💬（1）<div>历史上最好的讲泛型，通俗易懂</div>2022-04-30</li><br/><li><span>阿翔</span> 👍（0） 💬（1）<div>需要子类集合，传入父类集合的代码有乱码</div>2022-03-08</li><br/><li><span>白乾涛</span> 👍（0） 💬（1）<div>老师，文稿中的很多代码都不符合语法，能不能把代码完善一下呀
