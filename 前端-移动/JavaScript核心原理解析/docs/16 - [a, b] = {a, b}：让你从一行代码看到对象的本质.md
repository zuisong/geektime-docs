...
<li><span>Astrogladiator-埃蒂纳度斯</span> 👍（11） 💬（1）<div>“有迭代器的对象”在哪些场合中可以替代“索引数组”？
正如文中老师所讲的那样， 究其根本来说，索引数组其实是关联数组的一个特例——被存取的数据所关联的名字就是它的索引。关联数组可以通过实现对象迭代器的接口去实现。所以说有迭代器的对象可以适应于那些键名是非数字的场合。但是索引数组是一组连续的数据在一块内存区间，一般会有初始大小，而用迭代器实现的关联数组是非连续的数据。Map，Set， WeakMap, WeakSet，就是这种关联数组。</div>2019-12-20</li><br/><li><span>穿秋裤的男孩</span> 👍（2） 💬（2）<div>[a, b] = Object.values({a, b})
这个并不能确定a一定等于obj.a吧</div>2020-01-08</li><br/><li><span>Elmer</span> 👍（1） 💬（1）<div>[a, b] = {a, b}如果从可执行角度可以这样理解吗
本身是个赋值表达式，右侧是个对象字面量，左侧是一个可执行结构？</div>2020-01-07</li><br/><li><span>青史成灰</span> 👍（0） 💬（1）<div>老师，有2个问题想问下：
...
②：从&gt; Astrogladiator的评论中描述看，老师说索引数组的连续性意义不大。但是在连续的数据结构对于查找效率和缓存都是很友好的，请问老师这又要作何理解？</div>2020-01-18</li><br/><li><span>墨灵</span> 👍（9） 💬（0）<div>这绝对是我今年看过最难的专栏，但也是我顿悟最多的专栏。</div>2020-03-20</li><br/><li><span>孜孜</span> 👍（0） 💬（0）<div>我最近在读一本书，Structure and Interpretation of Computer Programs(sicp)，突然感觉和这本课不知道哪里有很多相似。</div>2022-08-09</li><br/><li><span>小炭</span> 👍（0） 💬（0）<div>https:&#47;&#47;juejin.im&#47;post&#47;6844903715078406151
私有字段
...
6. 为了证明这一部分的合理性而创建了单独的FAQ，然而不够强力论据又导致了新的争论（133，136）；</div>2020-11-10</li><br/><li><span>kkxue</span> 👍（0） 💬（0）<div>化繁为简，程序原本已成宝典</div>2020-08-02</li><br/><li><span>水木年华</span> 👍（0） 💬（0）<div>周老师关于对象和数组的描述，让我恍然大悟。</div>2020-01-05</li><br/><li><span>许童童</span> 👍（0） 💬（0）<div>当需要遍历一个数组的时候，比如for of时，但对其遍历的顺序不是特别敏感时，是可以用可迭代的对象来替代索引数组的。</div>2019-12-22</li><br/>
</ul>