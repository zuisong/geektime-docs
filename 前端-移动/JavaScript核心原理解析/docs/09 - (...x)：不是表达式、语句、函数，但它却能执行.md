...
<li><span>leslee</span> 👍（4） 💬（1）<div>1. 他可以手动调用..., 在you dont know js 里面说过这个 .throw 可以委托异常.
...
4. 把x展开, 返回迭代值 , 如果 没有 * 返回的将是迭代器函数, .</div>2019-12-26</li><br/><li><span>HoSalt</span> 👍（3） 💬（1）<div>老师，为什么fn(...args)怎么变成了一个个的参数，而在{...args}怎么变成了对象属性的一部分，这是谁控制的？</div>2020-05-18</li><br/><li><span>Smallfly</span> 👍（1） 💬（2）<div>
&#47;&#47; 迭代函数
function foo(x = 5) {
  return {
    next: () =&gt; {
...
&gt; Object.getOwnPropertyNames(tor.constructor.prototype)
[ &#39;constructor&#39;, &#39;next&#39;, &#39;return&#39;, &#39;throw&#39; ]
</div>2020-01-16</li><br/><li><span>Sam</span> 👍（0） 💬（2）<div>根据周老师的指点，谁创建谁调用。以下是我想到一个办法调用throw方法(不知理解是否合理)：
function* foo() {
  yield 1
  yield 2
  yield 3
...
}</div>2021-08-12</li><br/><li><span>油菜</span> 👍（0） 💬（1）<div>&quot;用户在这里设计异常处理过程，那么 foo2() 中的 touch(x) 管理和涉及的资源都无法处理。&quot;
...
结果：touch(x)可以处理异常
c:Error: hard break   
5 4 3 2 1</div>2020-11-13</li><br/><li><span>Elmer</span> 👍（0） 💬（1）<div>这个贴近语义的语法和解析为可执行结构的关系是什么呢</div>2020-07-07</li><br/><li><span>antimony</span> 👍（0） 💬（1）<div>老师，请问一下这个迭代和递归的关系您是从sicp中了解到的吗？</div>2020-02-09</li><br/><li><span>weineel</span> 👍（1） 💬（1）<div>四个思考题，都没能找到很好的答案。。。</div>2019-12-02</li><br/><li><span>吉法师</span> 👍（0） 💬（0）<div>迭代感觉用不着啊……</div>2021-04-06</li><br/><li><span>油菜</span> 👍（0） 💬（0）<div>1 语言设计者有考虑到异常处理，但功能上还不够完善。类似写了个todo
...
a.next(); &#47;&#47; { done: false, value: 4 }</div>2020-11-13</li><br/><li><span>亦枫丶</span> 👍（0） 💬（0）<div>看了好久，对于第二个思考题恍然大悟，我想可以用文章开头的 
