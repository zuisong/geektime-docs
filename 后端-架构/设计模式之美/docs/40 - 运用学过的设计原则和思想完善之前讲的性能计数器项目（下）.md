...
<li><span>李小四</span> 👍（94） 💬（2）<div>设计模式_40:
# 作业
...
2&gt; 一步一步地重构，其实解决的是自信问题：
	...
	- 成就感让你追求卓越，愧疚感只是让你不想犯错(而这是做不到的)。</div>2020-02-22</li><br/><li><span>辣么大</span> 👍（20） 💬（5）<div>思考题：startRepeatedReport()多次调用，会启动多个线程，每个线程都会执行统计和输出工作。
...
代码放在了：https:&#47;&#47;github.com&#47;gdhucoder&#47;Algorithms4&#47;tree&#47;master&#47;designpattern&#47;u40</div>2020-02-03</li><br/><li><span>Andy</span> 👍（16） 💬（0）<div>老师能提供课程代码吗？</div>2020-02-04</li><br/><li><span>平风造雨</span> 👍（15） 💬（0）<div>调用多次可以通过多线程共享的状态变量来解决，CAS或者加锁进行状态的变更。</div>2020-02-03</li><br/><li><span>6点无痛早起学习的和尚</span> 👍（8） 💬（1）<div>这两篇实战对自己的感觉就是越来越懵，可能还需要多读读，也不知道是不是跟自己的项目做的很少有原因，前面的理论都还能消化</div>2020-03-04</li><br/><li><span>undefined</span> 👍（7） 💬（0）<div>深入浅出，过瘾。</div>2020-02-03</li><br/><li><span>Jxin</span> 👍（7） 💬（6）<div>先回答问题：
...
</div>2020-02-03</li><br/><li><span>高源</span> 👍（7） 💬（3）<div>老师39,40课完整源代码可以提供下吗，我准备好好研究学习下</div>2020-02-03</li><br/><li><span>Q罗</span> 👍（4） 💬（0）<div>终于进去到期待已久的设计模式了</div>2020-03-18</li><br/><li><span>javaadu</span> 👍（4） 💬（0）<div>课堂讨论，使用一个标记flag作为该函数被调用国的标记，并给这个函数加锁，解决并发问题</div>2020-02-03</li><br/><li><span>小晏子</span> 👍（4） 💬（1）<div>课后思考：如果 startRepeatedReport()被多次调用，那么会生成多个线程以fixed rate去请求然后输出结果到console上，一方面导致输出结果混乱，另一方面增加了系统的负担。
...
```</div>2020-02-03</li><br/><li><span>空白昵称</span> 👍（3） 💬（1）<div>关于大量数据读取到内存导致性能下降等问题，应该就是算法之美那里讲的堆排吧…… 两个课程举例有相关性，点个赞👍</div>2020-03-15</li><br/><li><span>饭粒</span> 👍（3） 💬（0）<div>这个案例很赞，要找时间手敲一遍。</div>2020-03-08</li><br/><li><span>守拙</span> 👍（3） 💬（0）<div>课堂讨论:
...
startRepeatedReport()多次调用会导致SingleThreadExecutor排队执行任务.  从性能上, 浪费系统资源, 从需求上, 不符合方法设计的初衷. 解决方案之一是使用免锁容器存储唯一值, 作为任务已开始调度的flag, 在startRepeatedReport()方法判断: 如果任务已开始调度, 则直接return.</div>2020-02-04</li><br/><li><span>J.Smile</span> 👍（3） 💬（0）<div>沙发，打卡！一路跟进！</div>2020-02-03</li><br/>
</ul>