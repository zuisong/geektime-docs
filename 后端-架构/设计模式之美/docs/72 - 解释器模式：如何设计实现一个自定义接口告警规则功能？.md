...
<li><span>花儿少年</span> 👍（11） 💬（1）<div>类似于规则引擎是不是也是个解释器，只不过是个复杂很多的解释器</div>2020-04-27</li><br/><li><span>Panmax</span> 👍（68） 💬（8）<div>第二个代码示例中的代码段：
  if (operator.equals(&quot;+&quot;)) {
    combinedExp = new AdditionExpression(exp1, exp2);
  } else if (operator.equals(&quot;-&quot;)) {
    combinedExp = new AdditionExpression(exp1, exp2);
...
应改为：
  ...
  }</div>2020-04-18</li><br/><li><span>LiuHu</span> 👍（42） 💬（0）<div>SpEL（Spring Expression Language )  就是典型的解释器模式实现。
在项目中实现简单的自动化规则过滤器，就是将过滤器配置翻译成 SpEL 表达式执行的</div>2020-05-05</li><br/><li><span>javaadu</span> 👍（21） 💬（0）<div>我是做风控技术的，发现这个模式对我理解规则引擎的设计很有帮助</div>2020-04-29</li><br/><li><span>迷羊</span> 👍（16） 💬（3）<div>因为做的是数据类型的项目，就是根据自定义的SQL来创建API以供可以通过http形式直接调用，项目中就是用Druid来解析SQL的，用的就是解释器模式，SQL语句中的每个字符对应一个表达式。</div>2020-04-18</li><br/><li><span>kylexy_0817</span> 👍（11） 💬（0）<div>ES中的各种分词器</div>2020-04-25</li><br/><li><span>Ken张云忠</span> 👍（11） 💬（2）<div>Java中注解处理器做的就是解释的功能，以及前端编译时的语法分析、语义分析，后端编译时生成的中间表达式，用来触发更多优化，优化的处理可以理解为高效的解释，最终生成机器可以执行的汇编指令。</div>2020-04-17</li><br/><li><span>鲨鱼辣椒</span> 👍（10） 💬（0）<div>定时任务中的cron表达式</div>2020-05-20</li><br/><li><span>辣么大</span> 👍（10） 💬（0）<div>关于问题一，使用过。偏向科研，自定义一门语言，然后通过语法解析器分析读入。
...
instance inst_dbn {
	...
	horizon  = 20;
	discount = 0.9;
}</div>2020-04-17</li><br/><li><span>test</span> 👍（7） 💬（1）<div>加括号的话，要加一个ExpressionManager，在manager里面用括号把表达式划分为几段，再根据表达式间是 与 还是 或 来添加最上面那一层的表达式</div>2020-04-17</li><br/><li><span>李稳</span> 👍（6） 💬（0）<div>输入规则，解析规则，输入数据，使用规则判断数据</div>2020-04-17</li><br/><li><span>苗</span> 👍（5） 💬（0）<div>使用过nutz开源项目的表达式引擎，可以扩展各种操作符。</div>2020-04-29</li><br/><li><span>马球先生</span> 👍（5） 💬（0）<div>告警规则的代码 真是读了半天才看懂😂</div>2020-04-18</li><br/><li><span>Heaven</span> 👍（5） 💬（3）<div>https:&#47;&#47;github.com&#47;HeavenXin&#47;MonitorExpressionTest
关于第二道题,趁着中午写了写,大家可以参考一下</div>2020-04-17</li><br/><li><span>南山</span> 👍（3） 💬（0）<div>还真有项目有这种场景，动态表单的值校验，可以自定义校验，校验之间还可以相互组合。但是用的是组合模式，后面思考一下解释器模式如何使用，貌似更适合~</div>2020-04-21</li><br/>
</ul>