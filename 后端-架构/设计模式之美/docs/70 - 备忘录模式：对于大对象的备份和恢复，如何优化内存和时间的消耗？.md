...
<li><span>Demon.Lee</span> 👍（5） 💬（3）<div>System.out.println(inputText.toString());  ---&gt;    System.out.println(inputText.getText());</div>2020-04-13</li><br/><li><span>Geek_5227ac</span> 👍（4） 💬（1）<div>感觉很像prototype原型模式啊，比如考虑到内存，时间的消耗采用增量备份这里，老师能说下具体有什么区别吗？</div>2020-06-18</li><br/><li><span>DexterPoker</span> 👍（94） 💬（0）<div>MySQL数据库低频全量备份，结合binlog增量备份，来恢复数据。</div>2020-04-13</li><br/><li><span>张晋 🇨🇳</span> 👍（39） 💬（5）<div>看了王争老师很多课了，觉得都很好，但是不知道是疲惫了还是这个模式不是很重要，感觉没讲好，后面查看了网上资料才知道
...
可能老师觉得比较基础所以没讲吧，感觉听的云里雾里的。</div>2020-07-03</li><br/><li><span>jaryoung</span> 👍（24） 💬（2）<div>游戏存档是不是很典型的例子？想当年，玩《勇者斗恶龙》的时候，打不过boss不断回到存档的地方，不断尝试。</div>2020-04-13</li><br/><li><span>Jackey</span> 👍（24） 💬（1）<div>想起了Redis主从同步的增量和全量模式</div>2020-04-13</li><br/><li><span>何用</span> 👍（11） 💬（7）<div>老师用词太过主观了，灌输访问者模式难理解的潜意识。我倒觉得 Single Dispatch 和 Double Dispatch 容易理解，反倒是今天的备忘模式难理解了。。。</div>2020-04-13</li><br/><li><span>Smile @ Life</span> 👍（10） 💬（2）<div>redis通过备份的形式进行持久化，分为两种模式
1、RDS：低频率全量备份
2、AOF: 高频率增量备份
而在AOF模式下通过配置auto-aof-rewrite来达到全量备份和增量备份相结合以减小AOF的备份大小
https:&#47;&#47;redis.io&#47;topics&#47;persistence</div>2020-07-31</li><br/><li><span>蹦哒</span> 👍（9） 💬（0）<div>理一下备忘录模式诞生的过程：
...
以上便诞生了备忘录模式：单独用一个新的类保存原来的数据（备份），原来对象提供新接口接收数据恢复（恢复）</div>2020-06-27</li><br/><li><span>忆水寒</span> 👍（8） 💬（0）<div>还有比如word写文章的时候，选择撤销。这也是一种备忘录模式的实现。可以参考这个文章https:&#47;&#47;mp.weixin.qq.com&#47;s&#47;J5a5ZcNCBBExpyCpU2o3wg</div>2020-04-22</li><br/><li><span>辣么大</span> 👍（5） 💬（5）<div>我的电脑重不关机！</div>2020-04-25</li><br/><li><span>...？</span> 👍（3） 💬（0）<div>以前做过白板软件，手写字的笔画支持撤销，重做功能，每写一笔就把前一步的画面存到撤销栈里，每次撤销一笔把当前画面存到重做栈，并从撤销栈pop一个画面，重做就从重做栈恢复一个画面，限定栈的大小，就可以支持撤销几次和重做几次，但是当时还没学过设计模式，傻乎乎的做全量保存，根本不考虑性能</div>2021-03-06</li><br/><li><span>Edward Lee</span> 👍（3） 💬（0）<div>备忘录模式实现关键是利用栈的数据结构

课后思考
JDBC rollback 和 save point rollback</div>2020-05-19</li><br/><li><span>djfhchdh</span> 👍（3） 💬（0）<div>浏览器的后退、前进功能，也可以看作是一种“备份”</div>2020-04-13</li><br/><li><span>守拙</span> 👍（3） 💬（0）<div>备份的应用场景:
1. 游戏有自动备份功能, 掉线&#47;下线再次上线时, 可以继续之前的游戏;
2. Android提供了Activity状态的备份&#47;恢复, 当App处于后台, Activity被系统回收前会调用onSavedInstanceState()备份, App切回前台Activity会重建, 可以通过onRestoreInstanceState()恢复之前的状态.</div>2020-04-13</li><br/>
</ul>