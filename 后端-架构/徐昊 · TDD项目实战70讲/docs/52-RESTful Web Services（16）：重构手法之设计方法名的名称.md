...
<li><span>aoe</span> 👍（2） 💬（0）<div>视频中的流式计算按功能封装方法，因为实现了单一原则，所以每个方法都能拥有一个名副其实的名字。看似简单的背后运转着大道规则（设计原则）。</div>2022-07-18</li><br/><li><span>Luke</span> 👍（0） 💬（0）<div>又踩了个坑：重构后的 result 方法里，给的 path 参数要带有 `&#47;` 斜杠。或者之后 mock 的时候，满足的 path 条件要一致。。。</div>2022-09-21</li><br/>
</ul>