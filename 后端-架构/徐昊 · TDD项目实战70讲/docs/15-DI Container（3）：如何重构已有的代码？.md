...
<li><span>小5</span> 👍（4） 💬（0）<div>学习了这节课的内容，想到了之前章节的一段话：
...
在做练习的重构过程中明白了因为对所有的代码都有测试，改了代码跑一下如果有错误就很容易发现，所以给了我们改代码的勇气。</div>2022-09-25</li><br/><li><span>escray</span> 👍（4） 💬（0）<div>进入到第二阶段的课程之后，给出的代码相对比较详细，虽然实现的功能比起前面要复杂一些，但是跟上节奏就不那么困难了。
...
本课的代码链接：https:&#47;&#47;github.com&#47;escray&#47;TDDCourse&#47;tree&#47;ch15</div>2022-04-28</li><br/><li><span>汗香</span> 👍（3） 💬（0）<div>获取一个依赖前，先从已有依赖获取，
    若获取结果不为空则获取成功，
    若获取结果为空，先将被获取依赖类型放入一个待创建集合中，并判断当前组件本身是否在集合中，若在，说明有循环依赖
</div>2022-04-25</li><br/><li><span>努力努力再努力</span> 👍（1） 💬（0）<div>哭了，上一个留言发现有问题，自己实现的时候才发现单纯一个 set 集合记录正在创建的bean还不行，得利用 TLS（https:&#47;&#47;time.geekbang.org&#47;column&#47;article&#47;93745） 模式做线程隔离，否则多线程的测试用例下，就会出现明明没有循环依赖而报错的问题。</div>2022-09-23</li><br/><li><span>努力努力再努力</span> 👍（1） 💬（0）<div>循环依赖，可以加一个 bean 在创建中的标识，当 bean 被创建完之后，就把标识去掉，这样如果创建过程中发现这个 bean 正在创建中，就可以认为是发生循环依赖了</div>2022-09-23</li><br/><li><span>蝴蝶</span> 👍（0） 💬（0）<div>@Inject注解的构造方法如果没有参数，那肯定是有问题的，这次看明白了。</div>2022-08-13</li><br/><li><span>davix</span> 👍（0） 💬（0）<div>get() 重構受啟發！雖然看過《重構》了解其中的方法，但平時如果get()設計變化我通常還是直接改。這回直觀感受引入get_()的好處。</div>2022-05-25</li><br/>
</ul>