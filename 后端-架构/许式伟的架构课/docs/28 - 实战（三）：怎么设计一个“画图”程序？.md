...
<li><span>风清扬</span> 👍（8） 💬（1）<div>老师，localid可以理解为服务端id，display id是客户端id吗？displayid记录客服端用户操作轨迹，每次同步后，如果用户修改，则display id变更为下一个。而localid始终不变，与服务端同步数据时，用的也是它。</div>2019-07-26</li><br/><li><span>Bing573</span> 👍（6） 💬（1）<div>我们最近在做一款可视化编辑器时，整个文档是采用的JSON格式存储的。由于没采用分层，所以每次有任何改动都需要保存整个文档。但程序在结构和操作上感觉比分层这种方式简单，对可视元素的操作反映为对JSON的操作也很直观，而且每次保存整个文档似乎也没什么不妥，所以不太理解为何需要采用分层的形式？</div>2019-10-14</li><br/><li><span>笛神</span> 👍（2） 💬（1）<div>希望老师可以将这几讲的内容用图形表达出来，分为哪几层，每一层职责如何，关系如何，这样理解起来比较形象一点</div>2019-09-10</li><br/><li><span>不温暖啊不纯良</span> 👍（1） 💬（1）<div>加入shapeId和documentId是为了局部更新吗?局部更新是以图层为单位更新的,每次浏览器刷新,这些被在本地缓存的图形就会重新绘制,达到保存的效果. 

我仔细看了v27的代码,dom.js里面就是对业务的定义.在view.js中看到调用事件.第一次见到js代码还能这么写,简直就像java里面的实体,的确很容易就理解业务轮廓,代码部分还没看完,但看老师写的代码,我长见识了.</div>2021-04-15</li><br/><li><span>剑八</span> 👍（1） 💬（1）<div>model层数据变化怎么通知view层？</div>2019-08-02</li><br/><li><span>路伴友行</span> 👍（1） 💬（1）<div>看了架构整洁之道这本书，不知道大佬会不会讲整洁架构</div>2019-07-26</li><br/><li><span>有铭</span> 👍（1） 💬（1）<div>localID倒是能理解，其实就是这个文件本身，一旦建立，就不动了，但是displayID会变化？每改一次就会变吗？那这东西是不是有点类似git等版本管理工具的提交版本号？是这个意思吗？也就是说其实你支持回退能力</div>2019-07-26</li><br/><li><span>瓜瓜</span> 👍（0） 💬（1）<div>func handleDefault(w http.ResponseWriter, req *http.Request) {
	if req.URL.Path == &quot;&#47;&quot; {
...
用断点调试也走到这个方法里面了，但是页面上还是显示404 page not found，我调试的是v27分之</div>2020-01-17</li><br/><li><span>瓜瓜</span> 👍（0） 💬（3）<div>GOROOT=D:\Go #gosetup
...
我在idea中启动后，为什么是报404 page not found 是哪里配置不正确吗？？</div>2020-01-17</li><br/><li><span>黄伟洪</span> 👍（14） 💬（0）<div>许先生的课，真是收益匪浅！</div>2019-07-28</li><br/><li><span>许童童</span> 👍（1） 💬（0）<div>老师，这一节读懂了。</div>2019-07-26</li><br/><li><span>ifelse</span> 👍（0） 💬（0）<div>学习打卡</div>2023-08-29</li><br/><li><span>Geek_e083ce</span> 👍（0） 💬（0）<div>感觉好乱抓不住头绪</div>2021-06-17</li><br/><li><span>瓜瓜</span> 👍（0） 💬（0）<div>谢谢老师，望老师解答</div>2020-01-17</li><br/><li><span>剑八</span> 👍（0） 💬（0）<div>数据驱动view层变化</div>2019-08-02</li><br/>
</ul>