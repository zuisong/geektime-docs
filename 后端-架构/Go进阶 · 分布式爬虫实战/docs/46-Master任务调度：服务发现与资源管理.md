...
<li><span>Realm</span> 👍（1） 💬（0）<div>1 “当 Master 成为新的 Leader 后，我们还要全量地获取一次 etcd 中当前最新的资源信息，并把它保存到内存中”，
当添加单个task任务时，使用事件监听；
事件监听机制在服务异常时可能丢信息？
望勋哥指点.
...
2 雪花算法生成的id有可能会出现重复。
如一个节点时，把服务器时钟回拨的情况；
多个节点时候，假如服务器的标志位一样，同一毫秒不同的节点可能产生的id相同；</div>2023-02-01</li><br/><li><span>胡军</span> 👍（0） 💬（0）<div>老师语速好快，都来不及思考和看文档😅</div>2023-02-02</li><br/>
</ul>