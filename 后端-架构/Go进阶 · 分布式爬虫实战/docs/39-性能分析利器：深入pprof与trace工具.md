...
<li><span>Realm</span> 👍（1） 💬（0）<div>思考题：
单次执行占用很高的cpu和内存的函数，不一定是瓶颈。

调用非常频繁的函数，并且每次需要分配内存或者造成软中断，也可能形成瓶颈。</div>2023-01-12</li><br/><li><span>公共账号</span> 👍（0） 💬（0）<div>pprof  top后为什么会有负数出现</div>2024-01-06</li><br/><li><span>加油</span> 👍（0） 💬（0）<div>火焰图 : 框越长、颜色越深，代表当前函数占用 CPU 的时间越久。
——————
这句话有问题哈，框越长，代表当前函数占用CPU的时间越久。但是和颜色无关，火焰图的颜色是随机的暖色调。</div>2023-05-18</li><br/><li><span>徐海浪</span> 👍（0） 💬（0）<div>内存分配多和CPU耗时长，只能说明这个函数占用的资源多，还需要结合执行次数分析，计算平均每次执行时间和内存分配的情况。</div>2023-01-17</li><br/>
</ul>