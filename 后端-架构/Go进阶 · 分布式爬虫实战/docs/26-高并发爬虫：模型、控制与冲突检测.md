...
<li><span>Geek_c16d38</span> 👍（0） 💬（2）<div>
func worker(tasksCh &lt;-chan int, wg *sync.WaitGroup) {
  defer wg.Done()
  for {
    task, ok := &lt;-tasksCh
...
這個go c錯了要修正</div>2022-12-23</li><br/><li><span>Realm</span> 👍（6） 💬（0）<div>数据需要传递的时候用channel；
数据不动的时候，如获取、修改状态，用锁；</div>2022-12-08</li><br/><li><span>konyo</span> 👍（0） 💬（0）<div>最后的例子好难懂啊</div>2023-01-11</li><br/><li><span>shuff1e</span> 👍（0） 💬（1）<div>func pool(wg *sync.WaitGroup, workers, tasks int) { tasksCh := make(chan int) for i := 0; i &lt; workers; i++ { go c(tasksCh, wg) } for i := 0; i &lt; tasks; i++ { tasksCh &lt;- i } close(tasksCh)}
