...
<li><span>有学识的兔子</span> 👍（10） 💬（1）<div>1. 我想这种方式可以利用脚本语言的高产的优势，例如老师说的python；
2. 把C++嵌入到脚本语言可以有效提升脚本语言的执行效率；而把脚本语言嵌入到C++，有点通过利用脚本语言特性的优势来弥补相对C++的弱项，使得C++开发变得更灵活和开放。</div>2020-06-14</li><br/><li><span>IT老兵重开始</span> 👍（5） 💬（2）<div>一个C++零基础的初学者，正在看一个C++开源库，这章很有启发，因工作需要改造一下开源库代码</div>2020-09-09</li><br/><li><span>нáпの゛</span> 👍（5） 💬（4）<div>老师，我复制C++ 的 string、tuple 和 vector 用于 Python 的示例代码，编译报错了，没识别到类型。
下面是部分错误信息：
...
pybind.cpp:25:9: error: ‘get’ was not declared in this scope
         get&lt;0&gt;(x)++;
         ^
pybind.cpp:25:9: note: suggested alternative:</div>2020-07-21</li><br/><li><span>reverse</span> 👍（3） 💬（2）<div>老师，能增加一部分内容讲讲 jsBridge吗 我正常的研发都要和nodejs打交道 但是经验不足</div>2020-06-15</li><br/><li><span>Stephen</span> 👍（2） 💬（1）<div>1.脚本语言实现业务比较方便,开发周期短,而C++作为底层库,运算效率高,相当于优势互补吧.
2.关于脚本语言嵌入到C++语言中,我特意查了下,网上说&quot;游戏开发中,如果要修改C++为主体的代码逻辑,使用Lua可以不重启服务器就能做到&quot;.我平常接触比较多的是C++作为底层库嵌入到脚本语言中.感觉各有千秋,适合不同的场景吧,说不出来哪个更好</div>2021-07-13</li><br/><li><span>无为而立</span> 👍（2） 💬（1）<div>之前用swig，再尝尝pybind11，感谢大佬提供思路</div>2020-06-13</li><br/><li><span>浑浑噩噩cium</span> 👍（2） 💬（1）<div>以前遇到过升级软件就是lua和c+＋配合，业务流程就是lua脚本里面控制，c＋＋调用lua脚本。</div>2020-06-13</li><br/><li><span>奋斗</span> 👍（1） 💬（1）<div>老师，请教您三个问题：
1、除了 python 和 lua ，  perl 可以吗？
2、有没有 pybind11 的教程资料？
3、如果学习 c++ 必须掌握一门脚本语言，那个是首选？ </div>2021-05-26</li><br/><li><span>易轻尘</span> 👍（1） 💬（1）<div>把 C++ 嵌入脚本语言：适用于脚本语言运行速度满或者和操作系统底层交互不方便的时候；
把脚本语言嵌入 C++：C++开发需要注意的点多，开发效率较低，可以使用脚本语言方便开发者</div>2021-04-14</li><br/><li><span>hao</span> 👍（1） 💬（3）<div>C++和python配合适合实现大型高并发高性能服务端吗？</div>2020-07-02</li><br/><li><span>f</span> 👍（1） 💬（1）<div>pybind11例子举错了吧，python定义的函数用python调??.</div>2020-06-16</li><br/><li><span>Eglinux</span> 👍（1） 💬（1）<div>老师，能录个视频大概讲一下 github 上 cpp_study 仓库里面的代码吗？</div>2020-06-13</li><br/><li><span>当初莫相识</span> 👍（0） 💬（1）<div>想问一下老师，如果是C++里调用Python，而py文件调用了第三方库，比如requests，那么应该如何集成到C++里呢？或者是怎样将Python环境带上？pybind11里对于C++嵌入Python到文档较少。</div>2024-08-26</li><br/><li><span>远逝的栀子花</span> 👍（0） 💬（1）<div>JS+NAPI+C++，OpenHarmony的开发架构，JS构建上层应用，C++提供底层功能。</div>2023-06-21</li><br/><li><span>piboye</span> 👍（0） 💬（2）<div>c++ 同步js，这个方向怎么没人尝试？</div>2022-09-07</li><br/>
</ul>