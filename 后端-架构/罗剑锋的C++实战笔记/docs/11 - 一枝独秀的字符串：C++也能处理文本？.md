...
<li><span>禾桃</span> 👍（21） 💬（3）<div>string_view 较string的优势，能举个具体例子吗？
我感觉string里也是字符串指针和长度（可以变的长度），这个看不出string_view优势在哪？string可以做哪些string_view做不了的事？

谢谢！
</div>2020-06-01</li><br/><li><span>1coma</span> 👍（12） 💬（1）<div>罗老师也打游戏吗？我听着尼尔的歌看着教程，还以为幻视了。。</div>2020-05-31</li><br/><li><span>HoshinoKanade</span> 👍（8） 💬（1）<div>老師一直在用linux，而我活在windows裡面一大堆wchar之中不能自救。</div>2020-05-31</li><br/><li><span>李莹</span> 👍（4） 💬（2）<div>using namespace std::literals::string_literals;
编译
no member named &#39;literals&#39; in namespace &#39;std&#39;

这个是怎么回事。</div>2020-06-14</li><br/><li><span>Weining Cao</span> 👍（4） 💬（1）<div>处理string的话，标准C++的接口实在不够友好，易用。反而第三方库QT的QString用起来就舒服，顺手很多。

在这方面，python语言的string处理比C++要友好太多。比如最简单的string内子串替换功能，python可以直接str. replace（a，b），但C++的replace函数需要先计算出替换string的起始位置，非常繁琐。</div>2020-05-30</li><br/><li><span>c1rew</span> 👍（3） 💬（2）<div>针对std::regex与boost::regex之间的性能差异，以下链接有相应的测试及对应的讨论，供大家参考，可以再根据自己项目的实际使用情况再验证下。
https:&#47;&#47;www.cnblogs.com&#47;bbqzsl&#47;p&#47;13714008.html
https:&#47;&#47;stackoverflow.com&#47;questions&#47;14205096&#47;c11-regex-slower-than-python</div>2021-12-05</li><br/><li><span>涤生</span> 👍（3） 💬（2）<div>处理文件名需要分割的时候，特别不方便，Python用一个split函数就够了</div>2020-10-12</li><br/><li><span>TC128</span> 👍（3） 💬（2）<div>老师，小结2说尽量不要用UNICODE，但如果用C++写界面（MFC、DirectUI），且软件又需要国际化，这种情况也尽量不用UNICODE吗？还是说换个语言写界面？</div>2020-05-30</li><br/><li><span>怪兽</span> 👍（2） 💬（1）<div>我很赞同老师说的，字符串在程序中的作用就像人对空气一样，所有我也比较看重。也希望老师对字符串转码方面给点建议，以及稍微讲解一下string_view的一些坑或者注意事项。</div>2020-06-21</li><br/><li><span>怪兽</span> 👍（1） 💬（1）<div>老师，我在VS2017测试了一下，为什么不使用命名空间也可以使用s后缀？
&#47;&#47; using namespace std::literals::string_literals;
auto str = &quot;hello&quot;s;</div>2020-07-14</li><br/><li><span>SometimesNever</span> 👍（1） 💬（1）<div>老师，我想问个之前的智能指针相关的，什么情况不能用智能指针代替裸指针呢？</div>2020-05-30</li><br/><li><span>学习者</span> 👍（0） 💬（1）<div>周末打开</div>2023-05-21</li><br/><li><span>Geek_6a1d96</span> 👍（0） 💬（1）<div>老师咨询一下,在写接口函数的时候,需要在不同库之间传递字符串参数，应该怎么写。直接传string和vector&lt;char&gt;风险极大只要不同的库之间STL的版本不一样必然会出现很多问题。我目前的做法参数使用const char* 返回字符串使用shared_ptr&lt;char&gt;，这种用法很不方便。特别是需要传入和返回多组字符串的时候，代码难看的简直生不如死。</div>2023-04-08</li><br/><li><span>江湖中人</span> 👍（0） 💬（2）<div>请教一下老师，windows c++想要写linux程序大概需要完善哪些知识呢？常年windows开发，linux c程序也维护过一点，但是一直没有完整学习和独立开发过，熟悉标准库还要重点补充哪些知识呢？</div>2023-02-18</li><br/><li><span>小一日一</span> 👍（0） 💬（1）<div>课外贴示中的第2点错误：
