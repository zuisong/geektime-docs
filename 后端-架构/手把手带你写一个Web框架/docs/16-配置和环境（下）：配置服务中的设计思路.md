...
<li><span>友</span> 👍（1） 💬（1）<div>那必须是可以实现的 我现在就实现</div>2021-12-10</li><br/><li><span>qinsi</span> 👍（1） 💬（1）<div>配置服务和app服务强耦合了，或许它们本来就是一体的？</div>2021-10-23</li><br/><li><span>jack</span> 👍（2） 💬（0）<div>感觉这个 config 文件夹里面过于复杂了， 两层配置简单些。区分出 development &#47; production &#47; testing 没太大必要，每个环境不同，由 .env 文件决定就可以了</div>2022-08-27</li><br/><li><span>牛玉富</span> 👍（0） 💬（0）<div>嗯嗯，热加载要的要的。
现在正式有良好的容器、目录等设计才能有后来更复杂的ddd设计。</div>2022-01-13</li><br/><li><span>Panmax</span> 👍（0） 💬（1）<div>password 是通过环境变量传入的，优先级高于配置文件，我认为不应该被配置文件覆盖掉。</div>2021-10-22</li><br/>
</ul>