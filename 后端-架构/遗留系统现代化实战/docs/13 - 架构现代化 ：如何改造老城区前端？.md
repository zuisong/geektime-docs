...
<li><span>子夜枯灯</span> 👍（0） 💬（2）<div>老师，对于后端可以引入单元测试或者自动化测试进行验证。但对于前端来说，很多的js或者fuction修改，这种如何验证修改后的效果没有搞坏呢？另外测试覆盖率怎么搞？谢谢</div>2022-05-10</li><br/><li><span>aoe</span> 👍（4） 💬（0）<div>庆幸目前不用做前端，但还是收藏一下这些资料
