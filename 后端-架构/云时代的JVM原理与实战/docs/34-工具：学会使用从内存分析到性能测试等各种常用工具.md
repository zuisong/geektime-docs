...
<li><span>on</span> 👍（1） 💬（0）<div>使用流式处理：如果可能的话，可以使用 Stream API 来处理查询结果，而不是使用传统的迭代方法。这样可以避免将整个结果集加载到内存中。

java stream api还有这个作用呢？不在内存中的话，那查出来的db数据在哪里？</div>2023-11-21</li><br/><li><span>on</span> 👍（0） 💬（0）<div>使用流式处理：如果可能的话，可以使用 Stream API 来处理查询结果，而不是使用传统的迭代方法。这样可以避免将整个结果集加载到内存中。</div>2023-11-21</li><br/><li><span>peter</span> 👍（0） 💬（1）<div>请教老师几个问题啊：
