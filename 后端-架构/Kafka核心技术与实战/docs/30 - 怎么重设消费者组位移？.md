...
<li><span>小可爱</span> 👍（39） 💬（10）<div>current是回到最近提交位移处，但是消费者不是本来就从最近提交处继续消费吗</div>2019-08-29</li><br/><li><span>Curry</span> 👍（18） 💬（2）<div>老师，为什么要poll（0）一下？是为了获取元数据吗？</div>2020-05-05</li><br/><li><span>喜欢地球的阿培同学</span> 👍（11） 💬（2）<div>老师，问一个问题: 
像RocketMQ这样的消息引擎，如果消费者消费某条消息一直失败，会将这条消息放到 “死信”队列里，然后消费者继续消费下一条消息。在kafka中，如果消费者消费某条消息一直失败，会怎么处理呢？难道程序会一直消费这条消息，然后失败.. 继续消费这条消息 .. 然后继续失败 ......</div>2020-05-27</li><br/><li><span>水天一色</span> 👍（9） 💬（2）<div>请问，重置offset到 datetime，这个 datetime 是生产时间还是当前group的消费时间？</div>2019-12-17</li><br/><li><span>cricket1981</span> 👍（8） 💬（5）<div>&quot;最重要的是，一定要调用带长整型的 poll 方法，而不要调用 consumer.poll(Duration.ofSecond(0))。&quot;--- 能讲下为什么吗？如果不遵守会怎么样？</div>2019-08-12</li><br/><li><span>Li Shunduo</span> 👍（7） 💬（1）<div>试了下开着console consumer的时候去调整offset,遇到以下错误:
...
好像不能动态调整?</div>2019-08-12</li><br/><li><span>What for</span> 👍（5） 💬（1）<div>请问一下老师在重设位移之前为什么要调用 consumer.poll 方法？是为了连接集群吗？</div>2019-08-31</li><br/><li><span>JasonZhi</span> 👍（4） 💬（1）<div>老师，不是还可以通过auto offset reset配置项重设位移吗？怎么这里没有说</div>2019-08-30</li><br/><li><span>James</span> 👍（3） 💬（2）<div>老师,你好.存在以下问题,麻烦解答下:
1.对文中解释的Latest与Current感觉区分不清楚，老能能详细举例介绍下;
文中是禁止提交位移,那么这两个应该都是一样,最新的位移位置.
要是可以提交位移呢;不是特别懂
2.第一条评论中,current主要是为了调试场景,要是有提交位移(50-&gt;100),那么重新上线使用current策略,最新提交位移是不是100;</div>2020-07-02</li><br/><li><span>Curry</span> 👍（3） 💬（3）<div>请问如何不停止程序去指定位移呢？</div>2020-05-05</li><br/><li><span>helloworld</span> 👍（3） 💬（2）<div>这里的--all-topics是什么含义呢？为什么Shift-By-N、DateTime与Duration均没有？是不是可以这么认为：通过脚本的方式设置位移只能设置Topic的所有位移，并且所设置的位移会使各个分区都一样？而不能单独设置某个分区的位移？？</div>2020-03-16</li><br/><li><span>无菇朋友</span> 👍（3） 💬（1）<div>老师 问一下 如果 我想针对某个分区重置位移，怎么做</div>2019-09-18</li><br/><li><span>无菇朋友</span> 👍（3） 💬（1）<div>请问老师，current这个选项的应用场景是什么？</div>2019-08-26</li><br/><li><span>锋芒</span> 👍（3） 💬（1）<div>请问，用命令行重设位移，应该在当前group 的leader 节点上？</div>2019-08-23</li><br/><li><span>chp</span> 👍（2） 💬（1）<div>老师，为啥我指定消费者位移没效果
-------------------------------------------------
String topic = &quot;foo&quot;;
        try (final KafkaConsumer&lt;String, String&gt; consumer =
                      new KafkaConsumer&lt;&gt;(consumerProperties)) {
...
----------------------------------------------------------
没报错，就是设置了位移后，offset没变化</div>2020-03-01</li><br/>
</ul>