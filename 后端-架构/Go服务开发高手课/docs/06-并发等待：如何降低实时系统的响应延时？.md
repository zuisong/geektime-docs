...
<li><span>lJ</span> 👍（2） 💬（1）<div>可以基于 WaitGroup 和一个错误通道（error channel，容量不小于控制的协程并发数，避免阻塞）来收集并发协程中的错误信息，任务协程发生的错误写入err channel中，开启一个协程读取err channel。也可以初始化一个全局的错误切片errs，在协程出错时将err赋值给errs。
...
本质上都是通过err切片保存任务协程中的错误信息</div>2024-12-20</li><br/><li><span>橙</span> 👍（1） 💬（1）<div>func TestCancel(t *testing.T) {
    results := make([]string, len(urls))
    &#47;&#47; 用WithContext函数创建Group对象
    eg, ctx := errgroup.WithContext(context.Background())
    for index, url := range urls {
