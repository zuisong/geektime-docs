...
<li><span>老巫</span> 👍（12） 💬（1）<div>刚开始搭建玩玩，我觉得consul可以，它首页的KV模块就是做配置的吧。

大力推荐携程的配置中心，好用简单！</div>2018-10-11</li><br/><li><span>batman</span> 👍（4） 💬（1）<div>你好，怎么定义中小团队？</div>2018-10-18</li><br/><li><span>Douglas</span> 👍（4） 💬（1）<div>胡老师，注册中心配置修改后，应用需要重新加载的吧，比去数据库线程池，一般在应用启动后就会创建好指定数量的线程，如果通过配置中心，降低了数量，多余的线程该如何处理呢？</div>2018-10-11</li><br/><li><span>楼下小黑哥</span> 👍（12） 💬（0）<div>在前面我讲到 Zookeeper、Consul、etcd 作为服务的注册中心时，可以提供强一致性的服务发现功能，那么它们能够作为配置中心吗？为什么
...
</div>2018-10-22</li><br/><li><span>Zerone</span> 👍（5） 💬（0）<div>比如调用下面的接口请求就会返回配置项 global.property 中 Key 为 reload.locations 的配置值。
curl &quot;http:&#47;&#47;ip:port&#47;config&#47;service？action=getSign&amp;group=global.property&quot;

不是返回配置值，是返回sign 值吧
</div>2019-03-28</li><br/><li><span>Liam</span> 👍（3） 💬（0）<div>consul就可以作为配置中心，不过配置要分成启动时依赖和非启动依赖，前者是不可动态修改的</div>2018-10-23</li><br/><li><span>菠萝吹雪—Code</span> 👍（2） 💬（0）<div>我的理解：Zookeeper、Consul、etcd 这三个应该是 强一致性的，属于CP。一个节点不可用，会造成这个集群的不可用，这在系统中是不能接受的
</div>2021-07-01</li><br/><li><span>熊猫酒仙</span> 👍（1） 💬（0）<div>zk直接作为配置中心，各服务端需要比较多的适配吧，毕竟zk的通知变更不能太依赖了，这样一来对业务侵入性大了一点</div>2018-10-12</li><br/><li><span>XXL</span> 👍（1） 💬（0）<div>我司就是用的zk作为配置中心的，不晓得有什么不合适的...</div>2018-10-11</li><br/><li><span>6点无痛早起学习的和尚</span> 👍（0） 💬（0）<div>我司用的配置中心原理应该是每次发布的时候会推到机器本地文件里，代码每次读取本地问题，当然在服务启动的时候会强校验代码里需要的配置文件是否存在，如果不存在，就会启动失败</div>2024-05-13</li><br/><li><span>王维</span> 👍（0） 💬（0）<div>现在是2023年，新的配置中心有nacos了</div>2023-03-03</li><br/><li><span>jacky</span> 👍（0） 💬（0）<div>目前spring cloud config-server可通过mintor进行变更监控，再结合总线刷新消息通知机制实现配置变化自动通知</div>2022-10-28</li><br/><li><span>俯瞰风景.</span> 👍（0） 💬（0）<div>分布式系统中的每一个服务子系统的配置项有可能不一样，如果每个子系统单独管理配置会很麻烦，所以需要引入配置中心进行统一管理，提高运维效率。

配置中心要保证每个服务消费者能够及时拉取最新的配置信息，也要保证能够方便地进行配置更新。
而通过统一的配置中心，就可以对微服务系统集群进行统一动态调度，比如：动态熔断、动态降级、动态超时重试等等。</div>2021-10-08</li><br/><li><span>惘 闻</span> 👍（0） 💬（2）<div>老师服务降级和熔断听起来是一个意思呀?他们区别是什么啊</div>2021-01-28</li><br/><li><span>Joyce</span> 👍（0） 💬（0）<div>helm这个呢</div>2019-11-05</li><br/>
</ul>