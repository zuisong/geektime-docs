...
</div>2023-02-04</li><br/><li><span>Jaising</span> 👍（13） 💬（3）<div>还在犹豫是否要翻过墙的时候，就先把帽子丢到墙的另一边，剩下的自然会有办法。
...
祝各位兔年快乐！</div>2023-01-20</li><br/><li><span>bin</span> 👍（7） 💬（1）<div>handler如果不依赖数据库，这部分业务逻辑好像也可以放到领域对象里面。有没有一个好的边界区分？</div>2022-12-31</li><br/><li><span>aoe</span> 👍（4） 💬（2）<div>原来我现在的编程风格是偏过程式的

思考题
1. 可以。这样确实可以增强封装。再疯狂一点，可以使用 Java 模块化技术，让反射都无法打破封装。
2. 没发现不应该有 getter 的属性。</div>2023-01-03</li><br/><li><span>H·H</span> 👍（2） 💬（4）<div>领域对象是否可以通过依赖注入的方式注入 仓储Repository？</div>2023-01-30</li><br/><li><span>6点无痛早起学习的和尚</span> 👍（2） 💬（1）<div>一些问题：
5. 为什么在 addOrg 加事务？里面就一个 save 操作，是否没必要加@Transactional
6. 很多用@Autowired 注入的属性，有些是 final，有些不是 final，这个有什么讲究吗？</div>2023-01-17</li><br/><li><span>Breakthrough</span> 👍（1） 💬（1）<div>钟老师，DDD设计完成后的交付文档一般长啥样，是不是也要做类似设计需求说明书的WORD档。如果是，是否有相关的模板参考下？</div>2023-07-17</li><br/><li><span>龙腾</span> 👍（1） 💬（1）<div>老师请教一下，“第三，应用服务可以读写数据库。”这块没太明白，什么情况下读写数据库不包含领域知识呢？咱们的数据库设计不就是按照领域来划分设计的吗，只要读写都是在领域内进行的吧？</div>2023-01-02</li><br/><li><span>开心</span> 👍（0） 💬（1）<div>看到这里，没有感受到太多面向对象，更多是面向过程的实现。领域业务大都是放在了没有属性只有方法的领域服务，更多的是如何定义领域服务模块。有点不解。</div>2024-04-25</li><br/><li><span>路上</span> 👍（0） 💬（1）<div>类似撤销这部分校验逻辑，目前看领域对象自己就可以校验，目前是在外层进行的校验之后调用领域方法进行状态变更
是否可以在领域对象变更状态的方法内部自己进行校验呢</div>2023-12-15</li><br/><li><span>末日，成欢</span> 👍（0） 💬（1）<div>老师，有一些疑问。
第一，领域对象不访问数据库。
...
目前代码看到这里,还是偏向过程化, 只是利用了一些软件的设计原则(比如solid),代码更业务化, 还没理解有啥好处</div>2023-06-13</li><br/><li><span>🏄🏻米兰的大铁匠🎤🎈</span> 👍（0） 💬（1）<div>请问老师，&quot;只有有效的组织才能被撤销&quot;这段校验逻辑是否可以更进一步的内聚到org对象本身的cancel()方法中，这样外部触发&quot;撤销&quot;动作时，只需调用一个方法就行了</div>2023-05-28</li><br/><li><span>marker</span> 👍（0） 💬（1）<div>老师，有一个问题就是领域服务，比如下单需要验证商品一系列的验证、但是商品和订单在两个bc下、会去做rpc操作、这个应该放在哪里合适</div>2023-05-20</li><br/><li><span>Hesher</span> 👍（0） 💬（1）<div>回答一下课后问题：
1. 一般直接用私有，避免被外部访问，即使是包内，封装性一步到位；
2. 审计字段一般不会被业务使用，可以没有getter，其他字段看情况。</div>2023-04-07</li><br/><li><span>Geek_e10c1b</span> 👍（0） 💬（1）<div> 老师，领域对象中不访问数据库，那如何对某些属性进行懒加载呢</div>2023-03-28</li><br/>
</ul>