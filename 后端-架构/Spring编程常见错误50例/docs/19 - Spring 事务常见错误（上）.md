...
<li><span>辰砂</span> 👍（31） 💬（0）<div>@Transactional(rollbackFor = Exception.class, noRollbackFor = RuntimeException.class)</div>2021-06-04</li><br/><li><span>study的程序员</span> 👍（2） 💬（0）<div>事务层面默认 publicMethodsOnly=true,所以private或者protected都不生效
...
之后protected生效,private不生效,aop不能代理private方法&#47;final方法,所以不调用target的方法,所以spring注入的对象为null,所以会报NPE,studentMapper为空</div>2022-11-18</li><br/><li><span>陈汤姆</span> 👍（2） 💬（4）<div>学到了，一直以为注解只能对public生效是因为动态代理的原因！</div>2021-06-15</li><br/><li><span>LkS</span> 👍（2） 💬（0）<div>可以使用noRollbackFor
@Transactional(rollbackFor = Exception.class,noRollbackFor = RuntimeException.class)</div>2021-06-04</li><br/><li><span>安迪密恩</span> 👍（1） 💬（0）<div>我觉得可以把注解加在 saveStudent 这个方法上，要更自然一些。</div>2022-03-11</li><br/><li><span>qchang</span> 👍（1） 💬（0）<div>思考题：一种是try-catch判断异常类型后，非RuntimeException抛出；另一种可以采用注解@Transactional(noRollbackFor = RuntimeException.class)</div>2021-06-04</li><br/><li><span>小林桑</span> 👍（0） 💬（0）<div>1.Spring事务默认只有runtimeexception和error才会回滚；但是可以通过transactional注解指定回滚的异常类型。
2.对私有方法加transactional注解事务不会回滚。</div>2024-01-19</li><br/><li><span>番茄</span> 👍（0） 💬（0）<div>讲的非常清晰</div>2023-04-24</li><br/><li><span>雨落～紫竹</span> 👍（0） 💬（0）<div>其实就分2类 一类配置问题 一类代理问题</div>2022-06-22</li><br/><li><span>Monday</span> 👍（0） 💬（0）<div>案例2中，进入doSaveStudent()方法后，抛NPE（studentMapper==null 为true）</div>2021-07-19</li><br/><li><span>一记妙蛙直拳</span> 👍（0） 💬（0）<div>@Transactional(rollbackFor = Exception.class, noRollbackFor = RuntimeException.class)</div>2021-06-04</li><br/>
</ul>