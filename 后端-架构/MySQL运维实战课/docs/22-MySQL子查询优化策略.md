...
第二种改写方式对应半连接转换中的 Duplicate Weedout 策略，使用到了内存临时表，并用主键来去重。</div>2024-10-12</li><br/><li><span>陈星宇(2.11)</span> 👍（0） 💬（1）<div>这些策略分别是 pullout、duplicate weedout、first match、loose scan、materialization。 这些是8.0才有吗？在5.7里很少看到。</div>2024-10-11</li><br/><li><span>陈星宇(2.11)</span> 👍（0） 💬（1）<div>mysql&gt; explain select item_id, sum(sold) as sold 
      from stat_item_detail 
      where item_id in (
           select item_id 
           from stat_item_detail 
