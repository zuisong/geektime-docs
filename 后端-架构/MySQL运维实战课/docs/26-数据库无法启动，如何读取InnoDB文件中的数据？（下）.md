...
<li><span>binzhang</span> 👍（1） 💬（1）<div>为啥在二级索引的root和branch page上要存储主键列的值？ 感觉没必要啊</div>2024-10-21</li><br/><li><span>叶明</span> 👍（0） 💬（1）<div>我觉得和系统表空间 ibdata1 有关系，系统表 mysql.indexes 在实例初始化时生成，其元数据信息（如根页面编号）可能被存储在系统表空间里，Innodb 在启动时可以直接访问这些位置，而不需要依赖数据字典。</div>2024-10-21</li><br/>
</ul>