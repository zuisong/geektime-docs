...
<li><span>廉思夏</span> 👍（0） 💬（1）<div>qps应该每秒的吞吐量吧，但是这里面的限流提到的qps是当前时刻在dubbo方法中未结束的请求数</div>2023-09-14</li><br/><li><span>yYe</span> 👍（0） 💬（1）<div>&#47;&#47; 若当前的计数值大于或等于已配置的限流值的话，那么返回 false 表示无法获取计数资源 if (currentCount.get() &gt;= qpsValue) { return false; } 这个代码没有并发的问题吗，如果两个线程进去都是 &gt; qpsValue, 就像多卖是的</div>2023-08-20</li><br/><li><span>Geek_090fe2</span> 👍（0） 💬（1）<div>限流没有时间区间吗
