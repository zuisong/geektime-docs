...
<li><span>小白</span> 👍（3） 💬（1）<div>老师，还有一个问题，为什么不直接用RpcContext进行get 和 set 传递呢？为什么还要涉及到invocation，这块不是很理解。</div>2022-12-27</li><br/><li><span>张三丰</span> 👍（2） 💬（4）<div>感觉文中获取traceid的地方有问题，不应该在提供者处生成，应该在消费端生成是traceid，再把traceid传给提供者，如果提供者拿到了traceid就打印出来，如果拿不到再生成traceid返回给消费者</div>2023-01-03</li><br/><li><span>高级按摩师 👁 ^ 👁⃢*</span> 👍（1） 💬（1）<div>RPCConext是怎么传递的，服务之间调用，上下文怎么传递的呢</div>2023-09-11</li><br/><li><span>Lum</span> 👍（1） 💬（1）<div>请问一下 CompleteableFuture那些并行计算的意义是什么呢？项目中比如从多个系统中取数据，一般我都用了5,6个CompletableFuture，然后直接每个都get了。。。  没怎么用到上面的那些api</div>2023-02-26</li><br/><li><span>王巍</span> 👍（1） 💬（1）<div>多线程的情况下，线程也能获取到正确的 traceId 吗？</div>2022-12-23</li><br/><li><span>乌凌先森</span> 👍（0） 💬（2）<div>老师你好，@DubboService + @Component 这种使用方式有啥好处？</div>2023-01-19</li><br/><li><span>Geek_10086</span> 👍（0） 💬（1）<div>老师您好，traceId是不是应该在消费者端（ReqNoConsumerFilter）生成，通过隐式传递到服务提供端（ReqNoProviderFilter），文中代码在ReqNoConsumerFilter中从上下文获取应该是null吧</div>2022-12-24</li><br/>
</ul>