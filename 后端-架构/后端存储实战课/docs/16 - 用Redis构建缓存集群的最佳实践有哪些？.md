...
<li><span>李玥</span> 👍（49） 💬（4）<div>Hi，我是李玥。

这里回顾一下上节课的思考题：

课后请你想一下，把订单表拆分之后，那些和订单有外键关联的表，该怎么处理？

对于这些表，我的建议是，和订单表一起拆分，让相同订单ID的订单和关联表的数据分布到相同的分片上，这样便于查询。</div>2020-04-03</li><br/><li><span>LVM_23</span> 👍（54） 💬（1）<div>老师你好，想问个问题。redis的list内元素的单独过期怎么做，而不是整个list过期。 网上都是用时间戳存，然后用定时任务来清除。有其他的方案吗? 写入频繁的，谢谢老师解答了</div>2020-05-10</li><br/><li><span>发条橙子 。</span> 👍（19） 💬（3）<div>老师 有两个疑问点

1.  大厂自建集群 是不是就是常说的使用一致性哈希来做槽的映射

2. 既然自带集群包含了哨兵， 代理也包含 ，是不是哨兵的功能也是在自建中使用 ，还是大厂自建的也不用哨兵 是自己做的高可用</div>2020-04-03</li><br/><li><span>特种流氓</span> 👍（14） 💬（1）<div>redis cluster集群 是不是就没有哨兵的概念了</div>2020-04-02</li><br/><li><span>正在减肥的胖籽。</span> 👍（11） 💬（1）<div>请教老师一个问题：
 1.redis集群新增分片后，线上怎么实现好的平滑迁移？这个一直没有想到好的解决方法</div>2020-04-02</li><br/><li><span>申学晋</span> 👍（7） 💬（1）<div>redis6.0的官方集群代理和客户端缓存是不是可以把您讲的两种方案结合使用？</div>2020-05-15</li><br/><li><span>暴君•熊</span> 👍（6） 💬（1）<div>老师，最后介绍的定制客户端模式来保存分片表。那redis的服务端不需要动嘛？当有新增或者删除节点的时候，服务端内部不还是需要使用流言来传播一次嘛？</div>2020-04-20</li><br/><li><span>喜欢地球的阿培同学</span> 👍（3） 💬（1）<div>老师，您好，问一个问题。最后介绍的定制客户端模式。是不是这些redis是一个一个节点，并没有一起组成一个集群。如果这些redis组成一个集群的话，那岂不是每次新增和删除redis节点时，也会流言传播一次吗？</div>2020-05-08</li><br/><li><span>haijian.yang</span> 👍（8） 💬（0）<div>阿里云有个 redis 方案，冷数据存磁盘，热数据放在内存。</div>2020-04-02</li><br/><li><span>sami</span> 👍（3） 💬（0）<div>cluster命令是受限的，跟mysql的分库分表一样，有一些场景也是无法支持</div>2020-04-02</li><br/><li><span>千锤百炼领悟之极限</span> 👍（1） 💬（0）<div>小规模集群适合使用Redis Cluster，适合几个到几十个节点的规模。

为什么不适合大规模的集群? 因为Redis Cluster使用了一种去中心化设计的Gossip（流言）协议，用于节点之间更新映射表。

如果集群的节点数量多，更新映射表速度较慢，导致数据同步等问题。</div>2022-03-03</li><br/><li><span>椿</span> 👍（1） 💬（0）<div>最后一种集群方案，感觉有点类似于sharding sphere这样的中间件，客户端读取元数据然后路由到具体的redis实例请求</div>2020-05-23</li><br/><li><span>leslie</span> 👍（1） 💬（1）<div>其实这也反向提出了另一个问题mysql cluster的问题：其自身同样有cluster,可是真实环境下很少企业会去用mysql cluster；都是各自运用自己的方案去做集群，方案之前的课程中都有提及就不提了。
自身的cluster在实际生产中极少被某些数据库使用这大概算是一大特点，尤其像我们所常见的mysql、redis、以及后面新出来的、、、</div>2020-04-02</li><br/><li><span>ifelse</span> 👍（0） 💬（0）<div>还有一个小问题需要注意的是，这几种集群方案对一些类似于“KEYS”这类的多 KEY 命令，都没法做到百分百支持。--记下来</div>2022-12-12</li><br/><li><span>ifelse</span> 👍（0） 💬（0）<div>学习打卡</div>2022-12-12</li><br/>
</ul>