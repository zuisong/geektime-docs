...
<li><span>写点啥呢</span> 👍（13） 💬（5）<div>请教老师，对于Lease操作，请求是否必须有Leader接收处理。这种写请求路由是通过client3客户端直接发到leader还是通过可以通过follower转发？

谢谢</div>2021-02-01</li><br/><li><span>kaizen</span> 👍（10） 💬（7）<div>您好，有个疑问，关于这句话:&quot;一方面不同 key 若 TTL 相同，可复用同一个 Lease， 显著减少了 Lease 数。&quot;
这句话其实不太理解，相同TTL的k-v可以用同一个lease,但lease过期，会删除所有k-v，是不是这里虽然是多个k-v,且TTL相同，但其实他们是有事务关系的，既要么都可用，要么都过期，并不是因为TTL相同而放在一个lease下面呢</div>2021-02-01</li><br/><li><span>mmm</span> 👍（9） 💬（3）<div>老师，您好，我有几个概念理解的不知是否准确：
...
4、如果同步的是剩余时间而不是到期时间戳，那这么设计是为了要保证Follower和Leader时钟不一致时也要正确处理租约过期吗？etcd对Leader和Follower的时钟是否有同步的要求，不同步会有什么问题吗？</div>2021-02-01</li><br/><li><span>奕</span> 👍（6） 💬（3）<div>lease 代表 过期的一个 ttl ，多个 key 复用一个 lease 的时候，lease 是不是没有办法保存每个 key 的具体过期时间点是多少，因为每个 key 的创建时间不一样，所以过期时间也不一样。

还有就是当多个 key 复用同一个 lease 的时候， 某个客户端再发送  keepalive 请求的时候，是可以直接修改lease 的剩余 ttl吗？ 若能修改的话，不就把关联到该 lease 上所有 key 的 ttl 都修改了？</div>2021-02-03</li><br/><li><span>Geek_5a8405</span> 👍（3） 💬（1）<div>续期操作不通过raft协议同步到follower，那如果读带lease的key是不是得经过leader处理？因为只有leader的lease过期时间是最准确的（虽然会定时checkpoint同步ttl到follower，但是我理解这个不是非常准确到）。</div>2021-02-15</li><br/><li><span>Fone</span> 👍（0） 💬（1）<div>可以手动执行etcdctl命令让lease续期吗？</div>2021-09-06</li><br/><li><span>花晨少年</span> 👍（0） 💬（1）<div>如果只是因为旧版本，才会导致重建，而使得lease永远不会过期，而使用checkpoint机制可以解决这个问题。那新版本就没必要用checkpoint机制了吧？</div>2021-04-17</li><br/><li><span>田奇</span> 👍（0） 💬（1）<div>1.lease 过期以后，删除关联的 key，这个删除动作也只能 leader 触发吗？也就是说 Follower 的 lessor 模块其实不会运行？

2. 我能理解put key 这种写操作有一致性日志复制，但是 del key的这种数据同步，看文中没有描述走 raft 协议，他是直接发送 lease 过期给所有的 Follower，然后Follower自己处理删除数据，那这个删除的key应该也得记录日志吧，不然状态机恢复没法恢复，这块比较模糊疑惑</div>2021-03-24</li><br/><li><span>mckee</span> 👍（7） 💬（1）<div>思考题：etcd lease 最小的 TTL 是多少？
可能为了确保发生leader选举时，lease不会过期，最小ttl应该比选举时间长，看代码
minTTL := time.Duration((3*cfg.ElectionTicks)&#47;2) * heartbeat
minTTLSec := int64(math.Ceil(minTTL.Seconds()))
默认的情况下应该是2s，
</div>2021-02-03</li><br/><li><span>lewis</span> 👍（2） 💬（2）<div>请教老师，lease的续期操作不会经过raft模块，follower是怎么知道lease的具体的过期时间内的呢？ 请求lease的过期时间 timetolive 命令也是由leader执行吗？</div>2021-07-30</li><br/><li><span>gopherliu</span> 👍（1） 💬（0）<div>golang etcd clientv3，如果KeepAlive返回的channel因为etcd集群异常（大约30min）close了。当etcd集群正常后使用原有的leaseID续租可以吗？还是需要重新grant？</div>2021-09-14</li><br/><li><span>哈登</span> 👍（0） 💬（0）<div>etcd lease 最小的 TTL 时间，minTTL := time.Duration((3*cfg.ElectionTicks)&#47;2) * heartbeat</div>2025-01-22</li><br/><li><span>好学boy</span> 👍（0） 💬（0）<div>老师，我有个非常烦恼的问题，求回答：
etcd的集群，一个节点的系统盘性能差时，etcd的lease为什么会丢失租约？源码里应该是clinet请求到服务端，然后操作的内存的lease，但是网络没有问题，为什么会租约超时呢？</div>2023-04-27</li><br/><li><span>molamola</span> 👍（0） 💬（0）<div>老师，您好，有个问题请教下。
我是使用etcd选主时，偶现续租失败的情况，etcd日志显示&quot;msg&quot;: “failed to apply request.” &quot;error&quot;: &quot;lease not found&quot;. 请问下遇到过这种情况么</div>2022-06-02</li><br/><li><span>po</span> 👍（0） 💬（0）<div>想kubernetes的node节点挂了，那么对应的lease过期后node节点应该就从etcd中删除了？</div>2022-03-07</li><br/>
</ul>