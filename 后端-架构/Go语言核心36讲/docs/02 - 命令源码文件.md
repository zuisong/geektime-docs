...
<li><span>姜雨生</span> 👍（32） 💬（0）<div>flag的讲解很棒，通过这个命令，我们就可以控制程序在不同环境的执行内容了，通过控制参数设置更多的内容！</div>2019-02-28</li><br/><li><span>梦里追逐</span> 👍（7） 💬（1）<div>咱们用的都是哪个IDE？</div>2018-08-15</li><br/><li><span>吉祥</span> 👍（5） 💬（2）<div>undefined: os 怎么回事</div>2018-08-13</li><br/><li><span>传说中的成大大</span> 👍（4） 💬（2）<div>不太明白的地方在于 flag.stringvar 和flag.parse的作用 前面调用了flag.stringvar并且已经传入了name地址 为什么还要再调用一次flag.parse？</div>2020-02-10</li><br/><li><span>弥撒</span> 👍（4） 💬（1）<div>老师好，在执行demo2.go程序运行的时候，我打印了一下init()方法和main()方法的执行顺序，发现init方法先与flag.Parse()这句话之前就运行了，这是什么原因，程序入口不是main方法吗？</div>2019-05-25</li><br/><li><span>mayunian</span> 👍（4） 💬（2）<div>老师，今天试了一下类型转换。
为什么转换var x uint = uint(-1) 的时候会报错？
而var y int = -1
var x uint = uint(y)就不会报错呢？</div>2018-08-30</li><br/><li><span>NEVER SETTLE</span> 👍（2） 💬（1）<div>老师，package name 是不能使用下划线吗，我在用vscode时候，go-lint插件会报这个警告。</div>2019-04-17</li><br/><li><span>但爱</span> 👍（2） 💬（1）<div>老师帮忙解答一下注释中的问题：
package main
...
}</div>2019-02-22</li><br/><li><span>王聪 Claire</span> 👍（1） 💬（1）<div>请问linux下有什么好用的go语言的IDE推荐吗？谢谢。</div>2019-04-25</li><br/><li><span>小样</span> 👍（1） 💬（2）<div>??init函数是什么，没地方调用啊。是系统的默认的？</div>2019-03-21</li><br/><li><span>郁柏</span> 👍（1） 💬（1）<div>执行 go get -v github.com&#47;sqs&#47;goreturns 失败。 
请求连接超时。 请问怎么办。</div>2018-08-30</li><br/><li><span>金阳</span> 👍（1） 💬（1）<div>想请教一下老师，这种go 语言自带的api哪里可以查看</div>2018-08-16</li><br/><li><span>Geek_9a13d0</span> 👍（0） 💬（1）<div>新建了两个文件 分别是dome1.go 和dome2.go 两个文件代码都没问题。但是不删除其中一个的话。两个文件都显示 func main（） 中的main 报错。请问老师是什么原因</div>2022-12-10</li><br/><li><span>Geek_9a13d0</span> 👍（0） 💬（2）<div>小白想问。配置好环境变量 为什么打不开go 是要再安装什么嘛</div>2022-12-08</li><br/><li><span>微微超级丹💫</span> 👍（0） 💬（1）<div>1、默认情况下，我们可以让命令源码文件接受哪些类型的参数值？
可以接受 string、float64、bool、time.Duration、Int64、Int、String、Text、Uint64、Uint
2、我们可以把自定义的数据类型作为参数值的类型吗？如果可以，怎样做？
可以使用自定义的数据类型，使用flag.Val方法，自定义类型，必须实现Value接口，该接口定义了String和Set方法</div>2022-11-12</li><br/>
</ul>