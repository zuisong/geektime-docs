...
<li><span>Marvichov</span> 👍（1） 💬（0）<div>`func Decorator(decoPtr, fn interface{}) (err error)` 里的err没有被调用...当`Elem()` return nil的时候, 可以assign err</div>2021-05-25</li><br/><li><span>紫陌桑田</span> 👍（0） 💬（0）<div>不建议使用反射，不仅是性能，更要考虑代码的健壮性，多写几个 type func 也要不了多少时间。</div>2024-07-12</li><br/><li><span>特立独行的猪</span> 👍（0） 💬（0）<div>反射看的还是挺懵的，不过现在有泛型了，实现起来简单多了</div>2024-03-10</li><br/>
</ul>