...
<li><span>limix</span> 👍（5） 💬（0）<div>控制逻辑的特征是可复用性比较高，多场景可用，而业务逻辑的特征是专用性，控制逻辑，应该复用控制逻辑，而不是复用也许逻辑</div>2022-09-07</li><br/><li><span>debugtalk</span> 👍（4） 💬（0）<div>手动点赞</div>2021-06-12</li><br/><li><span>cvvz</span> 👍（1） 💬（0）<div>反转控制：
第一种改写方法——继承，好处是原有代码可以复用，控制逻辑和业务逻辑解耦，坏处是通用的控制逻辑无法复用
第二种改写方法——反转控制，好处是复用通用的控制逻辑，坏处是要修改原有代码的逻辑，把控制逻辑嵌入到了业务逻辑中</div>2022-08-27</li><br/><li><span>Geek_ce6971</span> 👍（0） 💬（1）<div>实现undo功能，有个地方是写反了吗
在 Override 的 Add()中，记录 Delete 操作；在 Override 的 Delete() 中，记录 Add 操作；</div>2022-01-19</li><br/><li><span>方勇(gopher)</span> 👍（0） 💬（0）<div>日常中有些装饰器其实可以用这种方式替换</div>2021-12-20</li><br/><li><span>衡子</span> 👍（0） 💬（0）<div>厉害了👍</div>2021-11-30</li><br/><li><span>一光年</span> 👍（0） 💬（0）<div>控制逻辑依赖业务逻辑，不如让业务容器依赖控制逻辑</div>2021-08-11</li><br/><li><span>Geek_46da16</span> 👍（0） 💬（0）<div>会玩，</div>2021-07-26</li><br/>
</ul>