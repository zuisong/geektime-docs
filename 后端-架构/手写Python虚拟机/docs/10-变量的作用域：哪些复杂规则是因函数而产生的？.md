...
<li><span>ifelse</span> 👍（0） 💬（0）<div>学习打卡</div>2024-10-25</li><br/><li><span>Geek_66a783</span> 👍（0） 💬（0）<div>LOAD_NAME字节码的实现是不是有点小问题。如果某个python变量的值恰好就是Universe::HiNone，那么这个时候直接返回它才是正确的行为。</div>2024-09-20</li><br/><li><span>Geek_66a783</span> 👍（0） 💬（1）<div>全局变量那一块混入了python2.7的字节码</div>2024-09-20</li><br/>
</ul>