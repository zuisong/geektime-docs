...
<li><span>达芬奇</span> 👍（1） 💬（1）<div>精通设计模式就是再无设计模式，寡闻了，第一次听说</div>2022-11-05</li><br/><li><span>Farewell丶</span> 👍（17） 💬（1）<div>```
...
这样歪</div>2020-01-16</li><br/><li><span>Jesse</span> 👍（0） 💬（3）<div>使用组合模式遍历树：我的想法是，每棵树都可以看成是根节点和子树（左子树，右子树）的组合。因此在定义节点Node的时候，就可以持有两个子树根节点的的引用（left,right），这也是一般链式结构树的实现，用到的就是组合模式。而树的遍历，就可以递归的遍历所有的子树了。</div>2019-12-30</li><br/><li><span>UncleNo2</span> 👍（16） 💬（2）<div>原来装饰器模式是用来写诗的，看来我误解它了</div>2020-06-22</li><br/><li><span>escray</span> 👍（4） 💬（0）<div>看了前面几讲，确实能感觉到多态才是面向对象的精髓。
...
手工敲了一遍 @Farewell丶 留言中的代码，体验了一下组合模式遍历树。</div>2020-09-25</li><br/><li><span>Geek_d68bf9</span> 👍（3） 💬（0）<div>侃侃而谈，但也是泛泛而谈</div>2022-04-20</li><br/><li><span>蓝魔丶</span> 👍（3） 💬（0）<div>每个子类构造函数传入父类实现组合，通过递归获取父类</div>2019-12-27</li><br/><li><span>Andrew</span> 👍（2） 💬（0）<div>精通设计模式，就是忘了什么是设计模式，但是程序和架构又是健壮、灵活、清晰、易于复用和扩展的，臻于武学最高境界，很赞👍</div>2022-10-05</li><br/><li><span>尹宗昌</span> 👍（2） 💬（0）<div>无招胜有招</div>2019-12-27</li><br/><li><span>java小霸王</span> 👍（1） 💬（0）<div>适配器和装饰器模式的区别？组合模式具体是？后面再来补坑</div>2022-06-29</li><br/><li><span>不要挑战自己的智商</span> 👍（0） 💬（0）<div>interface Tree {
