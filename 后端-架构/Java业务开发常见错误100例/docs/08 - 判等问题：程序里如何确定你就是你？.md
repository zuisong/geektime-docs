...
TreeSet本质TreeMap的key组成的，数据结构是红黑树，是自带排序功能的，可以在放入元素的时候指定comparator（比较器），或者是放入的元素要实现Comparable接口后元素自己实现compareTo方法，contains方法是根据比较器或者compareTo去判断相等</div>2020-03-26</li><br/><li><span>👽</span> 👍（24） 💬（1）<div>2 . HashSet 底册是HashMap。TreeSet底层是TreeMap
...
简单来说，TreeSet底层使用compareTo方法比较，HashSet底层使用hash值比较。</div>2020-03-26</li><br/><li><span>Sun</span> 👍（12） 💬（1）<div>老师的课程，真的是干货，每天凌晨更新完看一遍，早上上班前在看一遍，感受都不一样，期待出更多干货，共同进步</div>2020-03-26</li><br/><li><span>东方奇骥</span> 👍（4） 💬（1）<div>看到这节，说起Lombok，老师觉得Lombok 适合用于生产环境吗？之前一直都是自己业余练习使用，但是工作中项目都还是没有使用。</div>2020-03-26</li><br/><li><span>pedro</span> 👍（4） 💬（4）<div>1楼的回答已经趋于完美，我也翻了一下 JDK 源码，HashSet 的本质是 HashMap，会通过 hash 函数来比较值，TreeSet 的本质是 TreeMap 会通过 compareTo 比较。
至于类加载器的问题，我想这个不好复现，有没有楼下的小伙伴补充一下的。</div>2020-03-26</li><br/><li><span>阿郑</span> 👍（3） 💬（1）<div>老师的每一篇文章都是满满的干货呀，手动点赞👍👍👍</div>2020-03-26</li><br/><li><span>James</span> 👍（2） 💬（1）<div>每一个案例都是独立的SpringBoot或Java命令行应用程序，可以单独启动，避免相互干扰
请问下,为啥我启动的是独立main类为啥还会启动其他无关这个包的类.比如连接redis</div>2020-07-11</li><br/><li><span>Huodefa_0426</span> 👍（2） 💬（2）<div>老师，文中你说：在启动程序时设置 JVM 参数 -XX:+PrintStringTableStatistics，程序退出时可以打印出字符串常量表的统计信息。调用接口后关闭程序，输出如下。我设置了关闭程序怎么没看见输出的信息，是输出在控制台还是在日志文件中？如果是文件 是哪个文件？</div>2020-03-26</li><br/><li><span>yihang</span> 👍（1） 💬（1）<div>补充一点，浮点数的==比较也有坑，跟浮点数小数精度有关</div>2020-03-28</li><br/><li><span>justin</span> 👍（0） 💬（1）<div>老师您好，就是关于这个例子，加了intern()会在常量池驻留，导致每个bucket size变得很大。如果没有加intern()方法的时候，Average bucket size就变得很低，测试的是0.276，这个帮忙解释下
@GetMapping(&quot;internperformance&quot;)
public int internperformance(@RequestParam(value = &quot;size&quot;, defaultValue = &quot;10000000&quot;)int size) {
    &#47;&#47;-XX:+PrintStringTableStatistics
    &#47;&#47;-XX:StringTableSize=10000000
...
</div>2020-04-02</li><br/><li><span>👽</span> 👍（9） 💬（0）<div>问题1：
...
区别在此，getClass更加严格，而instanceof  子类instanceof  父类，也是true</div>2020-03-26</li><br/><li><span>yihang</span> 👍（7） 💬（0）<div>另外对于 intern 也有它的用武之处，据说 twitter 使用它减少重复地址（字符串）大量节约了内存</div>2020-03-28</li><br/><li><span>搞起来</span> 👍（2） 💬（0）<div>老师的课程特别好</div>2020-08-05</li><br/><li><span>失火的夏天</span> 👍（2） 💬（0）<div>hashset和treeSet从根本上来说，没什么关系，只是有个N代以前的祖宗了，哈哈，一个玩hash，一个玩comparator。一个底层是散列表，一个底层是红黑树。</div>2020-03-28</li><br/><li><span>Geek_3b1096</span> 👍（2） 💬（0）<div>上班前看一遍+1</div>2020-03-27</li><br/>
</ul>