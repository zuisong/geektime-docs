...
01. 客户端发送记录到WORKER；
02. Handler收到消息后处理失败；
03. 第一次重试，发送消息到BUFFER；
04. 3秒后消息过期，自动转发到WORKER；
05. Handler再次收到消息后处理失败；
06. 第二次重试，发送消息到BUFFER；
07. 3秒后消息过期，还是自动转发到WORKER；
08. Handler再次收到消息后处理失败，达到最大重试次数；
09. 发送消息到DEAD（作为死信消息）；
...
<li><span>铎</span> 👍（16） 💬（1）<div>老师您好，一直想请教个问题：我们现在的项目目前正在将原来的老项目拆分为多个微服务，随之而来的一个就是分布式事务问题，一直没找到好的解决方案，请问老师您们是如何来处理分布式事务或者使用什么开源框架，能否提供几种选择。多谢老师！</div>2020-07-22</li><br/><li><span>程序员小跃</span> 👍（4） 💬（1）<div>我发现咱们专栏这个课后答疑挺有特色的，已经有好几期的课后答疑让我学到了很多，至少又进一步巩固了我学习的内容，给老师这个设计点赞</div>2020-10-25</li><br/><li><span>常惠源</span> 👍（1） 💬（1）<div>老师您好，可否说一下巨衫数据库中MongoDB数据的存储，是否可以应用在重要数据的存储上？</div>2021-01-19</li><br/><li><span>J.Smile</span> 👍（1） 💬（1）<div>老师，线程池的监控有没有什么好的方式？</div>2020-08-04</li><br/><li><span>Michael</span> 👍（0） 💬（1）<div>老师，你好！
redis集群模式下，根据模糊key删除缓存有什么好的办法？scan命令在集群下不行</div>2020-07-31</li><br/><li><span>惘 闻</span> 👍（2） 💬（2）<div>&quot;虽然 MongoDB 的写入性能较高，但复杂查询性能却相比 Elasticsearch 来说没啥优势；虽然 MongoDB 有 Sharding 功能，但是还不太稳定。因此，我个人建议在数据写入量不大、更新不频繁，并且不需要考虑事务的情况下，使用 Elasticsearch 来替换 MongoDB。&quot;
老师最后一题从这句话看来只看到了MongoDB相对于ES查询没什么优势,没看到ES的优势在哪里,所以老师推荐ES的原因具体是ES哪些方面比MongoDB要好啊.存储稳定性更高和查询效率更好吗?</div>2020-12-24</li><br/><li><span>文涛</span> 👍（0） 💬（0）<div>老师，您好，关于那个事务还未提交但是消息已经消费的问题，可以先提交事务在发送MQ，您又说的如果MQ消息发送失败，这时候数据已经入库，是否可以发送MQ消息的方法try catch处理，如果走到catch，说明发送消息失败，此时手动回滚数据，但是这种可能会有极端情况，还没有回滚数据的时候，这个入库的数据被查询到了</div>2024-11-07</li><br/><li><span>Joker</span> 👍（0） 💬（1）<div>把业务处理和保存 MQ 消息到本地消息表的操作，放在相同事务内处理。
