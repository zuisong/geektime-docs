...
<li><span>梦倚栏杆</span> 👍（25） 💬（1）<div>老师，现在像fastJson, jackson 一般使用序列化和反序列化不都是属性类型兼容就能来回序列化吗？java序列化的时候存储序列化id记录版本号的意义是什么。 java序列化一开始存在的意义是什么？为什么要那样处理呢？如果按照现在fastJson 和jackson等的处理方式，toString 不也是一种序列化方式吗？反序列化时按照一种规则解析回去不就行了</div>2020-04-14</li><br/><li><span>Darren</span> 👍（22） 💬（5）<div>试着回答下今天的问题：
...
2、Jackson2ObjectMapperBuilder的采用了构建者模式创建对象；调用的是build()方法
 ...
 然后configure里面出了了甚多事情，比如：日式、Local、时间等的格式化器以及自定义属性命名策略等，具体详见https:&#47;&#47;github.com&#47;y645194203&#47;geektime-java-100&#47;blob&#47;master&#47;ConfigInfo.java</div>2020-04-14</li><br/><li><span>左琪</span> 👍（10） 💬（1）<div>老师，我之前遇到一个，我用redis存入一个Map&lt;Long,object&gt;，取出时发现却是Map&lt;int,object&gt;,然后响应给前端springmvc就报类型转换异常了，我redis的value也是用的Jackson序列化，自定义了objectmapper，正常对象都能序列化，反序列化，就是Long不行，我想知道该如何修正呢</div>2020-04-16</li><br/><li><span>👽</span> 👍（7） 💬（1）<div>之前其实一直还是比较喜欢枚举的，一直只是觉得枚举这个是个好的功能，只是我不会用。
...
</div>2020-04-14</li><br/><li><span>扎紧绷带</span> 👍（6） 💬（1）<div>有个同学说不用 int 来枚举，而选择语义性的字符串。我也觉得语义明确的字符串更好一些，但很多人认为数字占空间小，应该用int。老师怎么看，你们是怎么用的呢？</div>2020-07-04</li><br/><li><span>pedro</span> 👍（5） 💬（2）<div>关于枚举，无论是在 dto 还是数据库存储，我们都已经不用 int 来枚举了，而选择语义性的字符串，这在 debug 和维护上十分方便，也有利于迁移，int 枚举太难看了，每次调试，眼睛都花了😭</div>2020-04-14</li><br/><li><span>Michael</span> 👍（2） 💬（2）<div>我们项目中遇到的坑是：key是字符串，value是一个自定义对象，我们环境分为inner和prd,inner验证过了才会发生产，但是inner和prd是同一个Redis缓存和DB，value值对应的对象中加了字段，生产和inner同时作用，prd缓存失效了，正好把inner的给存进去了，结果导致生产接口从缓存取数据的时候出现反序列化报错问题，影响了生产。
后面采取方法是在缓存key加上环境前缀来避免这个问题。</div>2020-04-19</li><br/><li><span>💢 星星💢</span> 👍（1） 💬（1）<div>老师，我最近遇到一个坑，也是Jackson 序列化反序列的。一开始在xml中定义只定义mappingJacksonHttpMessageConverter，然后在DTO中某日期字段中加上@JsonDeserialize(using = DateJsonDeserialize.class)能实现日期转为功能。但是前台页面中显示的日期全都为时间戳。
于是在定义了ObjectMapper 里面重写了日期格式化的序列化方法,然后原先的进行接口Json格式反序列，@JsonDeserialize这个功能不能用了，找了好久都没有解决办法。老师这个坑，该如何解决？</div>2020-07-07</li><br/><li><span>小杰</span> 👍（1） 💬（1）<div>   log.info(&quot;longRedisTemplate get {}&quot;, (Long)longRedisTemplate.opsForValue().get(&quot;longRedisTemplate&quot;));
java.lang.ClassCastException: java.lang.Integer cannot be cast to java.lang.Long
强转异常，也就是说我们获取到这样的值还要自己从Integer转成Long是吗老师？</div>2020-04-14</li><br/><li><span>捞鱼的搬砖奇</span> 👍（0） 💬（1）<div>ObjectMapper 的 activateDefaultTyping 方法 在2.10版本才提供，那之前的版本有替代方案吗。</div>2020-08-21</li><br/><li><span>Geek_3b1096</span> 👍（2） 💬（0）<div>习惯用枚举表示数据库状态字段
希望老师能指点枚举最佳实践</div>2020-04-18</li><br/><li><span>鲁鸣</span> 👍（1） 💬（0）<div>gson处理数值的时候也有类似的情况，整数变为浮点数，或者是长整数变为科学计数法</div>2020-09-24</li><br/><li><span>额。</span> 👍（1） 💬（0）<div>我们遇到了一个坑，在用restTemplate远程调用的时候，用FastJson的JSONObject接收，然后springboot利用jackson序列化和反序列化返回来一个LinkedHashMap，开发同学的操作不规范，喜欢直接转型，就报错了</div>2020-06-04</li><br/><li><span>梦倚栏杆</span> 👍（1） 💬（0）<div>redis 的序列化也遇到过问题，但是当时处理的比较紧急比较模糊，记不清了。
  一个类里需要不同结构的redis或者类型不同==&gt; 
  a&gt; 序列化报错，写不进去。
  b&gt; 没报错，阿里云后台查看redis的结构是None,key是乱码，也无法删除。
  最终的解决方案是：自己new RedisTemplate, 对于每一种结构对象都设置对应的序列化反序列化方式。
 redis 的incrby需要long接收，因为返回值是64的整数。当时看到integer 单词还以为是java的integer，结果是64位的integer</div>2020-04-14</li><br/><li><span>Geek_7de969</span> 👍（0） 💬（0）<div>@JsonFormat(pattern = &quot;yyyy-MM-dd&quot;)
private Date signDate;
这是一个对象的某个字段是日期类型 redis用的上边的序列化存进去发现和数据库中日期相差一天  是怎么回事呢</div>2022-06-02</li><br/>
</ul>