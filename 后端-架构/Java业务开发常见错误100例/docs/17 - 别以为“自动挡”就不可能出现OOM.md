...
<li><span>一个汉子~</span> 👍（27） 💬（3）<div>针对第二点，可以先compile，然后在内存中保存，脚本内容的hash作为key，compile结果作为value，用ConcurrentReferenceHashMap保存
同样的风险还出现在表达式框架aviator中</div>2020-04-18</li><br/><li><span>Darren</span> 👍（12） 💬（3）<div>试着回到下问题：
第一个：
...
第二种方法：GroovyShell设置为全局的，然后使用缓存，每次都是先从缓存中获取，获取不到了，在加载，然后更新缓存。</div>2020-04-20</li><br/><li><span>汝林外史</span> 👍（12） 💬（1）<div>1. 对于老师说的autoComplete的场景是不是Trie树更适合一些？
...
3. 对于问题1，应该是用软引用更好一些，用弱引用总是被gc回收就失去了缓存的意义。对于问题2不是很了解。</div>2020-04-20</li><br/><li><span>👽</span> 👍（9） 💬（2）<div>应该用哪种引用，首先考虑的肯定是四大引用的区别。
...
但是这里又要说点额外的，单说缓存设计，还要涉及其他的因素。包括缓存大小，缓存的过期时间等。让我来说的话，我可能会考虑使用现有的缓存实现，或者是redis。自己实现一套缓存，成本略高。</div>2020-04-18</li><br/><li><span>一个汉子~</span> 👍（8） 💬（3）<div>之前还遇到一个，一个导出功能，拥有管理员权限的人几乎没有限制，造成了全表查，再加上框架禁止join，所以又把外键拉出来做了一次in查询，也是全表扫，大量的Bo对象和超长sql，直接把系统oom了</div>2020-04-18</li><br/><li><span>不能忍的地精</span> 👍（7） 💬（1）<div>我遇到一个OOM,是这样的
...
3. 出现5个连接都超时,任务卡住了,但是还是不断的往任务队列里面添加任务,最终导致OOM</div>2020-04-24</li><br/><li><span>ddosyang</span> 👍（2） 💬（1）<div>老师想问一下，在WeakHashMap的那个例子里，可不可以直接用String name当作Key，而不是用User做Key。这样是不是也可以解决问题？</div>2020-04-20</li><br/><li><span>Carisy</span> 👍（1） 💬（1）<div>老师请教一个问题，在使用CompletableFuture的时候出现了很奇怪的场景，就是buffers飙升出现oom，应用程序使用内存并不多，处理逻辑也相对简单就是调用接口通过http上传下载文件</div>2020-06-05</li><br/><li><span>旅途</span> 👍（1） 💬（2）<div>有点没懂 java.lang.IllegalArgumentException: Request header is too large 的意思不就是request header过大了么 为什么开发人员还设置的那么大？</div>2020-04-28</li><br/><li><span>pedro</span> 👍（1） 💬（3）<div>问题一，弱引用是在内存不足时被 gc 掉，而软引用是只要 gc 就回收掉，自然就不能用来做缓存，否则动不动就缓存失效，数据库怕是要被玩坏哦，因为适合做缓存的是弱引用。
问题二，没用过 groovy，希望看到别人的解答。😄</div>2020-04-18</li><br/><li><span>Geek_3b1096</span> 👍（2） 💬（1）<div>周六第一件事跟上老师进度</div>2020-04-18</li><br/><li><span>自由港</span> 👍（2） 💬（0）<div>关于第二个向题限制了metadata的堆大小，发现就可自动回收了</div>2020-04-18</li><br/><li><span>Geek_d432e7</span> 👍（0） 💬（0）<div>专栏内容非常具有参考价值，希望能够多推送此类专栏。</div>2023-10-03</li><br/><li><span>子陌.</span> 👍（0） 💬（0）<div>享元模式</div>2023-09-30</li><br/><li><span>yang</span> 👍（0） 💬（0）<div>技术方案也够奇葩的------------怎么设计出来的-----~-~----</div>2020-04-20</li><br/>
</ul>