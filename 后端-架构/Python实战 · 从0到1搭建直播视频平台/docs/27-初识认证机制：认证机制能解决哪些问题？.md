...
<li><span>ZENG</span> 👍（1） 💬（1）<div>思考题：使用请求拦截，每次请求接口可以带上token等信息
...
3、前端发来的token如果还没到过期时间，但是redis又查不到，可不可以通过解密token然后根据荷载数据的某些信息通过mysql去查数据返回呢</div>2023-06-23</li><br/><li><span>peter</span> 👍（0） 💬（1）<div>请教老师两个问题：
Q1：OAuth2会用到吗？
