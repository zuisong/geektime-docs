...
<li><span>半瓶醋</span> 👍（14） 💬（6）<div>注意：CREATE PROCEDURE `get_sum_score`这里不是单引号，是` `号哦~
...
SELECT @sum_max_hp;</div>2019-10-25</li><br/><li><span>毛豆</span> 👍（14） 💬（3）<div>存储过程很重要，可不可以详细讲讲</div>2019-07-12</li><br/><li><span>番茄</span> 👍（11） 💬（1）<div>请教下老师或者各位小伙伴，调用程序这里，直接call就出结果了，下面那句select为什么要加上呢

CALL get_hero_scores(@max_max_hp, @min_max_mp, @avg_max_attack, &#39;战士&#39;);
SELECT @max_max_hp, @min_max_mp, @avg_max_attack;</div>2019-11-26</li><br/><li><span>高志强</span> 👍（6） 💬（1）<div>终于明白了啥是存储过程，谢谢老师~</div>2019-11-01</li><br/><li><span>Monday</span> 👍（4） 💬（3）<div>老师的脑图总结得很到位，求共享啊。谢谢</div>2019-08-07</li><br/><li><span>丁丁历险记</span> 👍（2） 💬（5）<div>就是个毒瘤，慎用</div>2019-11-14</li><br/><li><span>Elliot</span> 👍（2） 💬（2）<div>极课时间页面的交互要是能像Medium那样就更完美了，比如可以自己mark一些书签标记</div>2019-07-21</li><br/><li><span>我行我素</span> 👍（2） 💬（1）<div>CREATE PROCEDURE `get_sum_score`(
...
SELECT @sum_hp_max;</div>2019-07-10</li><br/><li><span>Coool</span> 👍（0） 💬（1）<div>CREATE  PROCEDURE `get_sum_score`(out sum_max_hp float,in s varchar(255))
BEGIN
    select sum(hp_max) 
    from heros
    where role_main=s
...
call get_sum_score(@sum_max_hp,&#39;战士&#39;) ;
select @sum_max_hp ;</div>2019-10-22</li><br/><li><span>未来的胡先森</span> 👍（0） 💬（1）<div>交作业了
DELIMITER &#47;&#47;
CREATE PROCEDURE get_sum_score(
			OUT sum_max_hp FLOAT,
			t VARCHAR(255))
...
CALL get_sum_score(@sum_max_hp,&#39;战士&#39;);
SELECT @sum_max_hp;</div>2019-08-08</li><br/><li><span>Ronnyz</span> 👍（0） 💬（1）<div>作业：
...
+-------------+
|      124873 |
+-------------+</div>2019-07-23</li><br/><li><span>mickey</span> 👍（0） 💬（1）<div>&#47;*
...
最大生命值的总和 [坦克]
83124
</div>2019-07-18</li><br/><li><span>假装自己不胖</span> 👍（0） 💬（1）<div>CREATE PROCEDURE `get_sum_score`(
OUT sum_hp_max FLOAT,
s VARCHAR(255)
)
BEGIN
...
偷了一个同学的改了改</div>2019-07-15</li><br/><li><span>supermouse</span> 👍（0） 💬（1）<div>创建存储过程：
DELIMITER &#47;&#47;
...
SELECT @max_hp_sum;</div>2019-07-14</li><br/><li><span>Geek_70747e</span> 👍（0） 💬（1）<div>课后作业
DROP PROCEDURE IF EXISTS get_sum_score;
DELIMITER &#47;&#47;
CREATE PROCEDURE get_sum_score (
	IN role VARCHAR (32),
...
SELECT @sum_score;</div>2019-07-12</li><br/>
</ul>