...
<li><span>Dan</span> 👍（3） 💬（1）<div>你知道在 Django 项目中如he创建管理员账号吗？
python manage.py cratesuperuser

如何在 admin.py 文件中增加 Watering 数据库模型对象，实现对自动浇花器监测数据的查询呢？
admin.site.register(Watering)</div>2021-01-04</li><br/><li><span>柠檬水</span> 👍（6） 💬（0）<div>越来越佩服老师了，真的是全栈啊，从硬件到软件，从前端到后端。</div>2021-01-06</li><br/><li><span>pg黑凤凰</span> 👍（0） 💬（0）<div>学习难度呈指数级上升</div>2022-01-24</li><br/>
</ul>