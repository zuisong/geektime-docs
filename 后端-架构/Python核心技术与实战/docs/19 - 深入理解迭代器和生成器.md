...
<li><span>Jingxiao</span> 👍（52） 💬（5）<div>思考题答案：
很多同学的回复非常正确，生成器只能遍历一次，继续调用 next() 会 raise StopIteration。只有复位生成器才能重新进行遍历。</div>2019-06-23</li><br/><li><span>John Si</span> 👍（41） 💬（3）<div>我不知道如何把这技巧运用在编程中，老师能否举几个例子来说明一下呢？谢谢</div>2019-06-21</li><br/><li><span>TKbook</span> 👍（57） 💬（6）<div>def is_subsequence(a, b):
    b = iter(b)
    print(b)
...
</div>2019-06-21</li><br/><li><span>SCAR</span> 👍（32） 💬（1）<div>思考题：对于一个有限元素的生成器，如果迭代完成后，继续调用 next(),会跳出StopIteration:。生成器可以遍历多次吗？不行。也正是这个原因，老师代码复杂化那段代码，在
    gen = ((i in b) for i in a)
    for i in gen: 
        print(i)
之后应该是需要给b复位下，不然b会是空的，那么return回来永远会是False。

这段判断是否是子序列的指针用的真是巧妙，区区几行，精华尽现。</div>2019-06-21</li><br/><li><span>恨你</span> 👍（21） 💬（8）<div>有错误的地方：list_1 = [i for i in range(100000000)]结果是一个可迭代对象，是一个列表，而不是一个迭代器。所以文中使用例子来说明的是生成器比列表节省内存，而不是迭代器比生成器节省内存。
...
，请老师深入研究下后讲给我们。谢谢。
</div>2019-11-15</li><br/><li><span>tt</span> 👍（15） 💬（2）<div>明白为啥要把b转换成迭代器了，是为了下面的代码中可以用next():
...
这样才可以利用next()可以保存指针位置的特性，从而确保子序列中元素的顺序。</div>2019-06-21</li><br/><li><span>farFlight</span> 👍（10） 💬（1）<div>迭代完成后，继续调用 next()会出现StopIteration。
生成器只能遍历一次，但是可以重新调用重新遍历。</div>2019-06-21</li><br/><li><span>Lonely绿豆蛙</span> 👍（8） 💬（2）<div>比较下return 与 yield的区别：

return：在程序函数中返回某个值，返回之后函数不在继续执行，彻底结束。
yield: 带有yield的函数是一个迭代器，函数返回某个值时，会停留在某个位置，返回函数值后，会在前面停留的位置继续执行，直到程序结束。</div>2020-02-05</li><br/><li><span>力维</span> 👍（3） 💬（1）<div>最后的例子的确比较有意思，个人理解有三个关键点：
一是(i in b)的含义
b = (i for i in range(5))
print(2 in b)
...
另外，第七段“集合（set: set([0, 1, 2])）都是容器。”  是否改为 set: {0,1,2}比较好？虽然都是同一个意思。</div>2020-02-19</li><br/><li><span>Geek_59f23e</span> 👍（3） 💬（1）<div>1、大家对next函数可能有些误区，迭代完成后继续调用next函数会返回默认值None。
...
2、生成器遍历到最后一个元素后抛出StopIteration，不能遍历多次，重新遍历需要生成一个新的生成器。</div>2019-06-22</li><br/><li><span>单色</span> 👍（2） 💬（1）<div>生成器之前一直是一知半解，似懂非懂，今天学习后，有了更深入的了解。</div>2020-03-29</li><br/><li><span>鱼_XueTr</span> 👍（2） 💬（1）<div>会引发StopIteration。
生成器只能使用一次。</div>2019-06-21</li><br/><li><span>youaresherlock</span> 👍（0） 💬（1）<div>from collections import Iterable, Iterator 
isinstance(object, Iterable)等等可以判断是否是可迭代对象或者迭代器对象</div>2020-06-30</li><br/><li><span>单色</span> 👍（0） 💬（1）<div>问题
生成器的next()函数：是保存返回本次结果，并去计算下次调用结果？</div>2020-03-29</li><br/><li><span>苹果</span> 👍（0） 💬（1）<div>dic = {1:&#39;a&#39;,2:&#39;b&#39;,3:&#39;c&#39;}
iterater = iter(dic)
next(iterater)
Out[105]: 1
next(iterater)
Out[106]: 2
对于字典生成的迭代器，只返回Key的值，怎么放回value，或k,v 的值？</div>2020-01-31</li><br/>
</ul>