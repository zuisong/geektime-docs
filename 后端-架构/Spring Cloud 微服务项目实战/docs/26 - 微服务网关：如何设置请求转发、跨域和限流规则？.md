...
<li><span>勤奋的樂</span> 👍（6） 💬（1）<div>向老师请教一个问题，gateway做限流和sentinel限流两者的关系，是配合使用还是二选一？
...
不知道我的理解是不是对的，望指正
PS: gitee上最新的代码中找不到request_rate_limiter.lua文件</div>2022-05-10</li><br/><li><span>inrtyx</span> 👍（3） 💬（1）<div>老师，能否讲讲网关如何鉴权？即鉴权时序图</div>2022-02-11</li><br/><li><span>黄叶</span> 👍（1） 💬（3）<div>老师请问下，今天试着写了gateway+vue整合。
vue请求发送给后端，预检通过了但是当真正发送跨域请求时，提示：CORS错误，也配置了gateway跨域 但是就是不成功</div>2022-02-11</li><br/><li><span>周钦华</span> 👍（0） 💬（1）<div>我也是按文章中的配置，跨域没生效，谁知道怎么配置？</div>2024-04-18</li><br/><li><span>乘风</span> 👍（0） 💬（1）<div>我怎么没看明白如何测试，是访问哪个地址？</div>2023-04-14</li><br/><li><span>Geek_8d3106</span> 👍（0） 💬（1）<div>使用阿里云Redis集群，会报以下错误
...
不知道是否有好的解决方案？</div>2022-04-01</li><br/><li><span>peter</span> 👍（15） 💬（1）<div>请教老师几个问题：
...
搜索方法：Edit -&gt;Find-&gt;Find in Path,但搜不到，为什么？（下载的是最新代码，IDEA2019）</div>2022-02-12</li><br/><li><span>刚强</span> 👍（1） 💬（0）<div>postman测试gateway，一直403。调查DefaultCorsProcessor.processRequest方法后发现，CorsUtils.isCorsRequest(request)=true。跨域了，origin=chrome-extension:&#47;&#47;XXXX。把这个加到yml里面就可以了。</div>2022-04-18</li><br/><li><span>王凯</span> 👍（0） 💬（0）<div>不支持一个路由多个限流规则配置</div>2023-04-12</li><br/><li><span>小猪丶快跑</span> 👍（0） 💬（1）<div>完全没搞明白为什么限流是这样配置就可以了？？？</div>2023-02-18</li><br/>
</ul>