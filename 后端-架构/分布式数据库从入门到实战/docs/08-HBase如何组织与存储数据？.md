...
<li><span>Geek_03c08d</span> 👍（2） 💬（1）<div>有个疑问，不建议多个列族，为什么还要设计多个列族这个东西？</div>2024-06-29</li><br/><li><span>Geek_03c08d</span> 👍（2） 💬（1）<div>在HBase中，列族（Column Family）是一个重要的概念，但通常建议一个HBase表尽量少使用多个列族，甚至只使用一个列族。原因如下：
存储效率和性能：
...
以上回答来自gpt4-o</div>2024-06-28</li><br/><li><span>密码123456</span> 👍（1） 💬（1）<div>由于不同的列族存储在不同的文件，是不是担心跨文件读取数据？</div>2024-06-27</li><br/><li><span>lufofire</span> 👍（0） 💬（1）<div>思考题感觉本身有点奇怪了。先说疑问：
