...
<li><span>好运来</span> 👍（0） 💬（1）<div>我在公司实践时，消费者的蓝绿发布状态有下面三个值。所有： 表示该消费组未接入蓝绿。蓝：表示该消费组接入蓝绿，并且消息属性中未带颜色的消息由蓝环境的消费者进行消费。绿：表示该消费组接入蓝绿，并且消息属性中未带颜色的消息由绿环境的消费者进行消费。

这段话是不是有误，读起来逻辑不通。</div>2022-08-27</li><br/><li><span>a、</span> 👍（3） 💬（1）<div>1.可以，但是这么做会导致如果broker里有蓝绿消息的话，就会出现消息丢失,
