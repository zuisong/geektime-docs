...
<li><span>ZR2021</span> 👍（4） 💬（1）<div>使用while是防止广播通知方式的虚假唤醒吧，需要用户进一步判断，但是，我看还有人说可能存在非广播方式的虚假唤醒，不知道什么场景下会出现这种情况，还有，我们现在使用的都是pthread 库，不知道跟老师讲的c库自带的这些原子等操作有什么区别……</div>2022-01-18</li><br/><li><span>白花风信子</span> 👍（3） 💬（1）<div>并发编程这儿感觉有点不熟悉.....请问有什么练习的建议吗？感觉和平常接触的有很多不同。</div>2022-01-15</li><br/><li><span>ckj</span> 👍（2） 💬（2）<div>请问下 条件变量的例子中 21行 main中的互斥量重新加锁是不是需要等run中的释放之后才能加锁？ 因为mtx_plain?</div>2022-05-21</li><br/><li><span>墨</span> 👍（2） 💬（1）<div>用了条件变量后，还是有循环一直在判断 done 的值啊，只不过变成到 main 函数中判断了，感觉和之前没啥区别啊</div>2022-02-13</li><br/><li><span>赖淦</span> 👍（1） 💬（2）<div>老师，条件变量和信号量是相同的概念吗？</div>2022-07-17</li><br/><li><span>猪小擎</span> 👍（1） 💬（1）<div>mutex有syscall吗？需要调用fetex系统调用吗？写一段cpp代码，在20个线程里每个县城++10万次，一个用mutex抢一个锁，一个用原子变量++，在mac的m1 max上运行，mutex要快得多</div>2022-06-15</li><br/><li><span>左星辰</span> 👍（1） 💬（1）<div>memory_order的设置会被编译器处理很好理解，但是是如何被处理器理解的呢？</div>2022-04-24</li><br/><li><span>liu_liu</span> 👍（4） 💬（1）<div>使用 while 的原因是，当阻塞的线程被重新调度运行时，done 的值可能被改变了，不是预期值。</div>2022-01-14</li><br/><li><span>dapaul</span> 👍（3） 💬（2）<div>老师，使用条件变量那段的例子不会死锁吗？main线程中加锁，然后阻塞等待,锁没释放。创建的线程等锁去释放条件变量，一直等不到锁，两边都在等</div>2022-07-14</li><br/><li><span>Cyan</span> 👍（1） 💬（0）<div>提个问题，问什么我将“使用条件变量”的C语言程序，改为C++程序，程序退出的时候就会报core dump呢？具体的程序如下：
```
#include &lt;iostream&gt;
#include &lt;mutex&gt;
