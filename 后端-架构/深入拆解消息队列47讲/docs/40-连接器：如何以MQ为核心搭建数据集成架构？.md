...
<li><span>Geek_ec80d2</span> 👍（0） 💬（1）<div>好一点的数据集成工具都是商业化的产品，开源的flink CDC导库不能保证数据可靠性。</div>2023-09-26</li><br/><li><span>Geek_ec80d2</span> 👍（1） 💬（0）<div>1、是在讲数据库的导库，还是讲数据库的CDC？方法和实现也不一样。
