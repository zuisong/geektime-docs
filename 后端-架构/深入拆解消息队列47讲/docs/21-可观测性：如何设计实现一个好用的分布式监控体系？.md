...
<li><span>虚竹</span> 👍（0） 💬（1）<div>动态调整日志的级别，也是只能看到调整后的错误信息，调整之前的错误信息看不到了吧？</div>2023-10-12</li><br/><li><span>takumi</span> 👍（0） 💬（0）<div>老师，为什么接口调用耗时使用Histogram而不是Summary？Summary还可以记录P99 P95这种</div>2023-11-03</li><br/>
</ul>