...
<li><span>peter</span> 👍（4） 💬（2）<div>请教老师几个问题：
Q1：MySQL支持XA的bug很多，那么，实际项目中不能用XA吗？
Q2：TCC既可以是XA，也可以不是XA，对吗？
Q3：seat是采用XA的吗？还是普通的事务？
Q4：中厂、大厂一般用什么来做分布式事务？
Q5：一个网站，50万用户，这种规模的网站用什么来实现分布式事务？</div>2022-11-11</li><br/><li><span>Sam Fu</span> 👍（1） 💬（1）<div>老师 xa这玩意有人用吗</div>2023-09-01</li><br/><li><span>花花大脸猫</span> 👍（1） 💬（1）<div>前提是能接受短暂的数据不一致（非强一致性），所以业务上我们一般很少会使用分布式事务来管控，而是通过补偿机制来实现业务数据的一致性，但是如果调用的链路很长的话，补偿也是一个很头疼的事情</div>2022-11-18</li><br/><li><span>txjlrk</span> 👍（1） 💬（1）<div>厉害，由浅入深，由理论到实践，面面俱到。
分布式事务也分为柔性和刚性两种</div>2022-11-11</li><br/><li><span>阿昕</span> 👍（0） 💬（1）<div>思考题：1.利用本地消息表+mq，或者mq的事务消息实现，可以达到解耦的目的；2.利用seata类框架，对复杂场景或者长事务可以使用sega模式。</div>2023-04-06</li><br/><li><span>👽</span> 👍（0） 💬（1）<div>有一个没用的废话就是，要尽可能减少分布式事务的发生以及，尽可能使分布式事务的力度更细。
...
而且有时候不得不考虑一个问题，多个服务间很可能不是同一套协议，也不适合让人去兼容你的事务解决方案。</div>2023-03-31</li><br/><li><span>Mr.差不多</span> 👍（0） 💬（1）<div>您好，老师 TCC 和二阶段是什么关系？</div>2023-03-03</li><br/><li><span>jjn0703</span> 👍（0） 💬（2）<div>AT和Saga这是另外的两种模式吗？</div>2023-02-11</li><br/><li><span>李二木</span> 👍（0） 💬（1）<div>TCC 协议 是2pc 吗</div>2022-11-22</li><br/>
</ul>