...
<li><span>钱</span> 👍（30） 💬（1）<div>现在浏览器终于也可以写留言了，非常好！希望能将和老师相互的讨论的功能也开开，否则，不能进行对话，讲某些问题的效果不太好！
...
3：取巧，事情少做了，但是目的依旧能够达到</div>2018-09-17</li><br/><li><span>Geek_488a8e</span> 👍（25） 💬（3）<div>这些都是DSP代码典型的优化方法，目的是防止打断CPU的指令流水，提高指令处理的并行度</div>2018-09-19</li><br/><li><span>Len</span> 👍（14） 💬（1）<div>老师，如果有这样一段代码：
...
也能借助 IR 外提 「x + y」吗？</div>2018-09-18</li><br/><li><span>一个坏人</span> 👍（7） 💬（1）<div>是不是写应用系统的时候没必要按照优化方式写，编译器反正会优化？！</div>2018-09-17</li><br/><li><span>Scott</span> 👍（3） 💬（1）<div>这样展开后有一个强度削弱的机会，四个byte的赋值合并成一个int？</div>2018-09-17</li><br/><li><span>饭粒</span> 👍（2） 💬（1）<div>有点像小学四则运算里运用提公因式法等技巧来使计算简单。</div>2019-12-25</li><br/><li><span>Leon Wong</span> 👍（0） 💬（1）<div>请问老师，实践环节的循环展开后的数组越界，编译器是怎么处理的？是不是当length小于4，循环完全展开就可以了，实际上这个展开有一个隐含的假定，即length大于4的情况。</div>2018-10-07</li><br/><li><span>白三岁</span> 👍（0） 💬（1）<div>实践环节的代码，由于i++相应的变成了i+4。前面的判断条件dst.length就不应该减4了吧。</div>2018-09-28</li><br/><li><span>无言的约定</span> 👍（1） 💬（0）<div>for (int i = INIT; i &lt; LIMIT; i += STRIDE) {
  if (i &lt; 0 || i &gt;= a.length) { &#47;&#47; range check
    throw new ArrayIndexOutOfBoundsException();
  }
  sum += a[i];
...
老师，这个IMAX该如何初始化？</div>2019-10-12</li><br/><li><span>天之蓝</span> 👍（1） 💬（0）<div>请教两个问题，循环展开那个例子如果64是65是不是就越界了？实践的代码如果length为6按条件只会循环一次那下标为4、5的不就执行不到了吗？</div>2018-11-28</li><br/><li><span>靠人品去赢</span> 👍（0） 💬（0）<div>可以的，第一次就按到有人通过编译后的指令查找重复的，优化循环，厉害了。</div>2020-08-06</li><br/><li><span>小羊</span> 👍（0） 💬（0）<div>想起之前看的 duff装置 了</div>2020-04-21</li><br/><li><span>Yoph</span> 👍（0） 💬（1）<div>这些优化全都是即时编译器做的，解释器的执行过程中有相关的优化吗？</div>2019-07-16</li><br/><li><span>杨春鹏</span> 👍（0） 💬（0）<div>循环展开优化，如何防止出现数组下边越界？
