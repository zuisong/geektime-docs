...
<li><span>ppd0705</span> 👍（12） 💬（2）<div>还没写过大应用，感觉这个框架有点复杂，慢慢品</div>2021-08-08</li><br/><li><span>二的根比方</span> 👍（3） 💬（2）<div>谢谢老师，有个问题是一般框架都遵循“依赖倒置”原则，请问go应用在构建时应该考虑那些点，谢谢</div>2021-07-25</li><br/><li><span>雪枫</span> 👍（2） 💬（1）<div>照着项目抄都感觉有点费劲，所以要把这个项目吃透的话，大概需要多久的时间。</div>2022-04-08</li><br/><li><span>lesserror</span> 👍（2） 💬（1）<div>孔老板，像Cobra、Viper、Pflag这三个包，是很多知名项目都使用的方案吗？</div>2021-10-16</li><br/><li><span>huntersudo</span> 👍（2） 💬（1）<div>老师好，最后一部分，第 4 步：配置文件解析，源码部分，得这样贴出来，才能和下文对的上
func addConfigFlag(basename string, fs *pflag.FlagSet) {
	fs.AddFlag(pflag.Lookup(configFlagName))
...
}</div>2021-07-19</li><br/><li><span>女干部</span> 👍（2） 💬（2）<div>像im这种即时通讯类的，是不是可以归类到消息处理服务中去</div>2021-07-19</li><br/><li><span>邵康</span> 👍（1） 💬（1）<div>想问一下，如何实现命令行补全功能？</div>2022-06-10</li><br/><li><span>多选参数</span> 👍（1） 💬（1）<div>像 kubernete 这样的是不是也是采用类似的应用构建方法？</div>2021-12-29</li><br/><li><span>Fan</span> 👍（1） 💬（1）<div>50 个后端应用都有啥类型的呀？分享下可好？</div>2021-08-12</li><br/><li><span>helloworld</span> 👍（1） 💬（1）<div>“例如：配置 HTTPS 的绑定端口，可以通过 --secure.bind-port 配置，也可以通过配置文件配置（命令行参数优先于配置文件）：
secure:    
    bind-address: 0.0.0.0”，文中这块前后没呼应上.</div>2021-07-21</li><br/><li><span>我要去卖冰淇淋</span> 👍（0） 💬（1）<div>github.com&#47;marmotedu&#47;component-base&#47;pkg&#47;cli&#47;flag 老师这个项目404了</div>2022-04-19</li><br/><li><span>yandongxiao</span> 👍（0） 💬（1）<div>总结：
1. 如何将 viper、pflag、cobra 结合起来，构建出应用框架？
2. 应用框架主要从命令行或者配置文件，解析参数；提供 --help, --version 等辅助性命令；调用方仍然以struct方式组织这些数据。
3. 对命令行参数进行分组，</div>2021-11-27</li><br/><li><span>XI</span> 👍（0） 💬（1）<div>app.go 里面 buildCommand() 方法报错了，
...
</div>2021-10-18</li><br/><li><span>陈麒文</span> 👍（0） 💬（1）<div>每章都很有的消化，加油💪</div>2021-08-19</li><br/><li><span>Geek8292</span> 👍（5） 💬（0）<div>太牛了，一个这个可以研究到这么细，给跪了</div>2021-09-16</li><br/>
</ul>