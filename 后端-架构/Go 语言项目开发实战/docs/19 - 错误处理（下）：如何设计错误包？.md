...
<li><span>yandongxiao</span> 👍（10） 💬（1）<div>总结：
...
4. 如何打印错误？只在错误最原始位置打印错误。</div>2021-11-27</li><br/><li><span>pedro</span> 👍（9） 💬（1）<div>以推荐老师的项目给组里其它小伙伴，一起学习沉淀打磨为自己的实用标准，多谢老师</div>2021-07-08</li><br/><li><span>漂泊的小飘</span> 👍（5） 💬（1）<div>感觉需要看着源码再学一遍。。。</div>2021-07-28</li><br/><li><span>树建</span> 👍（3） 💬（3）<div>文中提到withCode实现了format方法，但是看了errors.go中只查看withMessage实现了format</div>2022-05-05</li><br/><li><span>吹口哨yu</span> 👍（2） 💬（1）<div>withCode 没有实现 Format() 方法呀？</div>2022-07-13</li><br/><li><span>.</span> 👍（2） 💬（1）<div>看完了git中withCode错误的实现逻辑对于 WitCode需要分开cause和err 两个errors，很是迷惑。起初看完代码在猜想cause这个应该是为了递归打印，实现上个章节的”科学设计错误包“内容“ 错误码100101， 10: 服务。01: 某个服务下的某个模块。01: 模块下的错误码序号，每个模块可以注册 100 个错误。” 的功能，但是看了函数IsCode的逻辑，彻底懵逼了，code只对比了第一层的code就return，显然不是为了实现上一章节的内容，所以多个cause  errors感觉是一个特别累赘的设计。

另外WithCode和WithStack 的入参都是error 接口。 如果一个error起初是go原生库的error， 先withCode再WithMessgge, 在执行err. Error() 会丢失打印不出完整的错误栈。这块是原本iam的就是业务设计就是如此吗？WithMessgge会直接决定了err. Error() 值，不管前面被With了多少次？</div>2022-06-11</li><br/><li><span>你赖东东不错嘛</span> 👍（2） 💬（3）<div>记录错误方式二：在err发生处打印了log，却依旧把err上抛，而最外层又对err进行了一次处理，这样可能导致日志里写了两份重复的err信息。望解惑！</div>2021-07-16</li><br/><li><span>Geek_25f93f</span> 👍（1） 💬（1）<div>要写这么一个codegen工具应该要废很多时间</div>2022-07-09</li><br/><li><span>Geek_c2089d</span> 👍（1） 💬（1）<div>func New(message string) error {
	return &amp;fundamental{
		msg:   message,
		stack: callers(),
	}
}
老师，想问一下errors&#47;code.go 的包里面的callers()函数，哪里来的</div>2022-04-19</li><br/><li><span>莫林</span> 👍（1） 💬（1）<div>老师能介绍一下如何适配gprc 的 status 响应吗？</div>2021-12-07</li><br/><li><span>倪昊</span> 👍（1） 💬（1）<div>错误码实际使用方法实例里的WriteResponse，如果err != nil，if分支执行完，还是会继续执行下面的c.JSON，这样不就返回两个Response结构了吗？</div>2021-09-08</li><br/><li><span>Sch0ng</span> 👍（1） 💬（1）<div>把错误处理落地成简单实用的错误处理包，有助于错误码的长期治理。</div>2021-08-10</li><br/><li><span>大刘</span> 👍（1） 💬（1）<div>错误码实际使用方法示例中
...
code.ErrUserNotFound.Error()  这个Error方法是从哪里来的？ ErrUserNotFound按照上面所说应该只是一个int值吧？</div>2021-08-02</li><br/><li><span>ppd0705</span> 👍（1） 💬（2）<div>请问一下老师，当一个接口有多个用户输入参数，其中某个参数错误, 如果需要告诉用户具体哪个参数有问题，这样CODE.Ext是动态的，该怎么处理好呢？ 


</div>2021-07-30</li><br/><li><span>Daiver</span> 👍（1） 💬（1）<div>还是不清楚，codegen是怎么生成注册代码的</div>2021-07-12</li><br/>
</ul>