...
<li><span>helloworld</span> 👍（14） 💬（5）<div>目录不是推荐统一使用单数吗，为什么文中还写成configs、deployments等复数形式呢</div>2021-06-14</li><br/><li><span>Geek_c3e438</span> 👍（13） 💬（4）<div>老师好，受益匪浅！
...
4. 不建议使用&#47;src&#47;，那一些utils, common呢？</div>2021-06-07</li><br/><li><span>h</span> 👍（9） 💬（5）<div>老师好，我在做go web后台管理系统相关的项目，用的gin框架，因为类似于那种MVC模型，所以会有model(orm)，router，middleware等目录，这种在web应用里面建议使用吗？还是说也建议按照你这种目录来划分</div>2021-06-09</li><br/><li><span>h</span> 👍（8） 💬（2）<div>可能我说的这些router，model，controller等目录是使用其他语言留下来的习惯吧，如果放在您这个目录结构，路由 orm相关的代码应该放在那个目录下呢？</div>2021-06-09</li><br/><li><span>yandongxiao</span> 👍（7） 💬（2）<div>总结：
    ...
    5. 项目管理：&#47;Makfile, &#47;scripts, &#47;tools, &#47;githooks</div>2021-11-21</li><br/><li><span>王副会长</span> 👍（3） 💬（1）<div>老师，我是小白，有些地方不太懂。比如&#47;cmd目录下面是放组件的核心代码吗，那到底是放哪些呢，逻辑不是拆分为&#47;internal和&#47;pkg了吗，那&#47;cmd下到底是放啥？还有&#47;internal中如何拆分轻量级和复杂的服务，哪些放api层下，哪些放service层里</div>2021-10-31</li><br/><li><span>Geek_d308cd</span> 👍（3） 💬（1）<div>老师好，关于不建议把类型定义放在model这点，如果它被多个模块依赖，应该放在哪呢</div>2021-06-28</li><br/><li><span>Geek_0klv1q</span> 👍（3） 💬（1）<div>数据库操作都在store&#47;mysql，那SQL语句组装是在service还是在store组装呢？</div>2021-06-17</li><br/><li><span>Cheng</span> 👍（3） 💬（1）<div>文件夹命名没有规范吗？
如：
1. 需不需要加 s
2. 要不要使用缩写，如controller 变 ctl
3. 文件夹名称的单词直接要不要加分隔符，已经加什么分隔符，目前看到: 有些不加分隔符，加-或_
4. 文件夹名称大小写问题，看到有些使用大写开头和小写开头</div>2021-06-14</li><br/><li><span>olym</span> 👍（3） 💬（2）<div>老师好，&#47;internal&#47;apiserver&#47;store&#47;mysq 这里存放的查询方法查到数据之后，数据的strcut定义应该要放在哪个目录比较合适？</div>2021-06-11</li><br/><li><span>jssfy</span> 👍（3） 💬（2）<div>请问老师，拿结构化目录结构来说，你们在内部的各个项目会强制使用统一的一级目录结构不，还是会因场景而异</div>2021-06-06</li><br/><li><span>Geek_9f2339</span> 👍（2） 💬（2）<div>GO  基于功能划分目录
DDD 基于模型划分目录
...
</div>2021-07-12</li><br/><li><span>daz2yy</span> 👍（2） 💬（1）<div>老师好，看 internal 这里是可以放多个程序的，有个疑问就是，一个项目里有多少个工程这个有什么规则来做拆分呢？也就是要不要把新的服务、功能拆分到一个单独的项目里，还是在这个项目里写，这也关系到维护的成本，部署的成本等（新的一套又是一样的目录结构要维护对吧）

PS：&quot;Makefile 常用管理内容&quot; 的链接 404 了</div>2021-06-26</li><br/><li><span>作甚</span> 👍（2） 💬（1）<div>老师好，如果我要开发一个，cms内容管理，还有需要给app或者小程序提供服务的，单独做两个项目合适还是一个项目运行两个服务合适，因为APP和CMS管理员操作和业务逻辑差异也大，而且数据库要做两个user表，管理员user表和用户user表？</div>2021-06-07</li><br/><li><span>Vackine</span> 👍（2） 💬（4）<div>所以main.go 这种 建议放根目录？</div>2021-06-07</li><br/>
</ul>