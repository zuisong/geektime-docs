...
<li><span>木瓜777</span> 👍（14） 💬（2）<div>您好，您有没有感觉比较好的开源c++项目推荐？ 
希望从别人的项目中学到一些经验，谢谢！</div>2019-12-17</li><br/><li><span>瓜农</span> 👍（4） 💬（1）<div>一直有种感觉，相对于java，c++的语法设计略显随意不够自洽。
譬如const&#47;final在java里面就final搞定了，vitual这个关键字也感觉比较鸡肋。
老师怎么看</div>2022-06-04</li><br/><li><span>可爱的小奶狗</span> 👍（4） 💬（1）<div>代码如下：
#include &lt;iostream&gt;
#include &lt;memory&gt;
using namespace std;
...
老师，如上“疑惑”所述，为什么会调用Boy的构造函数呢？我觉得不应该调用Boy的构造函数，因为调用了boy 拷贝构造，然后再调用Man的拷贝构造，就可以了。想不通。</div>2020-09-15</li><br/><li><span>zhengfan</span> 👍（4） 💬（3）<div>吴老师，您好。
请问您在static_assert中使用的例子，是不是应该判断alignment &amp;&amp; (alignment &amp; (alignment - 1) ==0)?
另外，这个算法真是精巧👍。效率之高让我甚至一度怀疑命题的等价性。证明了一下确实是等价的。</div>2020-07-01</li><br/><li><span>三味</span> 👍（4） 💬（1）<div>如果对一个函数声明了final, 我觉得没有必要在添加override了吧. 
...
PPPS: 最近对Data-oriented design感兴趣, 不知道作者以后是否有开这类理论实战课程的计划捏? 我搜索上面喷神, 就是从这里开始搜索出来的...</div>2019-12-20</li><br/><li><span>王旧业</span> 👍（2） 💬（1）<div>length的例子里，字面量operator&quot;&quot;实现中参数是long double,但是length成员变量value使用的是double，这是有意为之吗？</div>2020-03-15</li><br/><li><span>常振华</span> 👍（1） 💬（1）<div>唯一喜欢的易用性改造就是列表初始化，其它的反而增加了复杂度。</div>2021-09-30</li><br/><li><span>geek</span> 👍（1） 💬（1）<div>老师，关于子类中重新定义虚函数这个动作我之前认为是重写(overwrite)，但也碰到有人说是重载(overload)，另外我看文章中说的是覆写(就是override的翻译吧？)。
...
老师看下我说的有问题吗？</div>2021-03-05</li><br/><li><span>Milittle</span> 👍（1） 💬（1）<div>之前用default delete override final较多
static_assert 和 自定义字面量和二进制字面量用的少 争取以后可以使用在自己的项目中 提供便利 老师的硬货很多 受益匪浅 感谢</div>2020-02-25</li><br/><li><span>小一日一</span> 👍（1） 💬（2）<div>我最喜欢的C++易用性改进及理由：
auto： 少打字
scope for：少打字
类成员默认补始化语法：少打字
default 和 delete 成员函数：简化对类行为的控制难度
自定义字面量：代码看起来舒服。</div>2019-12-16</li><br/><li><span>helloworldorz</span> 👍（0） 💬（1）<div>最喜欢类数据成员的默认初始化，变量定义的时候就应该显示得去初始化它</div>2024-03-23</li><br/><li><span>铿</span> 👍（0） 💬（1）<div>auto 和 for的语法糖</div>2022-02-11</li><br/><li><span>201200861</span> 👍（0） 💬（1）<div>老师，你好，如何看移动构造函数和拷贝函数，是编译器提供的还是用户自己写的？</div>2020-09-04</li><br/><li><span>赵国辉</span> 👍（0） 💬（1）<div>老师，有没有对c++虚函数表实现原理讲解比较透彻的文章或者书籍推荐一下。对这方面比较感兴趣。</div>2020-07-22</li><br/><li><span>tr</span> 👍（0） 💬（3）<div>第四版中文版中给出的代码是这样的：
