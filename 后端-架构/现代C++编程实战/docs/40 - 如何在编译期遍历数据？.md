...
};</div>2023-11-26</li><br/><li><span>李云龙</span> 👍（1） 💬（1）<div>我在实际工程中遇到过c++ STL 容器需要序列化的场景，当时我们采用的方法是将容器中的数据转存到Json中，然后利用Json库的序列化和反序列化功能。我觉得这样的序列化应该是效率不高。学习了本讲后，我觉得STL容器的序列化有了一个新的方案：将容器中的数据转存至结构体中，然后使用本讲的静态反射进行序列化。</div>2023-11-24</li><br/><li><span>V万能的小黑V</span> 👍（0） 💬（1）<div>得反复看几遍才能吃够，现在只能看懂，还不知道怎么用</div>2024-05-07</li><br/><li><span>钱虎</span> 👍（0） 💬（0）<div>好</div>2022-07-18</li><br/>
</ul>