...
<li><span>李云龙</span> 👍（1） 💬（2）<div>分享一下我的思考题解决方案：我原本想着用递归来写，但编译一直没有通过。最后我只能用手动展开的方法，写了一个最多支持5层目录的路径前缀分离，除了下面这个函数之外，其他函数都与老师的保持一致。
template &lt;CARG Str&gt;
...
int main() {
    ...
    return 0;
}</div>2023-11-20</li><br/><li><span>李云龙</span> 👍（1） 💬（1）<div>老师，这段代码的两个return返回不同的array类型，但可以编译通过，是因为pos和start已经是编译期常量了，在编译期就可以确定走哪个return分支，从而就可以确定函数的返回值类型了。我的理解正确吗？
if constexpr (start == nullptr) {    return copy_str&lt;length(pos)&gt;(pos);  } else {    return copy_str&lt;length(start + 1)&gt;(start + 1);  }</div>2023-11-19</li><br/><li><span>陌音</span> 👍（0） 💬（1）<div>不使用-O2编译选项的话，生成的汇编还是会有&quot;&#47;usr&#47;local&quot;。在实际项目中，我们应该如何选择优化等级？</div>2024-05-06</li><br/><li><span>行大运</span> 👍（0） 💬（1）<div>什么时候出一个完整的C++20专栏！</div>2023-01-24</li><br/><li><span>piboye</span> 👍（0） 💬（4）<div>c++20 字符串处理，可以超越c了</div>2022-09-06</li><br/>
</ul>