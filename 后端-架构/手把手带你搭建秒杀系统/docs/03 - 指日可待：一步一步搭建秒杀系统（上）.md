...
<li><span>刘帅</span> 👍（34） 💬（0）<div>H5 在做灰度上线时，如何让新版本的页面请求始终打到新的灰度机器，而老页面的请求始终打到旧版本的服务上，两者不出现交叉呢？
答：在接口中增加版本号version字段，通过nginx+lua将新版本的接口请求路由到新的接口地址</div>2021-09-29</li><br/><li><span>李斌@28年华</span> 👍（6） 💬（0）<div>老师您好，看您的这个路线我想是 spring+dubbo 外加MySQL、Redis，还有一个JVM层面的架构设计。可否考虑用spring boot+spring cloud呢？

</div>2021-09-30</li><br/><li><span>yilong</span> 👍（0） 💬（0）<div>老师，请问有代码git地址吗？</div>2024-06-10</li><br/><li><span>BIGBANANA</span> 👍（0） 💬（0）<div>有没有windows说明呀。引入domain.com报错</div>2024-01-22</li><br/><li><span>pg逆袭的小红帽是谁</span> 👍（0） 💬（1）<div>默认nginx不支持lua模块吗，需要用集成的OpenResty？</div>2022-05-30</li><br/><li><span>superquanbin</span> 👍（0） 💬（2）<div>请问下你们怎么建表的，没有找到sql文件啊</div>2022-03-26</li><br/><li><span>saltedfish</span> 👍（0） 💬（2）<div>include ~&#47;Documents&#47;seckillproject&#47;demo-nginx&#47;domain&#47;domain.com;
nginx 启动的时候报这行有问题：
nginx: [emerg] open() &quot;~&#47;Documents&#47;seckillproject&#47;nginx&#47;conf&#47;~&#47;Documents&#47;seckillproject&#47;demo-nginx&#47;domain&#47;domain.com;;&quot; failed (2: No such file or directory) in ~&#47;Documents&#47;seckillproject&#47;nginx&#47;conf&#47;nginx.conf:8</div>2022-02-26</li><br/><li><span>🇰 🇦 🇳 🇬</span> 👍（0） 💬（1）<div>老师，demo-nginx这个服务的功能就是nginx提供的功能吗？那为什么不直接配置nginx呢？我不太明白。流程是nginx-&gt;demo-nginx这样的还是直接demo-nginx这样啊？</div>2021-11-25</li><br/><li><span>贤蛋蛋</span> 👍（0） 💬（0）<div>nginx与trace工具结合，对入口流量进行打标和路由</div>2021-10-25</li><br/><li><span>Aha hello xzy</span> 👍（0） 💬（1）<div>为什么每次执行前都要export，可以配置到环境吗？</div>2021-10-23</li><br/><li><span>柯察金</span> 👍（0） 💬（0）<div>Nginx -p 指定路径之后，里面的 include 指令老是直接跟在 pwd 后面，报文件找不到的错误，不知道老师的为啥可以运行。。。</div>2021-10-19</li><br/><li><span>在路上</span> 👍（0） 💬（0）<div>思考题，页面服务和接口服务都按机器部署，灰度也按相同机器顺序发布</div>2021-10-04</li><br/><li><span>nana👄</span> 👍（0） 💬（0）<div>通过nginx配置策略，来思考题中的场景</div>2021-09-29</li><br/><li><span>无脑熊</span> 👍（0） 💬（0）<div>思考题解法：
从nginx上通过upstream设置可请求到的ip:port 来做灰度发布
</div>2021-09-29</li><br/><li><span>马以</span> 👍（0） 💬（0）<div>直面痛点、蓄势待发、指日可待……，标题蛮好的😄</div>2021-09-29</li><br/>
</ul>