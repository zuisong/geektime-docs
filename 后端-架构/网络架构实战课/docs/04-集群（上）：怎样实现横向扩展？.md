...
<li><span>潘政宇</span> 👍（1） 💬（2）<div>&quot;一致性哈希（CONNASH）： 适合按请求资源分配机器的场景，如通过这种方式提升 cdn 命中率。&quot;一致性哈希，跟cdn有什么关系啊</div>2025-02-17</li><br/><li><span>Geek_706285</span> 👍（0） 💬（2）<div>1.在lb的nginx配置文件中将up_stream中配置中添加新server的ip端口，重启nginx服务 2.我自己把nginx配置文件中的http注释后重启服务发现不行，是因为http是应用层而tcp是传输层的缘故吗</div>2025-02-17</li><br/>
</ul>