...
<li><span>糖醋🏀</span> 👍（76） 💬（4）<div>java7开始io就有try-with-resource。
可以利用这一个特性，来说实现，自动释放。
代码如下：
...
}</div>2019-09-01</li><br/><li><span>humor</span> 👍（17） 💬（1）<div>&#47;**
*业务调用接口
**&#47;
...
}</div>2019-09-01</li><br/><li><span>Cast</span> 👍（11） 💬（4）<div>老师，请问为什么要按逆序去释放锁呢？按照获取的顺序去释放好像也没什么毛病吧？</div>2019-09-01</li><br/><li><span>喜欢地球的阿培同学</span> 👍（7） 💬（1）<div>老师突然想到一个问题，假设现在100个线程，一个线程正在运行，99个线程正在阻塞(等待锁释放)，那么会导致CPU上下文频繁切换吗？</div>2020-04-17</li><br/><li><span>道</span> 👍（1） 💬（1）<div>阻塞队列能够解决大部分并发访问的问题，Golang对他就提供了语言层面的支持，为何现实中用的不多哩？</div>2020-02-03</li><br/><li><span>长期规划</span> 👍（1） 💬（1）<div>那个读写锁，读锁和写锁之间应该也是互斥的吧</div>2019-10-08</li><br/><li><span>长期规划</span> 👍（0） 💬（3）<div>老师，只要保证所有使用锁的地方都按相同的顺序获取，而且按获取的顺序的反序解锁，应该就不会发生死锁了吧。</div>2019-10-08</li><br/><li><span>明日</span> 👍（0） 💬（1）<div>请问老师这种在new 对象过程中锁定共享锁的方式是否可行 
 代码: https:&#47;&#47;gist.github.com&#47;imgaoxin&#47;91234a9edbf083b10244221493ce7fb5</div>2019-09-03</li><br/><li><span>刘天鹏</span> 👍（0） 💬（1）<div>对于golang应该就是这样吧
func foo(){
  lock.Lock()
  defer lock.Unlock()
  &#47;&#47;do something...
}</div>2019-09-02</li><br/><li><span>你说的灰</span> 👍（0） 💬（2）<div>
public void visitShareResWithLock() {
  lock.lock();
  try {
    &#47;&#47; 在这里安全的访问共享资源
 ...
 lock.lock(); 加锁语句放在 try catch 里面是否可以。为什么很多示例代码都放在外面。</div>2019-09-02</li><br/><li><span>Geek_q29f6t</span> 👍（0） 💬（2）<div>老师，请教一个问题：假设有一个方法在计算报表，但这个计算的线程在执行过程中被意外释放了（并不是抛异常）， 此时try catch捕获是捕获不到这种情况的。而从客户端看来，这个计算过程就永远停在那里了，而后台又没能力告诉客户端：“你别等了”。 这种情况应该如果处理呢？</div>2019-08-31</li><br/><li><span>树梢的果实</span> 👍（0） 💬（1）<div>C语言下，通过宏很容易实现try-with-lock。
...
</div>2019-08-31</li><br/><li><span>菜鸟和谐号</span> 👍（46） 💬（10）<div>既然是消息队列自然要多讲讲消息队列的知识，一下io，一下缓存，一下多线程，讲的很杂，也不深入。</div>2019-09-08</li><br/><li><span>张三</span> 👍（7） 💬（1）<div>幸亏学过极客时间的并发编程专栏，看懂了。我觉得并发容器的选择比较复杂。</div>2019-08-31</li><br/><li><span>long</span> 👍（3） 💬（0）<div>要是能讲一下, 设计消息队列, 哪里需要用到锁, 就完美了.</div>2021-03-24</li><br/>
</ul>