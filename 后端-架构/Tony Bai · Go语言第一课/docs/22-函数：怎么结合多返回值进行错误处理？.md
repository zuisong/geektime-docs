...
<li><span>ROYIS</span> 👍（14） 💬（3）<div>这节课瞬间看不懂了，是不是最好先讲接口等其他知识点再讲这个呢？好像这节课跟函数没什么关系呀</div>2022-02-10</li><br/><li><span>五彩斑斓的輝</span> 👍（6） 💬（1）<div>为什么内置的类型，如int、error等，首字母都是小写的，可以不用遵从首字母大写导出规则，到处都可以使用。是因为它们是在最外层的宇宙代码块里面声明的吗，这在实际上怎么实现的？</div>2022-05-19</li><br/><li><span>郑印</span> 👍（5） 💬（2）<div>如果函数调用太深，每次做返回值判断都需要写，if err != nil { return err; } 感觉会多出好多的模板代码，有什么好的解决办法吗？</div>2021-12-23</li><br/><li><span>Geralt</span> 👍（5） 💬（1）<div>忽略错误算不算实用的策略呢
</div>2021-12-02</li><br/><li><span>Witt</span> 👍（5） 💬（10）<div>if err != nil { return err; }

看错误怎么能少得了这个 🐶</div>2021-12-01</li><br/><li><span>🦉</span> 👍（4） 💬（2）<div>请问文章这里
var e *MyError
if errors.As(err, &amp;e) {
    &#47;&#47; 如果err类型为*MyError，变量e将被设置为对应的错误值
}
&amp;e是pointer e的地址吗？如果是的话为什么这样处理呢？谢谢</div>2022-01-11</li><br/><li><span>在下宝龙、</span> 👍（4） 💬（8）<div>最后一部分的策略四的设计不是很懂  。。。</div>2021-12-01</li><br/><li><span>aoe</span> 👍（3） 💬（1）<div>多值返回的错误处理很好用。知道了4种错误使用场景，变强了！</div>2021-12-02</li><br/><li><span>Unknown element</span> 👍（2） 💬（1）<div>老师我想请教一下错误处理的策略二和策略三的区别是error具体的实现不同吗？哨兵错误处理策略相当于用的还是errorString，而错误值类型检视策略是自定义的struct，可以包含更多上下文？</div>2022-10-25</li><br/><li><span>冯仁彬</span> 👍（2） 💬（3）<div>感觉错误处理机制相对于Java没那么好用</div>2022-09-13</li><br/><li><span>Thomas</span> 👍（2） 💬（1）<div>“因此，OpError 实例可以被错误处理方通过net.Error接口的方法，判断它的行为是否满足 Temporary 或 Timeout 特征。”
是否需要否满足 Temporary 或 Timeout 中任意一个特征，e.(net.Error)返回的OK就是true？</div>2022-03-27</li><br/><li><span>罗杰</span> 👍（2） 💬（1）<div>再读错误处理，依然收获满满。</div>2021-12-01</li><br/><li><span>人言有力</span> 👍（1） 💬（1）<div>本文讲解了Go语言函数多返回值进行错误处理的目的、使用方法、应用场景和注意事项
...
2.处理上所有的错误都应该有处理，不能直接吞掉；其次是系统异常应该中止流程；业务异常则视情况而定，一般也要中断流程。</div>2024-05-16</li><br/><li><span>℡人见人爱浩然君゜</span> 👍（1） 💬（1）<div>越到后面越有点看不懂...
</div>2023-04-07</li><br/><li><span>被水淹没</span> 👍（1） 💬（1）<div>请教下老师：

&gt; 如果err类型为*MyError，变量e将被设置为对应的错误值

这里为何要赋值回变量e呢，相当于判断了类型，并且还把原始的错误给提取出来了？
后续这个e有什么实际的应用吗？</div>2023-02-12</li><br/>
</ul>