...
<li><span>步比天下</span> 👍（16） 💬（5）<div>老师，我想问个低级的问题，什么是临界区啊，不太懂</div>2022-01-16</li><br/><li><span>罗杰</span> 👍（11） 💬（6）<div>连续三节课都是需要花费 30 分钟才能阅读完，即使是复习也要超过 20 分钟，这几节的内容真的好充实。</div>2022-01-14</li><br/><li><span>Calvin</span> 👍（10） 💬（5）<div>思考题：
...
func TestDeadLock(t *testing.T) {
	...
	go op2(&amp;mu1, &amp;mu2, &amp;wg)
	wg.Wait()
}</div>2022-01-15</li><br/><li><span>Geek_62c21a</span> 👍（6） 💬（1）<div>老师，有两个问题请教您
...
ready = true 
groupSignal.L.Unlock()
groupSignal.Broadcast()</div>2022-04-25</li><br/><li><span>aoe</span> 👍（6） 💬（1）<div>读到这里，明白了 Go 的并发不是万能的！
...
3. 有锁的地方就有江湖，高并发下的性能主要拼的是算法，没有一门语言有压倒性优势</div>2022-01-18</li><br/><li><span>Calvin</span> 👍（5） 💬（9）<div>老师，Go 中没有可重入的锁吗？</div>2022-01-15</li><br/><li><span>非梧桐不止</span> 👍（4） 💬（2）<div>老师，既然共享内存的性能比channel好，为什么Go倡导用通信去共享内存呢？</div>2022-05-27</li><br/><li><span>singularity of space time</span> 👍（3） 💬（2）<div>老师，请问一下Go在没有volatile的情况下如何保证共享变量在不同Goroutine的可见性？如果可见性不能保证的话，那么CAS的正确性应该也不能保证吧？还是说CAS内部的实现已经保证了可见性呢？</div>2022-02-06</li><br/><li><span>文经</span> 👍（3） 💬（1）<div>白老师，我觉得条件变量的例子完全可以用channel来实现，代码逻辑还会更简单一些。
使用条件变量，跟Mutex一样，是基于性能的考虑吗？</div>2022-01-21</li><br/><li><span>文经</span> 👍（3） 💬（1）<div>白老师：
“atomic 原子操作可用来同步的范围有比较大限制，只能同步一个整型变量或自定义类型变量。如果我们要对一个复杂的临界区数据进行同步，那么首选的依旧是 sync 包中的原语。”
这里说的整型变量和自定义类型变量，是不是可以理解为一个字长的大小。在64位cpu上就是8个字节。因为CPU通过数据总线，一次从内存中最多只能获取一个字长的信息。所以atomic的限制也是一个字长。</div>2022-01-21</li><br/><li><span>Calvin</span> 👍（3） 💬（5）<div>老师，sync 包中还有其他的一些同步原语，后面会继续说下吗？比如前面课程和本节课中都有使用到的 sync.WaitGroup 就没有说到，直接就使用上了，另外还有 sync.Map，sync.Pool 等。</div>2022-01-15</li><br/><li><span>吃橘子的汤圆🐳</span> 👍（2） 💬（1）<div>请问使用Csp比使用共享内存的好处体现在哪里</div>2022-12-13</li><br/><li><span>冯仁彬</span> 👍（2） 💬（1）<div>以下代码会产生死锁:
func main() {
	var mu1 sync.Mutex
	var mu2 sync.Mutex
	mu1.Lock()
	...
	fmt.Println(&quot;main&quot;)
}</div>2022-09-22</li><br/><li><span>菠萝吹雪—Code</span> 👍（2） 💬（1）<div>并发这几节课需要多看几遍，老师讲解的太棒了！充实。
空结构体在channel中传递的好处，可以参考这篇文章关于空结构体的应用场景：https:&#47;&#47;mp.weixin.qq.com&#47;s&#47;zbYIdB0HlYwYSQRXFFpqSg</div>2022-09-09</li><br/><li><span>lesserror</span> 👍（2） 💬（1）<div>关于Go的并发，内容挺多的。专门讲Go并发编程的书籍都有不少。大白老师这里带领我们通俗易懂地入门了。另有几个小问题。
...
3.  $GOROOT&#47;src&#47;runtime&#47;internal&#47;asm_amd64.s 这个文件未找到（Mac Book 上面）。</div>2022-01-20</li><br/>
</ul>