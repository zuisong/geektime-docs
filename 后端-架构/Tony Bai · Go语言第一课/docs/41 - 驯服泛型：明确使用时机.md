...
<li><span>Geek14</span> 👍（4） 💬（2）<div>请教老师两个问题：
...
希望老师有时间帮忙解答下疑惑。</div>2023-01-06</li><br/><li><span>Geek14</span> 👍（3） 💬（1）<div>&#47;&#47; 定义一个支持比较的接口，用于类型参数约束
type ordered interface {
	~int | ~int8 | ~int16 | ~int32 | ~int64 |
	~uint | ~uint8 | ~uint16 | ~uint32 | ~uint64 | ~uintptr |
...
func SortGeneric[T ordered](s SortableSlice[T])  {
	sort.Sort(s)
}</div>2023-01-06</li><br/><li><span>return</span> 👍（1） 💬（1）<div>老师讲的好呀，感谢老师。
期待老师新作品。</div>2022-12-07</li><br/><li><span>Calvin</span> 👍（1） 💬（1）<div>思考题：
...
}</div>2022-11-10</li><br/><li><span>lesserror</span> 👍（0） 💬（1）<div>tony bai 老师，文中的：“并且被保存在 ELF 的只读数据区段（.data）中”，这里的ELF是什么的缩写呢？</div>2023-08-07</li><br/><li><span>罗杰</span> 👍（0） 💬（1）<div>看一遍肯定是不够的，🉐️好好吸收</div>2022-11-09</li><br/>
</ul>